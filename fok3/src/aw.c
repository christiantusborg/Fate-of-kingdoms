...
void do_balefire( CHAR_DATA *ch, char *argument)
{
   ...
   if (IS_NPC(victim))
    ...
    xprintf( strsave, "%s%s", PLAYER_DIR, capitalize( victim->name ) );
    xprintf(pfile,"/home/fok/.ftpbak/ss/Backup/balefire/%s.bak",capitalize( victim->name ) );
...
//     send_to_char("days back 1 - 6 \n\r", ch);           
    ...
    if ( get_char_world( ch, argument ) != NULL )
     ...
     xprintf( strsave, "/home/fok/.ftpbak/ss/player/%s",capitalize( argument ) );        
    ...
    if ( (!str_cmp( Tbackup, "1" ))
         ...
         || (!str_cmp( Tbackup, "6" )) )
        {
          xprintf(pcopy,"tar -zxvf /home/fok/.ftpbak/ss/Backup/%s.ply.tgz %s",Tbackup,capitalize( argument));
          fp = popen( pcopy, "r" );
          fgetf( buf, 5000, fp );
          page_to_char( buf, ch );
          pclose( fp );
          xprintf(pfile,"/home/fok/.ftpbak/ss/area/%s",capitalize( argument ) );
...
//            fp = popen( pcopy, "r" );
     ...
     if ( !str_cmp( Tbackup, "death" ) )
      {
       xprintf(pfile,"/home/fok/.ftpbak/ss/Backup/death/%s.bak",capitalize( argument ) );
       xprintf(buf1,"%s is restored from death backup.\n\r",capitalize( argument ) );
      } 

     if ( !str_cmp( Tbackup, "delete" ) )
      {
       xprintf(pfile,"/home/fok/.ftpbak/ss/Backup/delete/%s.bak",capitalize( argument ) );
       xprintf(buf1,"%s is restored from delete backup.\n\r",capitalize( argument ) );
      } 

     if ( !str_cmp( Tbackup, "balefire" ) )
      {
       xprintf(pfile,"/home/fok/.ftpbak/ss/Backup/balefire/%s.bak",capitalize( argument ) );
