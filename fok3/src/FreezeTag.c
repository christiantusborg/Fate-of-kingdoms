...
void do_red( CHAR_DATA *ch, char *argument )
{
    ...
    {
        xprintf(buf,"{R{{RED}{B %s{x: %s\n\r",ch->name,argument);
        send_to_char(buf, ch);
    }
    else
    {
        xprintf(buf,"{R{{RED}{R %s{x: %s\n\r",ch->name,argument);
    }
...
void do_blue( CHAR_DATA *ch, char *argument )
{
    ...
    {
        xprintf(buf,"{B{{BLUE}{R %s{x: %s\n\r",ch->name,argument);
        send_to_char(buf, ch);
    }
    else
    {
        xprintf(buf,"{B{{BLUE}{B %s{x: %s\n\r",ch->name,argument);
    }
...
void do_ftag( CHAR_DATA *ch, char *argument )
{
      ...
      for ( d = descriptor_list; d != NULL; d = d->next )
        {
          ...
          if ( IS_SET(d->character->tag_flags,TAG_PLAYING))
          {
            ...
            {
                xprintf(buf,"%s %d",d->character->name,ROOM_VNUM_TEMPLE);
                do_transfer(ch,buf);
            }
          }
    ...
    if ( !str_cmp( arg1, "start" ) )
    {
        for ( d = descriptor_list; d != NULL; d = d->next )
        {
          ...
          if ( IS_SET(d->character->tag_flags,TAG_PLAYING)
                 && !IS_NPC(d->character))
          {
            xprintf(buf,"%s %d",d->character->name,
    ...
    if ( !str_cmp( arg1, "auto" ) )
    {
                        ...
                        victim = victim->next_in_room )
       {
          ...
          if ((fRed = !fRed))
          {
            xprintf(buf,"red %s",victim->name);
          }
          else
          {
            xprintf(buf,"blue %s",victim->name);
          }
