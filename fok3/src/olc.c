...
void do_aedit( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( arg, "create" ) )
	{
		...
		{
	 send_to_char( "AEdit : You do not have the security to do that.\n\r",ch);
         return;
    }
       aedit_create( ch, "" );
    ...
    if (!IS_BUILDER(ch,pArea))
    {
    	send_to_char("You are not the builder...\n\r",ch);
    	return;
    }
...
void do_oedit( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "OEdit:  Try Oedit create <vnum>.\n\r", ch );
    return;
}
...
void do_medit( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "MEdit:  Try medit create <vnum>.\n\r", ch );
    return;
}
...
void display_resets( CHAR_DATA *ch )
{
  ...
  "\n\r", ch );
	...
	final[0] = '\0';
	xprintf( final, "[%2d] ", ++iReset );

	switch ( pReset->command )
	{
	default:
	    xprintf( buf, "Bad reset command: %c.", pReset->command );
	    xprintf( final, buf );
	    break;
	...
	case 'M':
	    ...
	    if ( !( pRoomIndex = get_room_index( pReset->arg3 ) ) )
	    {
            ...
            pMob = pMobIndex;
            xprintf( buf, "M[%5d] %-13.13s in room             R[%5d] %2d-%2d %-15.15s\n\r",
	...
	case 'O':
	    ...
	    {
                xprintf( buf, "Load Object - Bad Object %d\n\r",
		    pReset->arg1 );
                xprintf( final, buf );
                continue;
	    }
	...
	case 'P':
	    ...
	    {
                xprintf( buf, "Put Object - Bad To Object %d\n\r",
                    pReset->arg3 );
                xprintf( final, buf );
                continue;
	    }
	...
	case 'E':
	    if ( !( pObjIndex = get_obj_index( pReset->arg1 ) ) )
	    {
                xprintf( buf, "Give/Equip Object - Bad Object %d\n\r",
                    pReset->arg1 );
                xprintf( final, buf );
                continue;
	    }
	    ...
	    if ( !pMob )
	    {
                xprintf( buf, "Give/Equip Object - No Previous Mobile\n\r" );
                xprintf( final, buf );
                break;
	    }
	...
	case 'R':
	    ...
	    {
		xprintf( buf, "Randomize Exits - Bad Room %d\n\r",
		    pReset->arg1 );
		xprintf( final, buf );
		continue;
	    }
    ...
    if (!temp) 
      {
		...
		if ( !str_cmp( arg4, "room" ) )
		{
		    if (get_obj_index(atoi(arg3)) == NULL)
		      {
		         send_to_char( "This vnum doesn't exist.\n\r",ch);
		         return;
		      }
		    pReset->command  = 'O';
		    pReset->arg2     = 0;
		 ...
		 */
		{
		    ...
		    if (get_obj_index(atoi(arg3)) == NULL)
		      {
		         send_to_char( "This vnum don't exist.\n\r",ch);
		         return;
		      }
		    pReset->arg1 = atoi(arg3);
...
void do_alist( CHAR_DATA *ch, char *argument )
{
    ...
    xprintf( result, "[%3s] [%-27s] (%-5s-%5s) [%-10s] %3s [%-10s]\n\r",
       "Num", "Area Name", "lvnum", "uvnum", "Filename", "Sec", "Builders");
        send_to_char( result, ch );
        
    for ( pArea = area_first; pArea; pArea = pArea->next )
    {
        xprintf( buf, "[%3d] %-29.29s (%-5d-%5d) %-12.12s [%d] [%-10.10s]\n\r",
             pArea->vnum,
             pArea->name,
             pArea->min_vnum,
             pArea->max_vnum,
