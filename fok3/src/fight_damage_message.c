...
vp = "{r<={RCREMATES{r=>{w";}
 ...
 vp = "does {WUNSPEAKABLE{w things to"; }
     ...
     punct   = (dam <= 24) ? '.' : '!';
    xprintf(buf4, " {R({Y%d{R){w", dam);

    if ( dt == TYPE_HIT )
    {
	if (ch  == victim)
	{
	xprintf( buf1, "$n %s $melf%c",vp,punct);
	xprintf( buf2, "You %s yourself%c%s",vs,punct,(IS_SET(ch->act,PLR_AUTODAMAGE) ? buf4 : ""));
	xprintf( buf5, "%s %s %s",ch->name,vp,victim->name);
	}
	else
	{
   	xprintf( buf1, "$n %s $N%c",  vp, punct );
        xprintf( buf2, "You %s $N%c%s", vs, punct,(IS_SET(ch->act,PLR_AUTODAMAGE) ? buf4 : "" ));
        xprintf( buf3, "$n %s you%c%s", vp, punct,
	...
	if (immune)
	{
	    if (ch == victim)
	    {
		xprintf(buf1,"$n is unaffected by $s own %s.",attack);
		xprintf(buf2,"Luckily, you are immune to that.");
		xprintf(buf5,"%s is unaffected by %s.",ch->name,attack);
	    }
	    else
	    {
	    	xprintf(buf1,"$N is unaffected by $n's %s!",attack);
	    	xprintf(buf2,"$N is unaffected by your %s!",attack);
	    	xprintf(buf3,"$n's %s is powerless against you.",attack);
		xprintf(buf5,"%s is unaffected by %s's %s!",victim->name,ch->name,attack);
	    }
	}
	else
	{
	    if (ch == victim)
	    {
		xprintf( buf1, "$n's %s %s $m%c",attack,vp,punct);
                xprintf( buf2, "Your %s %s you%c%s",attack,vp,punct,(IS_SET(ch->act,PLR_AUTODAMAGE) ? buf4 : "" ));
                xprintf( buf5, "%s's %s %s %s",ch->name,attack,vp,victim->name);
	    }
	    else
	    {
         xprintf( buf1, "$n's %s %s $N%c%s",  attack, vp, punct,(IS_SET(ch->act,PLR_AUTODAMAGE) ? buf4 : "" ) );
                xprintf( buf2, "Your %s %s $N%c%s",  attack, vp, punct,(IS_SET(ch->act,PLR_AUTODAMAGE) ? buf4 : "" ));
                xprintf( buf3, "$n's %s %s you%c%s", attack, vp, punct,
		((IS_SET(victim->act,PLR_AUTODAMAGE) &&
		IS_SET(ch->act,PLR_AUTODAMAGE))? buf4 : "" ));
                xprintf( buf5, "%s's %s %s %s",ch->name,attack,vp,victim->name);
	    }
    ...
    if ( (dam > pblood1) && (victim->hit < pblood2) )
     {
      ...
      xprintf( pbuf1, "{R%s's blood flies everywhere{X",victim->name);
      xprintf( pbuf2, "{R%s's blood flies everywhere{X",victim->name);
      xprintf( pbuf3, "{RYour blood flies everywhere{X");
