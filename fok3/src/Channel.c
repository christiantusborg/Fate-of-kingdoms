...
void public_ch_socials ( CHAR_DATA *ch, char *argument, char *type, const int bitname,int auth)
{
	...
	if (argx[0] == '\0')
	  {
		xprintf(buf, "%s %s {x", type, social_table[cmd].char_no_arg );
		act_new(buf,ch,NULL,NULL,TO_CHAR,POS_DEAD);
		xprintf(buf, "%s %s {x",type, social_table[cmd].others_no_arg );
	  ...
	  {
		xprintf(buf,"%s %s {x",type, social_table[cmd].char_auto);
		act_new(buf,ch,NULL,NULL,TO_CHAR,POS_DEAD);
		xprintf(buf,"%s %s {x",type, social_table[cmd].others_auto);
	  ...
	  {
		xprintf(buf,"%s{X %s {x", type, social_table[cmd].char_found);
		act_new(buf,ch,NULL,victim,TO_CHAR,POS_DEAD);
		xprintf(buf,"%s{X %s {x",type, social_table[cmd].vict_found);
			 ...
			 && !IS_SET(vch->comm,COMM_QUIET))
			  {  
				if ( (NEW_IS_AUTHORIZED( vch, auth)) 
				 &&  (bitname != COMM_NOKINGDOM || victim->kingdom == ch->kingdom) )
				  {        
					xprintf(buf,"%s{X %s {x",type,social_table[cmd].others_found); 
	...
	if (argument[0] == '\0' )
	  {
		if (IS_SET(ch->comm,(bitname)))
		  {
			xprintf( buf, "%s channel is now ON.{x\n\r", type);
			send_to_char( buf ,ch);
			REMOVE_BIT(ch->comm,(bitname));
		  }
		else
		  {
			xprintf( buf, "%s channel is now OFF.{x\n\r", type);
			send_to_char(buf,ch);
		...
		if (!str_cmp(command, "emote") || !str_cmp(command, "em") ||!str_cmp(command, "!"))
		  {
			...
			xprintf( buf, "%s %s {X%s{x\n\r", type,ch->name, argument );
			send_to_char( buf, ch);
			xprintf( buf, "%s $n {X%s{x", type, argument );
		  }
		else
		  {             
		      	xprintf( buf, "%s You say{X: '%s'{x\n\r", type, buf3 );
			send_to_char(buf,ch);
			xprintf( buf, "%s $n:{X '%s'{x", type, buf3 );
		  } 
...
{
	public_ch( ch, argument, "{W[{DImplementor{W]{w", COMM_IMP,auth_imp);
}
