...
//    char buf[MAX_STRING_LENGTH];
    	...
    	if( difftime( current_time , fst.st_mtime ) > days*60.0*60.0*24.0 )
	{
	    ...
	    xprintf( new_file, "%s%s" , ARCHIVE_DIR, capitalize(name) );
  	    rename( old_file, new_file );
//	    xprintf( buf, "gzip -fq %s", new_file );
//	    system( buf );
	}
    }
}    
