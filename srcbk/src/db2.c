...
*/
        ...
        pMobIndex->mastery              = fread_number( fp );
...
//mbw
/*
	if (pMobIndex->wealth >10000)
	{
		char buf[1024];
	        FILE * eq = fopen("wealth.log","a+");
		xprintf(buf,"%d\r\n",pMobIndex->vnum);
...
void load_objects( FILE *fp )
{
    ...
    for ( ; ; )
    {
//    int hp=0,hitroll=0,mana=0,damroll=0,wismod=0,statmod=0;	
        int vnum;
        char letter;
        int iHash;
	...
	switch (letter)
 	{
		...
		paf->where		= TO_OBJECT;
            ...
            /*    switch (paf->location)
                {
                 case APPLY_HIT:   hp   += (paf->modifier>0) ? paf->modifier : paf->modifier/2; break;
		 case APPLY_MANA: mana += (paf->modifier>0) ? paf->modifier : paf->modifier/2; break;
		 case APPLY_HITROLL: hitroll += (paf->modifier>0) ? paf->modifier : paf->modifier/2; break;
		 case APPLY_DAMROLL: damroll += (paf->modifier>0) ? paf->modifier : paf->modifier/2; break;
		 case APPLY_WIS: wismod+= (paf->modifier>0) ? paf->modifier : paf->modifier/2; break;
		 case APPLY_STR: 
		 case APPLY_DEX: 
		 case APPLY_INT: 
		 case APPLY_CON: 
			statmod += (paf->modifier>0) ? paf->modifier : paf->modifier/2; break;
		}*/
 ...
 /*
 	if ((hp*2 + mana + hitroll*10 + damroll*10 + statmod*20 > pObjIndex->level * 5) || wismod > 2)
 	{
 		char buf[1024];
		FILE * eq = fopen("eq.log", "a+");
		
		if (eq!=NULL)
		{
			xprintf(buf, "%d \t%d\t%d\t", pObjIndex->vnum, hp, mana);
			fprintf(eq,buf);
			xprintf(buf, "%d\t%d\t%d\t%d\n\r", hitroll, damroll, statmod, wismod);
			fprintf(eq,buf);
			fclose(eq);
		}
