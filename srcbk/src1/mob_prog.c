	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
     ...
     */
    if ( mob->mob_mem == NULL && ch!=NULL)
	{
		mem = alloc_mem(sizeof(MOB_MEMORY));
		mem->next = mob->mob_mem;
		mem->target = ch;
		mem->value = -1;
		mob->mob_mem = mem;
		//mob->mprog_target = ch;
	...
	{
	    xprintf( buf, "Cmd_eval_mob: prog %ld syntax error(2) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
	}
    ...
    if ( buf[0] != '$' || buf[1] == '\0' )
    {
	xprintf( buf, "Cmd_eval_mob: prog %ld syntax error(3) '%s'",
		vnum, original );
	bug( buf, 0 );
        return FALSE;
    }
	...
	case 'q':
		if (mob->memory)
		{
			lval_char = mob->mob_mem->target; 
			fromMem = TRUE;
		} else
			lval_char = NULL;
		break;
	default:
	    xprintf( buf, "Cmd_eval_mob: prog %ld syntax error(4) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
    }
	...
	mem = mob->mob_mem;
	//for (mem=mob->mob_mem; mem!=NULL; mem=mem->next)
	do
	{
		if (fromMem == TRUE)
			lval_char = mem->target;
		/*
		...
		switch( check )
		{
			...
			case CHK_ISDELAY:
				...
				if (lval_char->mprog_delay > 0) return TRUE;
				for (mem=lval_char->mob_mem; mem!=NULL; mem=mem->next)
					if (mem->age > 0) return TRUE;
				return FALSE;
			...
			case CHK_HASTARGET:
				...
				for (mem = lval_char->mob_mem; mem!=NULL; mem=mem->next)
				{
					if (lval_char->in_room == mem->target->in_room) return TRUE;
				}
				return FALSE;
				//return( lval_char != NULL && lval_char->mob_mem != NULL
				//&&  lval_char->in_room == lval_char->mprog_target->in_room );
			case CHK_ISTARGET:
				if (lval_char==NULL) return FALSE;
				for (mem = mob->mob_mem; mem!=NULL; mem=mem->next)
				{
					if (lval_char == mem->target) return TRUE;
				}
				return FALSE;
		 ...
		 {
		 case CHK_CLAN:
			 return ( lval_char != NULL && lval_char->kingdom !=NULL && !strcmp(kingdom_table[lval_char->kingdom].name, buf));
			case CHK_AFFECTED:
			...
			case CHK_KNOW:
				if (fromMem == TRUE) //if testing against a character in memory, always return true
					return TRUE;
				else
				{   //search through all the remembered people, then if any of them is the target, 
					//test against value.  If value is the same, return true, otherwise false
					//if noone is found return false
					for (mem = mob->mob_mem; mem!=NULL; mem=mem->next)
					{
						if (mem->target == lval_char)
						{
							if (mem->value == atoi(buf)) 
								return TRUE;
							else
								return FALSE;
						}
		...
		if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
		{
			xprintf( buf, "Cmd_eval_mob: prog %ld syntax error(5): '%s'",
				vnum, original );
			bug( buf, 0 );
			return FALSE;
		}
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
	{
	    xprintf( buf, "Cmd_eval_obj: prog %ld syntax error(2) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
	}
    ...
    if ( buf[0] != '$' || buf[1] == '\0' )
    {
	xprintf( buf, "Cmd_eval_obj: prog %ld syntax error(3) '%s'",
		vnum, original );
	bug( buf, 0 );
        return FALSE;
    }
	...
	default:
	    xprintf( buf, "Cmd_eval_obj: prog %ld syntax error(4) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
    }
    ...
    if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
    {
	xprintf( buf, "Cmd_eval_obj: prog %ld syntax error(5): '%s'",
		vnum, original );
	bug( buf, 0 );
	return FALSE;
    }
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
	{
	    xprintf( buf, "Cmd_eval_room: prog %ld syntax error(2) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
	}
    ...
    if ( buf[0] != '$' || buf[1] == '\0' )
    {
	xprintf( buf, "Cmd_eval_room: prog %ld syntax error(3) '%s'",
		vnum, original );
	bug( buf, 0 );
        return FALSE;
    }
	...
	default:
	    xprintf( buf, "Cmd_eval_room: prog %ld syntax error(4) '%s'",
		vnum, original );
	    bug( buf, 0 );
	    return FALSE;
    }
    ...
    if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
    {
	xprintf( buf, "Cmd_eval_room: prog %ld syntax error(5): '%s'",
		vnum, original );
	bug( buf, 0 );
	return FALSE;
    }
    ...
    while ( *str != '\0' )
    {
        ...
        switch ( *str )
        {
            ...
            case 'n': 
				...
				break;
            case 'N': 
		    	i = (ch != NULL && can_see( mob, ch ) )	? ( IS_NPC( ch ) ? ch->short_descr : ch->name ) : someone;                         		
				break;
            case 't': 
				i = someone;
				...
				break;
            case 'T': 
		    	i = (vch != NULL && can_see( mob, vch )) ? ( IS_NPC( vch ) ? vch->short_descr : vch->name ) : someone;  
				break;
            case 'r': 
				if ( rch == NULL ) 
            ...
            case 'R': 
				if ( rch == NULL ) 
					rch = get_random_char( mob, NULL, NULL );
				i  = ( rch != NULL && can_see( mob, rch ) )	? ( IS_NPC( ch ) ? ch->short_descr : ch->name )	:someone;					
				break;
			case 'q':
				i = someone;
				mem = NULL;
				if (ch!=NULL)
				{
					for (mem=mob->mob_mem; mem!=NULL; mem=mem->next)
					{
						if (ch==mem->target)
						{
							one_argument(mem->target->name, fname);
							i = capitalize(fname);
							break;
						}
					}
				}
				if (mem==NULL && mob->mob_mem!=NULL) 
				{
					one_argument(mob->mob_mem->target->name, fname);
					i = capitalize(fname);
				}
			...
			case 'Q':
				...
				if (mem==NULL && mob->mob_mem!=NULL) 
				{
					if (IS_NPC(mob->mob_mem->target))
						i = mob->mob_mem->target->short_descr;
					else
						i = mob->mob_mem->target->name;
				}
	    		//i = (mob->mprog_target != NULL && can_see( mob, mob->mprog_target )) ? ( IS_NPC( mob->mprog_target ) ? mob->mprog_target->short_descr : mob->mprog_target->name ): someone;                         		
				break;
            case 'j': 
				i = he_she  [URANGE(0, mob->sex, 2)];     
				break;
            case 'e': 
	    		i = (ch != NULL && can_see( mob, ch )) ? he_she  [URANGE(0, ch->sex, 2)] : someone;					
				break;
            case 'E': 
	    		i = (vch != NULL && can_see( mob, vch )) ? he_she  [URANGE(0, vch->sex, 2)] : someone;					
				break;
            case 'J': 
				i = (rch != NULL && can_see( mob, rch )) ? he_she  [URANGE(0, rch->sex, 2)] : someone;					
				break;
			case 'X':
				i=someone;
				mem = NULL;
				if (ch!=NULL)
				{
					for (mem=mob->mob_mem; mem!=NULL; mem=mem->next)
					{
						if (ch==mem->target && can_see(mob, ch))
						{
				...
				//i = (mob->mprog_target != NULL && can_see( mob, mob->mprog_target))	? he_she  [URANGE(0, mob->mprog_target->sex, 2)] : someone;					
				break;
            case 'k': 
				i = him_her [URANGE(0, mob->sex, 2)];	
				break;
            case 'm': 
	    		i = (ch != NULL && can_see( mob, ch )) ? him_her [URANGE(0, ch  ->sex, 2)] : someone;        				
				break;
            case 'M': 
	    		i = (vch != NULL && can_see( mob, vch )) ? him_her [URANGE(0, vch ->sex, 2)] : someone;					
				break;
            case 'K': 
				if ( rch == NULL ) rch = get_random_char( mob, NULL, NULL );
				i = (rch != NULL && can_see( mob, rch )) ? him_her [URANGE(0, rch ->sex, 2)] : someone;					
				break;
            case 'Y': 
				i=someone;
				mem = NULL;
	    		...
	    		//i = (mob->mprog_target != NULL && can_see( mob, mob->mprog_target )) ? him_her [URANGE(0, mob->mprog_target->sex, 2)] : someone;					
				break;
            case 'l': 
				i = his_her [URANGE(0, mob ->sex, 2)];    
				break;
            case 's': 
	    		i = (ch != NULL && can_see( mob, ch )) ? his_her [URANGE(0, ch ->sex, 2)] : someones;					
				break;
            case 'S': 
	    		i = (vch != NULL && can_see( mob, vch )) ? his_her [URANGE(0, vch ->sex, 2)] : someones;					
				break;
            case 'L': 
				if ( rch == NULL ) 
					rch = get_random_char( mob, NULL, NULL );
				i = ( rch != NULL && can_see( mob, rch ) ) ? his_her [URANGE(0, rch ->sex, 2)] : someones;					
				break;
            case 'Z': 
				i=someone;
				mem = NULL;
	    		...
	    		//i = (mob->mprog_target != NULL && can_see( mob, mob->mprog_target )) ? his_her [URANGE(0, mob->mprog_target->sex, 2)] : someones;					
				break;
			case 'o':
				i = something;
				...
				break;
            case 'O':
                i = (obj1 != NULL && can_see_obj( mob, obj1 )) ? obj1->short_descr : something;					
				break;
            case 'p':
				i = something;
				...
				break;
            case 'P':
            	i = (obj2 != NULL && can_see_obj( mob, obj2 )) ? obj2->short_descr : something;					
				break;
        }
...

{
    ...
    {
	if ( mob )
	    xprintf( bugbuf, "Progs: MAX_CALL_LEVEL exceeded, vnum %d, mprog vnum %d",
	      mvnum, pvnum );
	else if ( obj )
	    xprintf( bugbuf, "Progs: MAX_CALL_LEVEL exceeded, vnum %d oprog vnum %d.",
	      ovnum, pvnum );
	else
	    xprintf( bugbuf, "Progs: MAX_CALL_LEVEL exceeded, vnum %d rprog vnum %d.",
	      rvnum, pvnum );
	bug( bugbuf, 0 );
	call_level--;
	return;
	...
	*b = *c = *d = '\0';
	...
	if ( !str_cmp( control, "if" ) )
	{
	    ...
	    {
		if ( mob )
		    xprintf( buf, "Mobprog: misplaced if statement, mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: misplaced if statement, obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: misplaced if statement, room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		call_level--;
		return;
	    ...
	    state[level] = BEGIN_BLOCK;
            ...
            {
		if ( mob )
		    xprintf( buf, "Mobprog: Max nested level exceeded, mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: Max nested level exceeded, obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: Max nested level exceeded, room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		call_level--;
		return;
	    ...
	    {
		if ( mob )
		    xprintf( buf, "Mobprog: invalid if_check (if), mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: invalid if_check (if), obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: invalid if_check (if), room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		call_level--;
		return;
	...
	else if ( !str_cmp( control, "or" ) )
	{
	    ...
	    {
		if ( mob )
		    xprintf( buf, "Mobprog: or without if, mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: or without if, obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: or without if, room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		call_level--;
		return;
		...
		eval = cmd_eval_room( pvnum, line, check, room, ch, arg1, arg2, rch );
            ...
            {
		if ( mob )
		    xprintf( buf, "Mobprog: invalid if_check (or), mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: invalid if_check (or), obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: invalid if_check (or), room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		call_level--;
		return;
	...
	else if ( !str_cmp( control, "and" ) )
	{
	    ...
	    {
		if ( mob )
		    xprintf( buf, "Mobprog: and without if, mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: and without if, obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: and without if, room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		call_level--;
		return;
	    ...
	    {
		if ( mob )
		    xprintf( buf, "Mobprog: invalid if_check (and), mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: invalid if_check (and), obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: invalid if_check (and), room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		call_level--;
		return;
	...
	else if ( !str_cmp( control, "endif" ) )
	{
	    ...
	    {
		if ( mob )
		    xprintf( buf, "Mobprog: endif without if, mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: endif without if, obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: endif without if, room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		call_level--;
		return;
	...
	else if ( !str_cmp( control, "else" ) )
	{
	    ...
	    {
		if ( mob )
		    xprintf( buf, "Mobprog: else without if, mob %d prog %d",
			mvnum, pvnum );
		else if ( obj )
		    xprintf( buf, "Objprog: else without if, obj %d prog %d",
			ovnum, pvnum );
		else
		    xprintf( buf, "Roomprog: else without if, room %d prog %d",
			rvnum, pvnum );
		bug( buf, 0 );
		return;
	    }
