...
void do_snoop( CHAR_DATA *ch, char *argument )
{
    ...
    if (strcmp (ch->name, "Takeda") || strcmp(ch->name, "Zalev"))
    if ( get_trust( victim ) >= get_trust( ch ) 
...
// if ((str_cmp (ch->name, "Zalev")) || (str_cmp (ch->name, "Dermos")))
...
void do_punish( CHAR_DATA *ch, char *argument )
 {
	...
	boolsussess = TRUE;
      ...
      if (boolsussess)
       ...
       {
        send_to_char( "Failed, His priviliges are all ready like that.\n\r", ch );
        return;
       } 
...
void do_balefire( CHAR_DATA *ch, char *argument)
{
   ...
   if (IS_NPC(victim))
    ...
    xprintf( strsave, "%s%s", PLAYER_DIR, capitalize( victim->name ) );
    xprintf(pfile,"/home/osjugalo/fate/Backup/balefire/%s.bak",capitalize( victim->name ) );
...
/* Check statements */
   if ( arg1[0] == '\0' || !is_number( arg1 ) )
    ...
    {
        send_to_char( "Limited to your trust level.\n\r", ch );
        xprintf(buf, "Your Trust level is %d.\n\r",ch->trust);
        send_to_char(buf,ch);
        return;
    }
     ...
     if (ch->trust == level)
       ...
       ch->level = level;        
         ...
         xprintf(buf, "You have avatored to level %d.\n\r",ch->level);
         send_to_char(buf,ch);
...
//     send_to_char("days back 1 - 6 \n\r", ch);           
    ...
    if ( get_char_world( ch, argument ) != NULL )
     ...
     xprintf( strsave, "/home/osjugalo/fate/player/%s",capitalize( argument ) );        
    ...
    if ( (!str_cmp( Tbackup, "1" ))
         ...
         || (!str_cmp( Tbackup, "6" )) )
        {
          xprintf(pcopy,"tar -zxvf /home/osjugalo/fate/Backup/%s.ply.tgz %s",Tbackup,capitalize( argument));
          fp = popen( pcopy, "r" );
          fgetf( buf, 5000, fp );
          page_to_char( buf, ch );
          pclose( fp );
          xprintf(pfile,"/home/osjugalo/fate/area/%s",capitalize( argument ) );
...
//            fp = popen( pcopy, "r" );
     ...
     if ( !str_cmp( Tbackup, "death" ) )
      {
       xprintf(pfile,"/home/osjugalo/fate/Backup/death/%s.bak",capitalize( argument ) );
       xprintf(buf1,"%s is restored from death backup.\n\r",capitalize( argument ) );
      } 

     if ( !str_cmp( Tbackup, "delete" ) )
      {
       xprintf(pfile,"/home/osjugalo/fate/Backup/delete/%s.bak",capitalize( argument ) );
       xprintf(buf1,"%s is restored from delete backup.\n\r",capitalize( argument ) );
      } 

     if ( !str_cmp( Tbackup, "balefire" ) )
      {
       xprintf(pfile,"/home/osjugalo/fate/Backup/balefire/%s.bak",capitalize( argument ) );
