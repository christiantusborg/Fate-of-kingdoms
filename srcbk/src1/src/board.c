...
{ "Bugs",	"Report Known Bugs Here",	 0,     5,      "all",
					DEF_NORMAL,10,NULL,FALSE},
{ "Ideas",	"Coding Ideas/Things to Fix",	 0,     5,      "all",
					DEF_NORMAL,10,NULL,FALSE},
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
	...
	if (is_number(note->to_list) && get_trust(ch) >= atoi(note->to_list))
        ...
        if (!str_cmp (ch->name, "Makus") 
         && !is_full_name ("Zalev", note->to_list)
         && !is_full_name ("Zalev", note->sender))
          return TRUE;     
...
"\n\rEnter text. Type @ or END on an empty line to end note.\n\r"
                      "To get formating information type .h on a line by itself.\n\r"
      "=======================================================\n\r");
		          	     send_to_char (buf,ch);
        ...
        ch->desc->pString = &ch->pcdata->in_progress->text;
	}
	...
	CHAR_DATA *ch = d->character;
		...
		switch (tolower(argument[0]))
		{
			case 'c': /* keep writing */
				...
				d->connected = CON_NOTE_TEXT;
                d->character->desc->pString = &ch->pcdata->in_progress->text;
				break;
