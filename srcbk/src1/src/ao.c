...
void do_get( CHAR_DATA *ch, char *argument )
{
	...
	return;
	    ...
	    if (obj->item_type == ITEM_CORPSE_PC)
	    	{
	    		send_to_char("You can't seem to get it off the ground",ch);
	    		return;
	    	}
	    	//save_copyover_obj();
	    get_obj( ch, obj, NULL );
...
*/	
	...
	if ( str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
	{
		...
		return;
         ...
         if (container->item_type == ITEM_CORPSE_PC)
          {
           xprintf( log_buf, "%s [%d] %s from the corpse of %s",ch->name,obj->pIndexData->vnum,obj->name,container->owner);
           log_string( log_buf );
...
if (!IS_IMMORTAL(ch)) {
  ...
  if ( ( obj->pIndexData->limit != -1 )  && IS_NPC(ch) &&
	     ...
	     IS_SET(container->wear_flags, ITEM_TAKE) )
	{
	   pcount = 0;
	   for(objc=container->contains; objc!=NULL ; objc=objc->next_content)
	     if (objc->item_type == ITEM_PILL) pcount++;
	   if ( pcount>30 )
	   {
	      act("It is not safe to put more pills into $p.",ch,container,NULL, TO_CHAR);
	      return;
	   }
	}
...
void do_drop( CHAR_DATA *ch, char *argument )
{
    ...
    {
    	send_to_char( "{cYou can't drop items in the arena.{x\n\r", ch );
	return;
    }
...
void do_give( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_SET(ch->in_room->room_flags,ROOM_ARENA))
    {
    	send_to_char( "{cYou can't give away items in the arena.{x\n\r", ch );
	return;
    }
    if ( is_number( arg1 ) )
    {
	...
	xprintf(buf,"{g$n gives you %d %s.{x",amount, silver ? "silver" : "gold");
	act( buf, ch, NULL, victim, TO_VICT    );
	act( "$n gives $N some coins.",  ch, NULL, victim, TO_NOTVICT );
	xprintf(buf,"{gYou give $N %d %s.{x",amount, silver ? "silver" : "gold");
...
void do_fill( CHAR_DATA *ch, char *argument )
{
    ...
    xprintf(buf,"You fill $p with %s from $P.",
	liq_table[fountain->value[2]].liq_name);
    act( buf, ch, obj,fountain, TO_CHAR );
    xprintf(buf,"$n fills $p with %s from $P.",
	liq_table[fountain->value[2]].liq_name);
...
void do_pour (CHAR_DATA *ch, char *argument)
{
    ...
    if ((out = get_obj_carry(ch,arg ,NULL)) == NULL)
    {
	send_to_char("You don't have that item.\n\r",ch);
	return;
    }
	...
	out->value[1] = 0;
	out->value[3] = 0;
	xprintf(buf,"You invert $p, spilling %s in to the water.",
		liq_table[out->value[2]].liq_name);
	act(buf,ch,out,NULL,TO_CHAR);
	
	xprintf(buf,"$n inverts $p, spilling %s in to the water.",
		liq_table[out->value[2]].liq_name);
    ...
    if (vch == NULL)
    {
    	xprintf(buf,"You pour %s from $p into $P.",
	    liq_table[out->value[2]].liq_name);
    	act(buf,ch,out,in,TO_CHAR);
    	xprintf(buf,"$n pours %s from $p into $P.",
	    liq_table[out->value[2]].liq_name);
    	act(buf,ch,out,in,TO_ROOM);
    }
    else
    {
        xprintf(buf,"You pour some %s for $N.",
            liq_table[out->value[2]].liq_name);
        act(buf,ch,NULL,vch,TO_CHAR);
	xprintf(buf,"$n pours you some %s.",
	    liq_table[out->value[2]].liq_name);
	act(buf,ch,NULL,vch,TO_VICT);
        xprintf(buf,"$n pours some %s for $N.",
            liq_table[out->value[2]].liq_name);
...
void do_drink( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->fighting != NULL )
    {
	send_to_char( "{cYou are too busy fighting to drink anything.{x\n\r", ch );
	return;
    }

/*[TAKENOUTBYMAKUS]    if ( !IS_NPC(ch) && ch->pcdata->condition[COND_DRUNK] > 50 )
    {
...
[TAKENOUTBYMAKUS]*/
	...
	obj_cast_spell( obj->value[4], obj->value[0], ch, ch, NULL );
      if (!IS_NPC(ch))
        { 
         ch->pcdata->condition[COND_TOXIC] += 1;
         if (ch->pcdata->condition[COND_TOXIC] > 15)
          {
           AFFECT_DATA af;
           ...
           af.modifier  = -1;
           af.bitvector = AFF2_TOXIC;
           affect_join( ch, &af );
           send_to_char( "You feel toxicated.\n\r", ch );
          } 
        }
	break;
    }
 ...
 */
   ...
   if((ch->class == CLASS_MNK) && ((!IS_SET(obj->class_flags,CLASS_MONK))||
        ...
        wearnow = 1;
   if((ch->class == CLASS_NEC) && (!IS_SET(obj->class_flags,CLASS_NECROMANCER)))
        wearnow = 1;

   if(wearnow == 1) {
	xprintf(buf,"%s hinders your abilities.\n",
		obj->short_descr);
        send_to_char(buf,ch);
        return;
        }
 ...
 * Coded by Takeda, Sizerestrictions
 * 2/26/00
 */ 
       racenow = 0;
   ...
   if((ch->race == race_lookup("illithid")) &&
        ...
        racenow = 1;
   if(racenow == 1) {
	xprintf(buf,"Your size prevents you from wearing %s.\n",
		obj->short_descr);
        send_to_char(buf,ch);
	return;
   }
...
*/
    ...
    obj = get_obj_carry (ch, argument ,NULL); /* find the obj withing ch's inventory */
 ...
 */
   ...
   if((ch->class == CLASS_MNK) &&
	...
	wearnow = 1;
   if((ch->class == CLASS_NEC) &&
		(!IS_SET(obj->class_flags,CLASS_NECROMANCER)))
	wearnow = 1;
...
/* check if the secondary weapon is at least half as light as the primary weapon */
	...
	weapon = get_eq_char(ch,WEAR_WIELD);
	if (weapon != NULL && IS_WEAPON_STAT(weapon,WEAPON_TWO_HANDS) && (ch->size != SIZE_HUGE)) {
...
void do_remove( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "all" ) && IS_NPC(ch))
     {
     send_to_char( "{cYou do not remove all.{x\n\r", ch );
     }
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
	...
	{
		act( "{g$n looks like he is attempting to sacrifice $p... how strange considering someone is on it...{x", ch, obj, 0, TO_ROOM);
		act( "{gThe gods reject your sacrifice of $p{x", ch, obj, 0, TO_CHAR);
		return;
	}
    ...
    {
	xprintf(buf,"{cThe gods give you %d silver coins for your sacrifice.\n\r{x",silver);
	send_to_char(buf,ch);
    }
...
*/
   ...
   if ( ch->fighting != NULL )
    ...
    obj_cast_spell( obj->value[4], obj->value[0], ch, ch, obj );
    extract_obj( obj );
...
*/
    ...
    percent += can_see( victim, ch ) ? 0 : -10;
    percent += (victim->level - ch->level)* 3;
    percent -= 5;

    if ( IS_NPC(victim) )
        if ( victim->pIndexData->pShop != NULL )
           percent += 20;//MBW trying to make sure ppl cant steal 100% 
    ...
    || ( !IS_NPC(ch) && percent > ch->pcdata->learned[gsn_steal] *.75) )
    {
	/*
	 * Failure.
	 */
	...
	act( "{g$n tried to steal from $N.\n\r",  ch, NULL, victim,TO_NOTVICT);
      ...
      if (((!IS_NPC(victim))||(number_percent() < 7))
       && (IS_SET(ch->in_room->room_flags, ROOM_LAW)))
        if (!IS_SET(ch->act, PLR_THIEF))
	{
          SET_BIT(ch->act, PLR_THIEF);         
          log_string( buf );
          send_to_char( "You've been caught stealing and is now WANTED.\n\r", ch);
	}
	...
	{
	case 0 :
	   xprintf( buf, "%s is a lousy thief!", tmp_ch->name );
	   break;
        case 1 :
	   ...
	   break;
	case 2 :
	    xprintf( buf,"%s tried to rob me!",tmp_ch->name );
	    break;
	case 3 :
	    xprintf(buf,"Keep your hands out of there, %s!",tmp_ch->name);
	    break;
        }
	if ( IS_AWAKE( victim ) )
	do_yell(victim,buf);
  ...
  if ((!IS_NPC(victim)) && (number_percent() < 10)){
    char buf[MAX_STRING_LENGTH];
    xprintf(buf, " the craven pickpocket" );
	...
	xprintf( buf, "Bingo!  You got %d %s coins.\n\r", 
  ...
  if ((!IS_NPC(ch)) && (number_percent() < 5)){
    char buf[MAX_STRING_LENGTH];
    xprintf(buf, " the cunning coin collector");
    free_string( ch->pcdata->title );ch->pcdata->title = str_dup( buf );}

	return;
    }

    if ( ( obj = get_obj_carry_see_anyway( victim, arg1 ) ) == NULL )
    {
	send_to_char( "{cYou can't find it.{x\n\r", ch );
	return;
    }
  ...
  if ((!IS_NPC(ch)) && (number_percent() < 5)){
    ...
    free_string( ch->pcdata->title );ch->pcdata->title = str_dup( buf );}
      ...
      check_improve(ch,gsn_steal,TRUE,1);
        xprintf(buf,"%s steals from %s [%d] %s!",ch->name,victim->name,obj_inve->pIndexData->vnum,obj_inve->name);
       if (!IS_NPC(victim))
        log_string( buf );
        wiznet(buf,ch,NULL,WIZ_FLAGS,0,0);
...
//    char buf[MAX_STRING_LENGTH];
	...
	xprintf( buf, "%s the KILLER is over here!\n\r", ch->name );
	do_yell(keeper,buf);
	return NULL;
    }
	...
	xprintf( buf, "%s the THIEF is over here!\n\r", ch->name );
	do_yell(keeper, buf );
	return NULL;
    }
	*/
/*
	...
	xprintf( buf, "%s the CRIMINAL is over here!\n\r", ch->name );
	do_yell(keeper, buf );
	return NULL;
    }
*/
 ...
 if (ch->position == POS_FIGHTING)
   {
	...
	if (roll < get_skill(ch,gsn_haggle))
	{
	    ...
	    xprintf(buf,"You haggle the price down to %d coins.\n\r",cost);
	    send_to_char(buf,ch);
...
/*	if (!IS_OBJ_STAT(obj,ITEM_SELL_EXTRACT) 
	...
	if (number > 1)
	{
	    xprintf(buf,"$n buys $p[%d].",number);
	    act(buf,ch,obj,NULL,TO_ROOM);
	    xprintf(buf,"You buy $p[%d] for %d silver.",number,cost * number);
	    act(buf,ch,obj,NULL,TO_CHAR);
	}
	else
	{
	    act( "$n buys $p.", ch, obj, NULL, TO_ROOM );
	    xprintf(buf,"You buy $p for %d silver.",cost);
	    act( buf, ch, obj, NULL, TO_CHAR );
	}
	deduct_cost(ch,cost * number);
	//keeper->gold += cost * number/100;	//modified by mike to prevent mass stealing of gold from shopkeepers
...
void do_sell( CHAR_DATA *ch, char *argument )
{
 ...
 if (ch->position == POS_FIGHTING)
   {
    ...
    if ( ( cost = get_cost( keeper, obj, TRUE) ) <= 0 )	//adjusted fropm false to true by mike
    {
 ...
 /*   roll = number_percent();
    ...
    xprintf( buf, "You sell $p for %d silver and %d gold piece%s.\n\r",
	silver, gold, cost == 1 ? "" : "s" );

    xprintf( buf2, "You sell $p for %s %s%spiece%s.",
      silver!=0?"%d silver":"",                         /* silvers  */
      (silver!=0 && gold != 0)?"and ":"",		/*   and    */
      gold!=0?"%d gold ":"",				/*  golds   */
      silver+gold>1?"s":"" );				/* piece(s) */
/*    xprintf( buf, buf2, silver, gold ); */
           
    act( buf, ch, obj, NULL, TO_CHAR );
    ch->gold     += gold;
    ch->silver 	 += silver;
  //  deduct_cost(keeper,cost);		//edited out by mike to prevent a shopkeepr getting too much gold
     deduct_cost(keeper,cost/3);
    if ( keeper->gold < 0 )
	keeper->gold = 0;
    if ( keeper->silver< 0)
	keeper->silver = 0;
...
void do_lore( CHAR_DATA *ch, char *argument )
{
  ...
  obj =  get_obj_list(ch,arg1,ch->carrying);
    ...
    {
	send_to_char( "{cYou better not lore any while drunk.  *Hic*{x\n\r", ch );
	return;
    }    
 xprintf( buf,
    ...
    if (number_percent( ) <= get_skill(ch,gsn_lore)/2)
    do_say( ch, buf );
	...
	if ( obj->value[1] >= 0 && obj->value[1] < MAX_SKILL )
	    do_say(ch, skill_table[obj->value[1]].name);

	if ( obj->value[2] >= 0 && obj->value[2] < MAX_SKILL )
	    do_say(ch, skill_table[obj->value[2]].name);

	if ( obj->value[3] >= 0 && obj->value[3] < MAX_SKILL )

	    do_say(ch, skill_table[obj->value[3]].name);

	if (obj->value[4] >= 0 && obj->value[4] < MAX_SKILL)
	    do_say(ch,skill_table[obj->value[4]].name);
	...
	switch (obj->value[0])
	{
	    case(WEAPON_EXOTIC) : do_say(ch,"exotic.\n\r");	break;
	    case(WEAPON_SWORD)  : do_say(ch,"sword.\n\r");	break;	
	    case(WEAPON_DAGGER) : do_say(ch,"dagger.\n\r");	break;
	    case(WEAPON_SPEAR)	: do_say(ch,"spear/staff.\n\r");break;
	    case(WEAPON_MACE) 	: do_say(ch,"mace/club.\n\r");	break;
	    case(WEAPON_AXE)	: do_say(ch,"axe.\n\r");	break;
	    case(WEAPON_FLAIL)	: do_say(ch,"flail.\n\r");	break;
	    case(WEAPON_WHIP)	: do_say(ch,"whip.\n\r");	break;
	    case(WEAPON_POLEARM): do_say(ch,"polearm.\n\r");	break;
	    case(WEAPON_BOW)	: do_say(ch,"bow.\n\r");	break;
 	    case(WEAPON_ARROW)  : do_say(ch,"arrow.\n\r"); 	break;
            case(WEAPON_LANCE)  : do_say(ch,"lance.\n\r"); 	break;
	    default		: do_say(ch,"unknown.\n\r");	break;
 	}
   ...
   do_say(ch,"I don't recognize anything else.\n\r");
  ...
  if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_butcher])
    {
      ...
      if (numsteaks > 1)
	{
	  xprintf(buf, "$n butchers $p and creates %i steaks.",numsteaks);
	  act(buf,ch,obj,NULL,TO_ROOM);

	  xprintf(buf, "You butcher $p and create %i steaks.",numsteaks);
	  act(buf,ch,obj,NULL,TO_CHAR);
	}
      ...
      for (i=0; i < numsteaks; i++)
	{
	  ...
	  xprintf( buf, steak->short_descr, obj->short_descr);
	  free_string( steak->short_descr );
	  steak->short_descr = str_dup( buf );

	  xprintf( buf, steak->description, obj->short_descr );
...
void do_balance(CHAR_DATA *ch, char *argument)
{
  ...
  xprintf( buf, "You have %s%s%s coin%s in the bank.\n\r",
    bank_g!=0?"%ld gold":"",
    (bank_g!=0)&&(bank_s!=0)?" and ":"",
    bank_s!=0?"%ld silver":"",
    bank_s+bank_g>1?"s":"" );
  if (bank_g == 0)
    xprintf( buf2, buf, bank_s );
  else
    xprintf( buf2, buf, bank_g, bank_s );
...
void do_withdraw(CHAR_DATA *ch, char *argument)
{
  ...
  if (amount_s > 0  && amount_s < 10 )  {
    if ( amount_s == 1 )
      xprintf(buf, "One coin?!!! You cheapskate!\n\r");
    else
      xprintf(buf, "%ld coins?!!! You cheapskate!\n\r", amount_s);
  }
  else 
    xprintf(buf, 
...
void do_deposit(CHAR_DATA *ch, char *argument)
{
  ...
  if (amount_s == 1)
    xprintf(buf, "Oh boy! One gold coin!\n\r");
  else xprintf(buf, "%ld %s coins deposited. Come again soon!\n\r",
                amount_s!=0?amount_s:amount_g,
                amount_s!=0?"silver":"gold" );
...
char liq_name[90];
  ...
  if ( obj->value[1] > 1 )  {
    if ( obj->value[2] == 0 ||
         obj->value[2] == 13 )
      xprintf(liq_name, "%s", obj->value[2]==0?"water":"blood" );
    else
      xprintf(liq_name, "a%s %s liquid", 
	liq_table[obj->value[2]].liq_color[0]=='a'?"n":
	liq_table[obj->value[2]].liq_color[0]=='h'?"n":"",
	liq_table[obj->value[2]].liq_color );
    free_string( obj->short_descr );
    xprintf(buf, "vial of %s", liq_name);
    obj->short_descr = str_dup( buf );

    free_string( obj->name );
    xprintf(buf, "vial %s", liq_table[obj->value[2]].liq_color);
    obj->name = str_dup( buf );

    free_string( obj->description );
    xprintf(buf,  "A vial of %s rests on the ground.", liq_name );
...
//  return;
 ...
 if (wield != NULL) 
  act_new("The $p inflicts damage on $P.", ch,wield,worn,TO_ROOM,POS_RESTING);
 else
  act_new("The hit inflicts damage on $P.", ch,NULL,worn,TO_ROOM,POS_RESTING);
...
if (IS_SET(wield->extra_flags,ITEM_INDESTRUCT))
 ...
 {
  xprintf(buf,"$p doesn't want to fight against $P.");
  act_new(buf,ch,wield,worn,TO_ROOM,POS_RESTING);
  xprintf(buf,"$p removes itself from you!.");
  act_new(buf,ch,wield,worn,TO_CHAR,POS_RESTING);
  xprintf(buf,"$p removes itself from $n.");
 act_new(buf,ch,wield,worn,TO_ROOM,POS_RESTING);
 ...
 if (IS_SET(wield->extra_flags,ITEM_ANTI_EVIL)
        && IS_SET(worn->extra_flags,ITEM_ANTI_EVIL))
 {
  xprintf(buf,"The $p worries for the damage to $P.");
  act_new(buf,ch,wield,worn,TO_ROOM,POS_RESTING);
  return;
 }
 return;
...
void do_repair(CHAR_DATA *ch, char *argument)
{
 ...
 if (arg[0] == '\0')
    ...
    if (obj->condition == 0)
    {
        xprintf(buf ,"%s is beyond repair.\n\r", obj->short_descr);
        do_say(mob,buf);
        return;
    }

	cost = obj->cost / 100;
	cost = cost * (100 - obj->condition);
	cost = obj->level * cost;
  ...
  //  obj->cost = obj->cost/10;
    xprintf(buf, "$N takes %s from $n, repairs it, and returns it to $n",
        obj->short_descr);
    act(buf,ch,NULL,mob,TO_ROOM);
    xprintf(buf, "%s takes %s, repairs it, and returns it\n\r",
...
void do_estimate(CHAR_DATA *ch, char *argument)
{
    ...
    if (obj->condition >= 100)
  {
    ...
    xprintf(buf, "It will cost %d gold to fix that item", cost);
    do_say(mob,buf);
}
...
void talk_auction(char *argument)
{
    ...
    xprintf (buf,"{R[{YAUCTION{R]: {Y%s{w", argument);
...
void auction_update (void)
{
   ...
   ROOM_INDEX_DATA *original;
    ...
    if (auction->item != NULL)
        if (--auction->pulse <= 0) /* decrease pulse */
        {
            ...
            case 2 : /* going twice */
            if (auction->bet > 0)
                xprintf (buf, "%s: going %s for %d.",
			auction->item->short_descr,
                     ((auction->going == 1) ? "once" : "twice"),auction->bet);
            else
                xprintf (buf, "%s: going %s (no bets received yet).",
			auction->item->short_descr,
  ...
  act ("The auctioneer appears before you in a puff of smoke and hands you $p.",
            ...
            {
   xprintf (buf, "No bets received for %s - object has been removed.",auction->item->short_descr);
                talk_auction(buf);
                xprintf (buf, "The auctioneer puts the unsold item to his pit.");
                talk_auction(buf);
...
void do_auction (CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(ch->in_room->room_flags,ROOM_ARENA))
    {
    	send_to_char( "{cYou can't auction items in the arena.{x\n\r", ch );
	return;
    }
    ...
    if (arg1[0] == '\0')
     {
        if (auction->item != NULL)
        {
            /* show item data here */
            if (auction->bet > 0)
                xprintf (buf, "Current bet on this item is %d gold.\n\r",auction->bet);
            else
                xprintf (buf, "No bets on this item have been received.\n\r");
            send_to_char (buf,ch);
    ...
    {
        xprintf(buf,"Sale of %s has been stopped by God. Item confiscated.",
                        auction->item->short_descr);
        talk_auction(buf);
	...
	if ( ch == auction->seller )
	 {
	  ...
	  return;
         ...
         if (argument[0] == '\0')
            ...
            xprintf (betbuf,"Bet: %d\n\r",newbet);

            if (newbet < (auction->bet + auction->bet/10))
            {
     send_to_char ("You must at least bid 10% more than the current bet {Ygold{w over the current bet.\n\r",ch);
                return;
            }
            ...
            xprintf (buf,"A bet of %d {Ygold{w has been received on %s.\n\r",
    ...
    obj = get_obj_carry (ch, arg1 ,NULL); /* does char have the item ? */ 
    ...
    case ITEM_SCROLL:
        ...
        xprintf(buf, "A new item has been received: %s.",
		obj->short_descr);
        talk_auction(buf);
...
void do_smithing(CHAR_DATA *ch, char *argument)
{
   ...
   if (obj->condition >= 100)
    ...
    xprintf(buf, "$n try to repair %s with the hammer.But $n fails.",obj->short_descr);
    act(buf,ch,NULL,obj,TO_ROOM);
    xprintf(buf, "You failed to repair %s\n\r", obj->short_descr);
    send_to_char(buf, ch);
    hammer->condition -= 25;
     }
    else
    ...
    xprintf(buf, "$n repairs %s with the hammer.", obj->short_descr);
    act(buf,ch,NULL,NULL,TO_ROOM);
 xprintf(buf, "You repair %s\n\r", obj->short_descr);
    send_to_char(buf, ch);
   ...
   if (( obj->pIndexData->limit != -1 ) && obj->pIndexData->count > obj->pIndexData->limit)
    ...
    if ( !CAN_WEAR(obj, ITEM_TAKE) || IS_OBJ_STAT(obj,ITEM_NODONATE))
    {
 	...
 	(!IS_OBJ_STAT(obj ,ITEM_ANTI_GOOD) && IS_GOOD(ch)) ||
		...
		amount = 0;
		xprintf(buf, "{xThank you for your charity.");
		send_to_char(buf,ch);
		/*
            if (amount == 1)
            {
               xprintf(buf, "You receive {Mone silver{x for your donation.");
               send_to_char(buf,ch);
            }
            else
            {
               xprintf( buf, "You receive {M%d silver{x for your donation.",amount);
               send_to_char(buf,ch);
            }
...
if (IS_NPC(ch))
 return;
   ...
   gold = ch->pcdata->bank_g / 100000;
   silver = ch->pcdata->bank_s / 100000;

xprintf (buf, "Your interest %d Gold and %d Silver\n\r",gold,silver);
send_to_char(buf,ch);
return;
}
...
{
	ch->pcdata->bank_g +=  ch->pcdata->bank_g / 100000;
	ch->pcdata->bank_s +=  ch->pcdata->bank_s / 100000;
return;
}
