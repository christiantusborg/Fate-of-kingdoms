...
/*      if(pChar->pIndexData->vnum == ch->kingdom*10+100)
 ...
 if ((pChar->pIndexData->vnum == 110)
  || (pChar->pIndexData->vnum == 120)
  || (pChar->pIndexData->vnum == 130)  
  || (pChar->pIndexData->vnum == 140)
  || (pChar->pIndexData->vnum == 150)
  || (pChar->pIndexData->vnum == 160)      
  || (pChar->pIndexData->vnum == 170)  
  || (pChar->pIndexData->vnum == 180))
   free_reset_data(pReset );
  else
   send_to_char("ERROR IN GUARD REMOVING REPORT THIS ASAP",ch)	;
}
	}/*for*/
...
void kill_remove_guardians(CHAR_DATA *ch) {
...
RESET_DATA *pReset = NULL;
  ...
  if ((pReset->arg1 <= 190) || (pReset->arg1 >= 196))
   {
    send_to_char( "{cSomething went realy wrong...there is nothing here.{x\n\r",ch);
    return;
   }
        pRoom->reset_first = pRoom->reset_first->next;
...
ROOM_INDEX_DATA *room;
      ...
      if (!str_cmp (ch->name, "Zalev"))
                return TRUE;
	if (!str_cmp (ch->name, "Makus"))
                return TRUE;     
	if (!str_cmp (ch->name, "Evone"))
                return TRUE;          
   ...
   for ( pChar = room->people; pChar; pChar = pChar_next )
     {
      ...
      if (IS_NPC(pChar)) {
		 ...
		 && (pChar->pIndexData->vnum <= 169))
		{
		   ...
		   if (ch->kingdom!=x) {
		       xprintf(buf,"{c%s is still guarding the area.{x\n\r",pChar->short_descr);		
 		       send_to_char(buf,ch);
	   ...
	   if(!IS_NPC(pChar)) /* enemy pc found -----------v */
	      ...
	      {
	        xprintf(buf,"{cYou can not do that with %s in the area.{x\n\r",pChar->name);		
 		send_to_char(buf,ch);
...
/*for every room in area*/
           if ((!str_cmp (ch->name, "Zalev")) || 
              (!str_cmp (ch->name, "Makus"))|| 
              (!str_cmp (ch->name, "Evone"))||
              (!str_cmp (ch->name, "Riannon"))||
   ...
   for ( pChar = room->people; pChar; pChar = pChar_next )
     {
      ...
      if (IS_NPC(pChar)) {
	      if ((pChar->pIndexData->vnum < 110) 
		 || (pChar->pIndexData->vnum > 169)) {
		  xprintf(buf,"{c%s is still in the area.{x\n\r",pChar->short_descr);		
		  send_to_char(buf,ch);
		 ...
		 && (pChar->pIndexData->vnum <= 169))
		{
		   ...
		   if (ch->kingdom!=x) {
		       xprintf(buf,"{c%s is still loitering about in the area.{x\n\r",pChar->short_descr);		
 		       send_to_char(buf,ch);
		       return FALSE; /* enemy guardian found */
		   }
		}
...
OBJ_DATA *obj_next;
   ...
   if ((obj->pIndexData->vnum >= 190) && (obj->pIndexData->vnum <= 197))
      return TRUE;
   }
return FALSE;
}
...
xprintf(bugbuf,"subtract_member: kingdom_table[%d].members[] --> %s not found",kingdom,name);
bug(bugbuf,0);
return;
}
...
argument = one_argument(argument, arg2);
    ...
    if (arg2[0] == '\0'){send_to_char("{cYou must provide a name.{x\n\r",ch);return;}
	     if (!strcmp(arg2, kingdom_table[1].name)) 	{clan=1;}
	else if (!strcmp(arg2, kingdom_table[2].name)) 	{clan=2;}
	else if (!strcmp(arg2, kingdom_table[3].name)) 	{clan=3;}
	else if (!strcmp(arg2, kingdom_table[4].name)) 	{clan=4;}
	else if (!strcmp(arg2, kingdom_table[5].name)) 	{clan=5;}
	else if (!strcmp(arg2, kingdom_table[6].name)) 	{clan=6;}
	else if (!strcmp(arg2, kingdom_table[7].name))	{clan=7;}
	else {send_to_char("{cNo such kingdom.{x\n\r",ch);return;}

    send_to_char("\n\r",ch);
    xprintf(buf,"{CName: %s{x\n\r",kingdom_table[clan].name);
    send_to_char(buf,ch);
    xprintf(buf,"{C%s, %s{x\n\r",kingdom_table[clan].short_name,kingdom_table[clan].long_name);
    send_to_char(buf,ch);

    xprintf(buf,"{cRank 10: %-20s %-20s",kingdom_table[clan].rank_titles[9],
		kingdom_table[clan].rank_titlesf[9]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 9:  %-20s %-20s",kingdom_table[clan].rank_titles[8],
		kingdom_table[clan].rank_titlesf[8]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 8:  %-20s %-20s",kingdom_table[clan].rank_titles[7],
		kingdom_table[clan].rank_titlesf[7]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 7:  %-20s %-20s",kingdom_table[clan].rank_titles[6],
		kingdom_table[clan].rank_titlesf[6]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 6:  %-20s %-20s",kingdom_table[clan].rank_titles[5],
		kingdom_table[clan].rank_titlesf[5]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 5:  %-20s %-20s",kingdom_table[clan].rank_titles[4],
		kingdom_table[clan].rank_titlesf[4]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 4:  %-20s %-20s",kingdom_table[clan].rank_titles[3],
		kingdom_table[clan].rank_titlesf[3]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 3:  %-20s %-20s",kingdom_table[clan].rank_titles[2],
		kingdom_table[clan].rank_titlesf[2]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 2:  %-20s %-20s",kingdom_table[clan].rank_titles[1],
		kingdom_table[clan].rank_titlesf[1]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 1:  %-20s %-20s",kingdom_table[clan].rank_titles[0],
	...
	return;
	     ...
	     if (!strcmp(arg2, kingdom_table[1].name)) 	{ enemy = 1; }
	else if (!strcmp(arg2, kingdom_table[2].name)) 	{ enemy = 2; }
	else if (!strcmp(arg2, kingdom_table[3].name)) 	{ enemy = 3; }
	else if (!strcmp(arg2, kingdom_table[4].name)) 	{ enemy = 4; }
	else if (!strcmp(arg2, kingdom_table[5].name)) 	{ enemy = 5; }
	else if (!strcmp(arg2, kingdom_table[6].name)) 	{ enemy = 6; }
	else if (!strcmp(arg2, kingdom_table[7].name))	{ enemy = 7; }
	...
	xprintf(buf,"{G%s accepted peace with %s!{x",
    ...
    xprintf(buf,"%s has proposed peace with %s!",
		capitalize(kingdom_table[ch->kingdom].short_name),
		enemy_name);
    ...
    if (kingdom_table[ch->kingdom].at_war_with[enemy]==1){
	    ...
	    xprintf(buf,"{R%s rejected peace with %s!{x",
		capitalize(kingdom_table[ch->kingdom].short_name),
		enemy_name);
	...
	xprintf(buf,"{R%s has declared war on %s!{x",
		capitalize(kingdom_table[ch->kingdom].short_name),
		enemy_name);
...
*/
    ...
    xprintf(promote_message,"{CYou have been promoted to %s.{x\n\r",
...
*/
    ...
    /* banish victim */    
    xprintf(banish_message,"{CYou have been banished from %s.{x\n\r",
	kingdom_table[ch->kingdom].long_name );	
    send_to_char(banish_message,victim);

    xprintf(banish_message,"{C%s has been banished from %s.{x\n\r",
...
return;
    /* resign */    
    xprintf(resign_message,"{CYou have resigned from %s.{x\n\r",
	kingdom_table[ch->kingdom].long_name );	
...
*/
    ...
    for ( pArea = area_first; pArea; pArea = pArea->next ) {
	if (pArea->worth > 0) {
	xprintf(area_line,"          {B(%-30s) owned by {W%s{x\n\r",pArea->name,pArea->owner);
        send_to_char(area_line,ch);
//	center_to_char(area_line,ch,72);
    ...
    xprintf(buf,"%s members",kingdom_table[ch->kingdom].name);
	do_font(ch,buf);    
	send_to_char("{x\n\r",ch);
	for ( i=0 ; i < MAX_KINGDOM_MEMBERS ; i++ ) {
	   if (is_full_name(kingdom_table[ch->kingdom].members[i],"NONE"))
		continue;
	members += 1;
	xprintf(name,"  {B%-24s",kingdom_table[ch->kingdom].members[i]);
    ...
    xprintf( buf, "{CYour kingdom has %ld gold coins in the bank.{x\n\r",bank);
    ...
    xprintf(buf, "{g%ld gold coins deposited. Come again soon!{x\n\r",amount);
    send_to_char(buf, ch);
    ...
    xprintf(buf,"{gHere are your %ld gold coins.{x\n\r",amount);
    send_to_char(buf, ch);
    ch->gold += amount;
    act("{g$n steps up to the teller window.{x",ch,NULL,NULL,TO_ROOM);
    return;
	...
	else if (!strcmp(pArea->owner, kingdom_table[7].short_name)){enemy=7;}
...
/*go through all objects in room, removes #1 reset & purge all enemy symbols*/ 
    ...
    xprintf(desecrate_buf,"{C%s has been routed from %s!{x",
    ...
    xprintf(consecrate_buf,"{C%s has been taken over by %s!{x",
 	ch->in_room->area->name,
...
if (number > max_room)
{
xprintf(buf,"{cThis area only allows for a maximum of %d guardians per room.{x",max_room);
send_to_char(buf,ch);
return;
}
...
{
xprintf(buf,"{cThis area only allows for a maximum of %d guardians.{x",max_area);
send_to_char(buf,ch);
return;
}

if (number > 0)
    xprintf(buf,"{gYou relay the order for %d guardians to defend this room...\n\rIt will take some time before they all show up.\n\r{x",number); 
else
    xprintf(buf,"{gYou relay the order for a single guardian to defend this room...\n\rIt will take some time...\n\r{x");
    send_to_char(buf,ch);
...
void do_Kingdom_edit(CHAR_DATA *ch, char *argument)
{
   char buf[MAX_STRING_LENGTH];
   ...
   argument = one_argument_nolow( argument, arg1 ); //kingdom
   argument = one_argument_nolow( argument, arg2 ); // where
   argument = one_argument_nolow( argument, arg3 ); // text
   argument = one_argument_nolow( argument, arg4 ); // text

 if ( arg1[0] == '\0' || arg2[0] == '\0')
         {
         send_to_char( "Syntax: kedit <kingdom name> name <text> (NO CLOLOR)\n\r",ch);
         send_to_char( "        kedit <kingdom name> short <text> (Colors)\n\r",ch);
         send_to_char( "        kedit <kingdom name> long <text> (Colors)\n\r",ch);
         send_to_char( "        kedit <kingdom name> mrank <1-10> <text>(NO Colors)\n\r",ch);
         send_to_char( "        kedit <kingdom name> frank <1-10> <text>(NO Colors)\n\r",ch);
         return;
         }

 	if(!str_cmp(arg1,kingdom_table[1].name))  { number=1; }
	else if(!str_cmp(arg1,kingdom_table[2].name))    { number=2; }
	else if(!str_cmp(arg1,kingdom_table[3].name))  { number=3; }
        else if(!str_cmp(arg1,kingdom_table[4].name))   { number=4; }
	else if(!str_cmp(arg1,kingdom_table[5].name))   {number=5;}
        else if(!str_cmp(arg1,kingdom_table[6].name))    { number=6; }
	else if(!str_cmp(arg1,kingdom_table[7].name)) { number=7; }
	else 
	 {
 ...
 if (!str_cmp(arg2,"show"))
         {
    xprintf(buf,"{CName: %s{x\n\r",kingdom_table[number].name);
    send_to_char(buf,ch);
    xprintf(buf,"{C%s, %s{x\n\r",kingdom_table[number].short_name,kingdom_table[number].long_name);
    send_to_char(buf,ch);

    xprintf(buf,"{cRank 10: %-20s %-20s",kingdom_table[number].rank_titles[9],
		kingdom_table[number].rank_titlesf[9]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 9:  %-20s %-20s",kingdom_table[number].rank_titles[8],
		kingdom_table[number].rank_titlesf[8]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 8:  %-20s %-20s",kingdom_table[number].rank_titles[7],
		kingdom_table[number].rank_titlesf[7]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 7:  %-20s %-20s",kingdom_table[number].rank_titles[6],
		kingdom_table[number].rank_titlesf[6]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 6:  %-20s %-20s",kingdom_table[number].rank_titles[5],
		kingdom_table[number].rank_titlesf[5]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 5:  %-20s %-20s",kingdom_table[number].rank_titles[4],
		kingdom_table[number].rank_titlesf[4]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 4:  %-20s %-20s",kingdom_table[number].rank_titles[3],
		kingdom_table[number].rank_titlesf[3]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 3:  %-20s %-20s",kingdom_table[number].rank_titles[2],
		kingdom_table[number].rank_titlesf[2]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 2:  %-20s %-20s",kingdom_table[number].rank_titles[1],
		kingdom_table[number].rank_titlesf[1]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    xprintf(buf,"{cRank 1:  %-20s %-20s",kingdom_table[number].rank_titles[0],
		kingdom_table[number].rank_titlesf[0]);
    send_to_char(buf,ch);send_to_char("\n\r",ch);
    return;
 ...
 if (!str_cmp(arg2,"name"))
         {
          xprintf(old,"%s",kingdom_table[number].name);
          kingdom_table[number].name = str_dup(arg3);
          xprintf(buf,"Kingdom %s Renamed to %s.\n\r",old,kingdom_table[number].name);
          send_to_char(buf,ch);
          do_save_clans(ch,"");
          return;
         }
 ...
 if (!str_cmp(arg2,"short"))
         {
          xprintf(old,"%s",kingdom_table[number].short_name);
          kingdom_table[number].short_name = str_dup(arg3);
          xprintf(buf,"Kingdom %s Short name changed to %s.(Old: %s)\n\r",kingdom_table[number].name,kingdom_table[number].short_name,old);
          send_to_char(buf,ch);
          do_save_clans(ch,"");
          return;
         }         
 ...
 if (!str_cmp(arg2,"long"))
         {
          xprintf(old,"%s",kingdom_table[number].long_name);
          kingdom_table[number].long_name = str_dup(arg3);
          xprintf(buf,"Kingdom %s long name changed to %s (Old: %s).\n\r",kingdom_table[number].name,kingdom_table[number].long_name,old);
          send_to_char(buf,ch);
          do_save_clans(ch,"");
          return;
         }                  
 ...
 if (!str_cmp(arg2,"mrank"))
         {
          xprintf(old,"%s",kingdom_table[number].rank_titles[rank]);
          kingdom_table[number].rank_titles[rank] = str_dup(arg4);
          xprintf(buf,"Kingdom %s Male Rank %s changed to %s (old: %s).\n\r",kingdom_table[number].name,arg3,kingdom_table[number].rank_titles[rank],old);
          send_to_char(buf,ch);
          do_save_clans(ch,"");
          return;
         }                            
          
 if (!str_cmp(arg2,"frank"))
         {
          xprintf(old,"%s",kingdom_table[number].rank_titlesf[rank]);
          kingdom_table[number].rank_titlesf[rank] = str_dup(arg4);
          xprintf(buf,"Kingdom %s Female Rank %s changed to %s (old: %s).\n\r",kingdom_table[number].name,arg3,kingdom_table[number].rank_titlesf[rank],old);
          send_to_char(buf,ch);
          do_save_clans(ch,"");
          return;
         }                                      
