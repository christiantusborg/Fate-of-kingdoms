...
case SECT_ROAD:		send_to_char("{c+",ch);		break;
...
case SECT_SWAMP:	send_to_char("{m&",ch);		break;
case SECT_JUNGLE:	send_to_char("{g&",ch);		break;
case SECT_RUINS:	send_to_char("{M#",ch);		break;
case SECT_CAVERN:	send_to_char("{y*",ch);		break;
...
case SECT_CAVERN:	send_to_char("*",ch);		break;
...
case SECT_CAVERN:	fprintf(fp,"*");		break;
...
map[min][min]=SECT_MAX;map[max-1][max-1]=SECT_MAX; /* rounds edges */
...
if (x==min) {
	xprintf(buf,"{x   {C%s{x",ch->in_room->name);
	send_to_char(buf,ch);
...
/* no brief in wilderness, ascii map is automatic, autoexits off too
        ...
        send_to_char( " {b",ch);do_exits( ch, "auto" );send_to_char( "{x",ch);
           	...
           	xprintf(buf," {c[Room %d]{x",ch->in_room->vnum);
           	send_to_char(buf,ch);
       		}
}
else
...
case SECT_CAVERN:	sector_name="inside a cavern";break;
...
");
if (exit[0]==SECT_ROAD) {
    xprintf(buf,"The road continues north and ");
    strcat(desc,buf);strcpy(buf,"  "); 
	if (exit[1]==SECT_ROAD) {
	    xprintf(buf,"east.");
...
if (exit[1]==SECT_ROAD) {
    xprintf(buf,"The road continues east and ");
    strcat(desc,buf);strcpy(buf,"  "); 
	if (exit[0]==SECT_ROAD) {
	    xprintf(buf,"north.");
	    strcat(desc,buf);strcpy(buf,"  ");break; 	
...
if (exit[2]==SECT_ROAD) {
    xprintf(buf,"The road continues south and ");
    strcat(desc,buf);strcpy(buf,"  "); 
	if (exit[0]==SECT_ROAD) {
	    xprintf(buf,"north.");
	    strcat(desc,buf);strcpy(buf,"  ");break; 	
...
if (exit[3]==SECT_ROAD) {
    xprintf(buf,"The road continues west and ");
    strcat(desc,buf);strcpy(buf,"  "); 
	if (exit[1]==SECT_ROAD) {
	    xprintf(buf,"east.");
	    strcat(desc,buf);strcpy(buf,"  ");break; 	
...
strcpy(desc,"You are in a dank swamps edge. The smell and humidity make this a repelling place to say the least.");
  if (exitsum==4) { 
...
strcpy(desc,"You are at the edge of a jungle. An assortment of rare and unique plant life grow in abundance here, making it nearly impassable.");
...
break;
case SECT_CAVERN:	
strcpy(name,"A cavern");
strcpy(desc,"You have stumbled upon a big cavern.  All around you are rocks.");
  if (exitsum==4) { 
    strcpy(name,"A big cavern");
    strcpy(desc,"You have seen to be in some sort of cavern.");
  }
  if
(((exit[0]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[0]==SECT_ROAD)){
    xprintf(buf,"You see %s to the north.",get_sector_name(exit[0]));
...
/* this sets the whole wilderness default descs and names*/
