...
/*  adds arg2, was just auto-0 */
	    fprintf( fp, "G 0 %d %d %s is given to %s\n",
            ...
            if ( !pLastMob )
            {
                xprintf( buf, "Save_resets: !NO_MOB! in [%s]", pArea->file_name );
                bug( buf, 0 );
            }
            break;
/* normal E code */
...
/*    {
	...
	for( pArea = area_first; pArea; pArea = pArea->next )
	{
	    ...
	    {
		save_area( pArea );
		xprintf( buf, "%24s - '%s'", pArea->name, pArea->file_name);
		if ( ch )
		{
			send_to_char( buf, ch );
			send_to_char( "\n\r", ch );
  ...
  //             bug("-a",0);
		save_helps();
		bug("Helps saved",0);
	//	send_to_char("{RHelps saved are disabled be Kazmir it crash the mud.{x\n\r", ch);
//		send_to_char("{RHelps If you like update help talk to kazmir.{x\n\r", ch);
//		bug("+a",0);
		return;
	}
