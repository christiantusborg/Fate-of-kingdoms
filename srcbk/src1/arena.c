...
void arena_credit(CHAR_DATA *ch, char *argument)
{
	...
	send_to_char(" - Bug reporting fok-bug@kingdom.dk                 -\n\r", ch);
...
void arena_begin(CHAR_DATA *ch, char *argument)
{
	...
	if (ArenaNextStart > 0)
	  {
		xprintf(buf,"{WArena janitor tells you 'Give me %d minutes to clean up.'{x\n\r",ArenaNextStart);
    		send_to_char(buf,ch);
    		return;
	  }
	...
	switch ( intArenaType  )
	  {
		case ARENA_FULL:
			...
			xprintf(strArenaStartText,"{RFull Circle Arena Started by {c%s at %s. (%d to %d){x\n\r",ch->name,arena_zone_table[ArenaZone].longname,ArenaLowLevel,ArenaHighLevel);
		    break;
		case ARENA_KINGDOM:
			//Set Arena tpye and arena send text
			ArenaType = ARENA_KINGDOM;
			xprintf(strArenaStartText,"{RBattle of Kingdoms Started by {c%s at %s. (%d to %d){x\n\r",ch->name,arena_zone_table[ArenaZone].longname,ArenaLowLevel,ArenaHighLevel);
		    break;
		case ARENA_RACE:
			//Set Arena tpye and arena send text
			ArenaType = ARENA_RACE;
			xprintf(strArenaStartText,"{RBattle of the Races Arena Started by {c%s at %s. (%d to %d){x\n\r",ch->name,arena_zone_table[ArenaZone].longname,ArenaLowLevel,ArenaHighLevel);
		    break;
		case ARENA_CLASS:
			//Set Arena tpye and arena send text
			ArenaType = ARENA_CLASS;
			xprintf(strArenaStartText,"{RBattle of the Classes Arena Started by {c%s at %s. (%d to %d){x\n\r",ch->name,arena_zone_table[ArenaZone].longname,ArenaLowLevel,ArenaHighLevel);
		    break;
		case ARENA_ALIGNMENT:
			//Set Arena tpye and arena send text
			ArenaType = ARENA_ALIGNMENT;
			xprintf(strArenaStartText,"{RBattle of Good and Evil Arena Started by {c%s at %s. (%d to %d){x\n\r",ch->name,arena_zone_table[ArenaZone].longname,ArenaLowLevel,ArenaHighLevel);
		    break;
		case ARENA_TEAM:
			//Set Arena tpye and arena send text
			ArenaType = ARENA_TEAM;
			xprintf(strArenaStartText,"{RFull Team Arena Started by {c%s at %s. (%d to %d){x\n\r",ch->name,arena_zone_table[ArenaZone].longname,ArenaLowLevel,ArenaHighLevel);
		    break;		    		    		    
	        default:
...
void arena_update(void)
{
	...
	if (!arenacanrun())
	  {
		xprintf(strArenaSendText,"{RThe arena announcer has cancelled today's fight due to lack of participants.{x\n\r");
		Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
		arena_reset();
	  }
//	else
...
void arena_start(void)
{
		...
		if (IS_SET(vch->act, PLR_ARENA))
		  {
			...
			indexnum = number_range(ArenaLowVnum,ArenaMaxVnum);
			random = get_room_index(indexnum);
  ...
  xprintf(strArenaSendText,"{ROn the signal of the arena announcer, the mayhem commences.{x\n\r");
  Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
return;
}
	...
	switch ( ArenaType  )
	  {
		case ARENA_KINGDOM:
			if (!blnKingdom)
			  {
		  	  	xprintf(strArenaSendText,"{RThe arena announcer calls the victory of %s in the battle of Kingdoms! {x\n\r",kingdom_table[LastKingdom].name);
Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
			  	arena_win();
				return TRUE;
			  }
		...
		case ARENA_RACE:
			if (!blnRace)
			  {
//		  	  	xprintf(strArenaSendText,"{RThe triumph of the %s is official in today's historical onslaught.{x\n\r",pc_race_table[LastRace].name);
//Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
				arena_win();
				return TRUE;
			  }
		...
		case ARENA_CLASS:
			if (!blnClass)
			  {
//		  	  	xprintf(strArenaSendText,"{RCarnage is brought to an end as the %ss stand proudly to see the sad demise of their foes. {x\n\r",class_table[LastClass].name);
//Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
				arena_win();
				return TRUE;
			  }
		...
		case ARENA_ALIGNMENT:
			if (!blnAlign)
			  {
		  	  	xprintf(strArenaSendText,"{RFrom the bloodshed among the heroes of good and evil, %s emerges victoriously!{x\n\r",kingdom_table[LastKingdom].name);
Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
				arena_win();
				return TRUE;
			  }
		...
		case ARENA_TEAM:
			if (countin < 2)
			  {
		  	  	xprintf(strArenaSendText,"{RThe %s team wins the arena campaign.{x\n\r",(IS_SET(vch->tag_flags,TAG_RED) ? "Red" : "Blue"));
				Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
			  	arena_win();
				return TRUE;
			  }
			...
			if (countin < 2)
			  {
		  	  	...
		  	  	xprintf(strArenaSendText,"{RThe arena announcer salutes %s, the champion of the battlefield.{x\n\r",victim->name);
	//	  	  	xprintf(strArenaSendText,"{RJust minutes ago, %t perished humiliatingly to a fierce lethal strike delivered by %s.{x\n\r");
				Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
			  	arena_win();
				return TRUE;
			  }
...
// ***  ***
	...
	else if ( ( location = get_room_index (ARENA_OUTERRORROOM)) == NULL )
		  {
			char_from_room( ch );
...
{
	char strArenaSendText[MAX_INPUT_LENGTH];	
	xprintf(strArenaSendText,"{RThe arena janitor was subjected to divine intervention by %s.{x\n\r",ch->name);
	Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
	xprintf(strArenaSendText,"{RSand, dirt and rocks whirl around uncontrollably, eradicating all{x\n\r");
	Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
	...
	if (atoi(cmd))
	 ...
	 ArenaNextStart = 0;
	xprintf(strArenaSendText,"{Rtraces of battle. The battlefield will be sparkling in %d ticks.{x\n\r",ArenaNextStart);
	Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
...
void arena_join(CHAR_DATA *ch, char *argument)
{
  ...
  char strArenaSendText[MAX_INPUT_LENGTH];	
	...
	xprintf(strArenaSendText,"%s joins the blood bath!.{x\n\r",ch->name);
        Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
...
char strArenaSendText[MAX_INPUT_LENGTH];	
 ...
 if (victim == NULL)
 	{ bug("No victim in arena!",0); return FALSE; }
...
stop_fighting( victim, TRUE );	
	...
	xprintf(strArenaSendText,"{R%s gets to play dead at the arena, when a devastating thrust by %s forces him/her to the ground.{x\n\r",victim->name,ch->name);
        Log_system (NULL,strArenaSendText,LOG_INET,KNET_NULL,INET_ARENA);
...
void arena_who(CHAR_DATA *ch, char *argument)
{
 ...
 int mvtemp;
 	...
 	xprintf( Text,"{c-----------------------------------------------------\n\r");
 	add_buf(output,Text);
 	xprintf( Text, "'{c-     Name       - Kills -    Race/Class -  Hp  -  Ma  -  Mv  - Position - Fighting -\n\r");
 	add_buf(output,Text);
		 ...
		 && IS_SET(vch->act, PLR_ARENA) )
		  {
 		  ...
 		  if (vch->fighting != NULL)
 		  	{
			 victim = vch->fighting;
			 xprintf( Fighing," %s",victim->name);
			}
 		  else
			xprintf( Fighing," %s","None");
 		  xprintf( Text, "{c%-14s %-5d %5s{x/%3s  %-5d  %-5d %-5d %-15s %-15s\n\r",
 		  vch->name,
 		  0,
...
for ( index = 11305; index != 11376; index++ )
{
