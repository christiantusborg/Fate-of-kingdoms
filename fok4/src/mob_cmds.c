...
void mob_interpret( CHAR_DATA *ch, char *argument )
{
    ...
    xprintf( buf, "Mob_interpret: invalid cmd from mob %d: '%s'",
...
void do_mpstat( CHAR_DATA *ch, char *argument )
{
    ...
    xprintf( arg, "Mobile #%-6d [%s]\n\r",
	victim->pIndexData->vnum, victim->short_descr );
    send_to_char( arg, ch );

    xprintf( arg, "Delay   %-6d [%s]\n\r",
	victim->mprog_delay,
    ...
    {
	xprintf( arg, "[%2d] Trigger [%-8s] Program [%4d] Phrase [%s]\n\r",
	      ++i,
...
void do_mpstat( CHAR_DATA *ch, char *argument )
{
    ...
    xprintf( arg, "Mobile #%d [%s]\n\r", victim->pIndexData->vnum, victim->short_descr );
    send_to_char( arg, ch );

	buf[0]=0;
	for (mem=victim->mob_mem; mem!=NULL; mem=mem->next)
	{
		xprintf(buf,"%s %s (%d %d)",buf, mem->target->name, mem->value, mem->age);
	}

    xprintf( arg, "Delay   %-6d [%s]\n\r", victim->mprog_delay, buf[0] == 0  ? "No target" : buf+1 );
    send_to_char( arg, ch );
    ...
    {
	xprintf( arg, "[%2d] Trigger [%-8s] Program [%d] Phrase [%s]\n\r",  ++i,  prog_type_to_name( mprg->trig_type ),  mprg->vnum,    mprg->trig_phrase );
	send_to_char( arg, ch );
    }
...
void do_mpmload( CHAR_DATA *ch, char *argument )
{
    ...
    {
	xprintf( arg, "Mpmload: bad mob index (%d) from mob %d",
...
void do_mptransfer( CHAR_DATA *ch, char *argument )
{
	...
	for ( victim = ch->in_room->people; victim != NULL; victim = victim_next )
	{
	    ...
	    if ( !IS_NPC(victim) )
	    {
		xprintf( buf, "%s %s", victim->name, arg2 );
 ...
 * with $q and $Q codes in MOBprograms. See also "mob forget".
 *
 * Syntax: mob remember [victim] ([number]) ([timer])
 */
void do_mpremember( CHAR_DATA *ch, char *argument )
{
    char arg[ MAX_INPUT_LENGTH ];
	...
	if (!can_see(ch, get_char_world(ch, arg)))
		return;

    if ( arg[0] != '\0' )
	{
		MOB_MEMORY * mem;
		/*for (mem = ch->mob_mem; mem!=NULL; mem=mem->next)
		{
			if (!strcmp(mem->target->name, arg) break;
		}*/
		
		//if (mem==NULL)
		mem = alloc_mem(sizeof(MOB_MEMORY));
		mem->target = get_char_world(ch, arg); //get the target first, so we can remove it in a sec
		do_mpforget(ch, mem->target->name); //make sure when mobs remember a person, it remembers them at the beginning of the file
...
void do_mpforget( CHAR_DATA *ch, char *argument )
{
	MOB_MEMORY * mem;
	MOB_MEMORY * memNext;
	MOB_MEMORY * memPrev = NULL;

	for (mem = ch->mob_mem; mem!=NULL; mem=memNext)
	{
		memNext = mem->next;
		if (argument[0]=='\0' || !strcmp(mem->target->name, argument))
		{
			if (memPrev == NULL)
				ch->mob_mem = memNext;
			else
				memPrev->next = memNext;
			free_mem(mem, sizeof(MOB_MEMORY));
		} else
			memPrev = mem;
	}
    //ch->mprog_target = NULL;
...
void do_mpcall( CHAR_DATA *ch, char *argument )
{
    ...
    OBJ_DATA *obj1, *obj2;
    PROG_CODE *prg;
    extern void program_flow( int, char *, CHAR_DATA *, OBJ_DATA *, ROOM_INDEX_DATA *, CHAR_DATA *, const void *, const void * );
...
void obj_interpret( OBJ_DATA *obj, char *argument )
{
    ...
    xprintf( buf, "Obj_interpret: invalid cmd from obj %d: '%s'",
...
void do_opmload( OBJ_DATA *obj, char *argument )
{
    ...
    if ( ( pMobIndex = get_mob_index( vnum ) ) == NULL )
    {
	xprintf( arg, "Opmload: bad mob index (%d) from obj %d",
...
void room_interpret( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    xprintf( buf, "Room_interpret: invalid cmd from room %d: '%s'",
	room->vnum, command );
    bug( buf, 0 );
}
...
void do_rpmload( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    if ( ( pMobIndex = get_mob_index( vnum ) ) == NULL )
    {
	xprintf( arg, "Rpmload: bad mob index (%d) from room %d",
	    vnum, room->vnum );
	bug( arg, 0 );
	return;
    }
