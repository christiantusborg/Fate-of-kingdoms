...
void who_html_update (void)
{
  ...
  for ( d = descriptor_list; d != NULL ; d = d->next )
  {
    ...
    {
      xprintf(buf2, "%6s", pc_race_table[wch->race].who_name);
      get_who_data(buf, buf2, strlen(buf2));  
      fprintf(fp, buf);
      xprintf(buf2, "%s", wch->pcdata->pretitle);
      get_who_data(buf, buf2, strlen(buf2));
      fprintf(fp, buf);
      xprintf(buf, "%s", wch->name);
      fprintf(fp, buf);
      buf2[0] = '\0';
      xprintf(buf2, "%s", wch->pcdata->title);
  ...
  xprintf(buf, "This file last updated at %s Eastern Time.\n", ((char *) ctime( &current_time )));
  fprintf(fp, buf);
  fprintf(fp, "<br>\n");
