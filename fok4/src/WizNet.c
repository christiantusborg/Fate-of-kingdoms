...
const   struct wiznet_type      wiznet_table    []              =
{
   ...
   {    "mem",        "{C[{R{&WARRNING{X{C]{x ",	WIZ_MEMCHECK,   L5 },      
   {    "bug",        "{C[{R{&BUG{X{C]{x ",	WIZ_BUG,   L5 },      
...
void do_wiznet( CHAR_DATA *ch, char *argument )
  {
				...
				if (wiznet_table[flag].level <= get_trust(ch))
				  {
					...
					if (IS_SET(ch->wiznet,wiznet_table[flag].flag))
						xprintf(flagz, "{C%-10s{x ",wiznet_table[flag].name);
					else
						xprintf(flagz, "{c%-10s{x ",wiznet_table[flag].name);
		...
		for (flag = 0; wiznet_table[flag].name != NULL; flag++)
		  {
			if (wiznet_table[flag].level <= get_trust(ch))
			  {
				xprintf(buf,"{cYou will now see {C%s{c on wiznet.{x\n\r",
	...
	if (IS_SET(ch->wiznet,wiznet_table[flag].flag))
	  {
		xprintf(buf,"{cYou will no longer see {C%s{c on wiznet.{x\n\r",wiznet_table[flag].name);
		send_to_char(buf,ch);
