...
*     By using this code, you have agreed to follow the terms of the       *
*     Fate license.  The use f this code without the permission of the    *
...
const struct olc_help_type help_table[] =
{
 ...
 //   {   "size",		size_flags,	 "Which sizes can wear."  },
    {   "race",		race_flags,	 "Which races can wear."         },
...
/* ROM specific bits: */
    ...
    {   "ingredient",	ingredient_flags,"Ingredient types."		 },
...
void show_spec_cmds( CHAR_DATA *ch )
{
    ...
    {
	xprintf( buf, "%-19.18s", &spec_table[spec].name[5] );
	xprintf( buf1,"%s%s",buf1,buf );
	if ( ++col % 4 == 0 )
	    xprintf( buf1, "%s\n\r",buf1 );
    }
...
AEDIT( aedit_show )
{
    ...
    xprintf( buf, "Name:     [%5d] %s\n\r", pArea->vnum, pArea->name );
    send_to_char( buf, ch );

#if 0  /* ROM OLC */
    xprintf( buf, "Recall:   [%5d] %s\n\r", pArea->recall,
    ...
    xprintf( buf, "Vnums:    [%d-%d]\n\r", pArea->min_vnum, pArea->max_vnum );
    send_to_char( buf, ch );

    xprintf( buf, "Age:      [%d]\n\r",	pArea->age );
    send_to_char( buf, ch );

    xprintf( buf, "Players:  [%d]\n\r", pArea->nplayer );
    send_to_char( buf, ch );

    xprintf( buf, "Security: [%d]\n\r", pArea->security );
    send_to_char( buf, ch );

    xprintf( buf, "Builders: [%s]\n\r", pArea->builders );
    send_to_char( buf, ch );

    xprintf( buf, "Credits : [%s]\n\r", pArea->credits );
    send_to_char( buf, ch );
...
REDIT( redit_mreset )
{
    ...
    xprintf( output, "%s (%d) has been loaded and added to resets.{x\n\r"
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
	case ITEM_WEAPON:
            xprintf( buf, "[v0] Weapon class:   %s\n\r",
		     flag_string( weapon_class, obj->value[0] ) );
	    send_to_char( buf, ch );
	    xprintf( buf, "[v1] Number of dice: [%d]\n\r", obj->value[1] );
	    send_to_char( buf, ch );
	    xprintf( buf, "[v2] Type of dice:   [%d]\n\r", obj->value[2] );
	    send_to_char( buf, ch );
	    xprintf( buf, "[v3] Type:           %s\n\r",
		    attack_table[obj->value[3]].name );
	    send_to_char( buf, ch );
 	    xprintf( buf, "[v4] Special type:   %s\n\r",
	...
	case ITEM_TRAP:    
            xprintf( buf, "[v0] Trap Triggers:  %s\n\r",
		     flag_string( trap_triggers, obj->value[0] ) );
	    send_to_char( buf, ch );            

	    xprintf( buf, "[v1] Damage Type:    %s\n\r",
		    trap_attack_table[obj->value[1]].name );
	    send_to_char( buf, ch );

	    xprintf( buf, "[v2] Charges Left:   [%d]\n\r", obj->value[2]);
	    send_to_char( buf, ch );

	    xprintf( buf, "[v3] Charges Total:  [%d]\n\r", obj->value[3]);
	    send_to_char( buf, ch );
...
/* WEAPONS changed in ROM */
	    ...
	    switch ( value_num )
	    {
	        ...
	        case 3:
	            ...
	            pObj->value[3] = weapon_attack_lookup( argument );
	            break;
	        case 4:
	...
	case ITEM_INGREDIENT:
	send_to_char( "INGREDIENT CHOSEN.{x\n\r\n\r", ch);
 	pObj->value[0] = flag_value( ingredient_flags, argument );
	break;
...
MEDIT( medit_nshow )
{
	...
	send_to_char("{c-Medit---------------------------------------------------------------\n\r", ch);
    		xprintf( buf,"{cArea nr:    {w%-15d {cArea name: {w%s{x\n\r",!pMob->area ? -1 : pMob->area->vnum ,!pMob->area ? "No Area" : pMob->area->name);
		send_to_char( buf, ch );

    		xprintf( buf, "{cVnum:       {w%-15d {cName:      {w%s{x\n\r",pMob->vnum,pMob->player_name);
		send_to_char( buf, ch );
	
		xprintf( buf, "{cClass:      {w%-15s {cRace:      {w%s {x\n\r","Warrior","Human");
		send_to_char( buf, ch );

		xprintf( buf, "{cLevel:      {w%-15d {cSize:      {w%s {x\n\r",pMob->level,flag_string( size_flags, pMob->size ));
		send_to_char( buf, ch );
		
		xprintf( buf, "{cDifficulty: {w%-15s {cSex        {w%s\n\r","Hard",pMob->sex == SEX_MALE    ? "male" : pMob->sex == SEX_FEMALE  ? "female" : pMob->sex == 3 ? "random" : "neutral");
		send_to_char( buf, ch );

    		xprintf( buf, "{cMaterial:   {w%-15s {cAlign      {w%d\n\r",pMob->material,pMob->alignment );
		send_to_char( buf, ch );

		xprintf( buf, "{cWealth:     {w%-15d\n\r",pMob->wealth );
		send_to_char( buf, ch );

		xprintf( buf, "{cGroup: {w%-15d {cAssist:{w %s", pMob->group,"Group, Vnum, Race, Class, Guard, Align" );
		send_to_char( buf, ch );

	send_to_char("{c---------------------------------------------------------------------\n\r", ch);
		
		xprintf( buf, "{cHit dice:    {w%3dd%-3d+%-5d   ",pMob->hit[DICE_NUMBER],pMob->hit[DICE_TYPE],pMob->hit[DICE_BONUS] );
		send_to_char( buf, ch );

		xprintf( buf, "{cMana dice:    {w%3dd%-3d+%-5d\n\r",pMob->mana[DICE_NUMBER], pMob->mana[DICE_TYPE],pMob->mana[DICE_BONUS] );
		send_to_char( buf, ch );

		xprintf( buf, "{cDamage dice: {w%3dd%-3d+%-5d   ",pMob->damage[DICE_NUMBER],pMob->damage[DICE_TYPE],pMob->damage[DICE_BONUS] );
		send_to_char( buf, ch );

		xprintf( buf, "{cHitroll:        {w%d\n\r",pMob->hitroll ); 
    		send_to_char( buf, ch );

		xprintf( buf, "{cAttacks:     {w%-15d ",4);
		send_to_char( buf, ch );

		xprintf( buf, "{cDamage type:    {w%s\n\r",attack_table[pMob->dam_type].name );
		send_to_char( buf, ch );

		xprintf( buf, "{cArmor: [{cpierce: {w%d  {cbash: {w%d  {cslash: {w%d  {cmagic: {w%d{c]\n\r",pMob->ac[AC_PIERCE], pMob->ac[AC_BASH],pMob->ac[AC_SLASH], pMob->ac[AC_EXOTIC] );
		send_to_char( buf, ch );

		xprintf( buf, "Stats: [{c Str: {w%-2d {cInt: {w%-2d {cWis: {w%-2d {cDex: {w%-2d {cAgi: {w%-2d {cCon: {w%-2d {cCha: {w%-2d {c]\n\r",1,2,3,4,5,6,7);
    		send_to_char( buf, ch );


	send_to_char("{c---------------------------------------------------------------------\n\r", ch);
		send_to_char("{cAct: ", ch);
		xprintf( buf, "{w%s\n\r",flag_string( act_flags, pMob->act ) );
		send_to_char( buf, ch );

		send_to_char("{cOffensive act: ", ch);
		xprintf( buf, "{w%s\n\r",flag_string( off_flags,  pMob->off_flags ) );
    		send_to_char( buf, ch );

		send_to_char("{cAffected byt: ", ch);
		xprintf( buf, "{w%s ",flag_string( affect_flags, pMob->affected_by ) );
    		send_to_char( buf, ch );
    		xprintf( buf, "{w%s\n\r",flag_string( affect2_flags, pMob->affected_by2 ) );
    		send_to_char( buf, ch );

		send_to_char("{cDetection Affects: ", ch);
		xprintf( buf, "{w%s\n\r",flag_string( detect_flags, pMob->detection ) );
		send_to_char( buf, ch );

		send_to_char("{cSmart acts: ", ch);
		xprintf( buf, "{w%s\n\r","None");
		send_to_char( buf, ch );

/*		send_to_char("{cImmunity: ", ch); send_to_char("{cVulnerability: ", ch); send_to_char("{cResistance: ", ch); */
	send_to_char("{c---------------------------------------------------------------------\n\r", ch);

		xprintf( buf, "{cStart:{w[%s]     ",flag_string( position_flags, pMob->start_pos ) );
    		send_to_char( buf, ch );

		xprintf( buf, "{cDefault:  {w[%s]\n\r",flag_string( position_flags, pMob->default_pos ) );
		send_to_char( buf, ch );

		xprintf( buf, "{cPart: {w%s\n\r",flag_string( part_flags, pMob->parts ) );
		send_to_char( buf, ch );

		xprintf( buf, "{cShort descr: {w%s\n\r{cLong descr:{w %s",pMob->short_descr,pMob->long_descr );
		send_to_char( buf, ch );

    		xprintf( buf, "{cDescription:\n\r{w%s", pMob->description );
		send_to_char( buf, ch );
    ...
    if ( pMob->spec_fun )
    {
	xprintf( buf, "{MSpec fun:    {c[%s]{x\n\r",  spec_name( pMob->spec_fun ) );
	send_to_char( buf, ch );
    }

    if ( pMob->practicer)
	{
	xprintf( buf, "{MPracticer:	{c[%s]{x\n\r",
...
OEDIT( oedit_show )
{
    ...
    xprintf( buf, "Level:       [%5d]\n\r", pObj->level );
    send_to_char( buf, ch );

    xprintf( buf, "Class flags:  [%s]\n\r",
	flag_string( class_flags, pObj->class_flags ) );
    send_to_char( buf, ch );

    xprintf( buf, "Size flags:  [%s]\n\r",
	flag_string( race_flags, pObj->race_flags ) );
    send_to_char( buf, ch );

    xprintf( buf, "Wear flags:  [%s]\n\r",
    ...
    xprintf( buf, "Material:    [%s]\n\r",                /* ROM */
	pObj->material );
    send_to_char( buf, ch );

    xprintf( buf, "Condition:   [%3d]\n\r",          
	pObj->condition );
    send_to_char( buf, ch );
 
    xprintf( buf, "Limited:	[%d]\n\r",
	pObj->limit );
    send_to_char( buf, ch );

    xprintf( buf, "Repop chance:[%3d]\n\r",               /* ROM */
	pObj->repop );
    send_to_char( buf, ch );

    xprintf( buf, "Weight:      [%5d]\n\rCost:        [%5d]\n\r",
...
/* adjust this next line to change the avg dmg your weapons will get! */
        ...
        dice = (templevel/33 + 1);
        size = dice/2;
        dice  = number_range(dice -2 ,dice +2 ); 
        if (dice < 1)
        	dice = 1;
...
OEDIT( oedit_class )      /* Class Prevention by Takeda 2/26/00 */
{
	...
	SET_BIT(pObj->class_flags,CLASS_NECROMANCER);
	SET_BIT(pObj->class_flags,CLASS_SPELLFILCHER);
	...
	REMOVE_BIT(pObj->class_flags,CLASS_NECROMANCER);
	REMOVE_BIT(pObj->class_flags,CLASS_SPELLFILCHER);
...
OEDIT( oedit_size )  /* Race Prevention by Takeda 2/26/00 */
{
    OBJ_INDEX_DATA *pObj;
    int value;
	...
	send_to_char("All Size Flags are ON.\n\r",ch);
	return TRUE;
	}
	if(!str_cmp(argument,"remove") ) {
	...
	send_to_char("All Size Flags are OFF.\n\r",ch);
	return TRUE;
	}
	...
	REMOVE_BIT(pObj->race_flags,RACE_TROLL);
	REMOVE_BIT(pObj->race_flags,RACE_OGRE);
	send_to_char("Giant Flag is SET.\n\r",ch);
	return TRUE;
	}
	...
	if ( ( value = flag_value( race_flags, argument ) ) != NO_FLAG )
	{
	    ...
	    send_to_char( "The race has been added.{x\n\r", ch);
	    return TRUE;
	}
    }

    send_to_char( "Syntax:  size [flag]\n\r"
		  "Type '? size' for a list of flags.{x\n\r", ch );
    return FALSE;
}
...
MEDIT( medit_show )
{
	...
	send_to_char("{c--------------------------------------------------------------------\n\r", ch);
	xprintf( buf, "{wVnum: {c({y%6d{c) {wArea: {c({y%d{c) {c({y%s{c)\n\r",pMob->vnum,!pMob->area ? -1        : pMob->area->vnum,!pMob->area ? "No Area" : pMob->area->name);
	send_to_char( buf, ch );
	send_to_char("{c--------------------------------------------------------------------\n\r", ch);
  xprintf( buf, "{wLevel: {y%3d {wName: {c({y%s{c){x\n\r",pMob->level,pMob->player_name);
    send_to_char( buf, ch );
  xprintf( buf, "{wRace: {c({y%6.6s{c) {wClass: {c({y%10.10s{c) {wSex: {c({y%s{c)\n\r",race_table[pMob->race].name,npc_class_table[pMob->class].name,pMob->sex == SEX_MALE    ? "male"  : pMob->sex == SEX_FEMALE  ? "female" : pMob->sex == 3           ? "random" : "neutral"  );
  send_to_char( buf, ch );
  xprintf( buf, "{wShort descr: {y%s\n\r",pMob->short_descr);
  send_to_char( buf, ch );
  xprintf( buf, "{wLong descr:  {y%s\n\r",pMob->long_descr);
  send_to_char( buf, ch );
	send_to_char("{c--------------------------------------------------------------------\n\r", ch);
  xprintf( buf,"{wAlignment: {c({y%6d{d{c) {wGroup: {c({y%6d{c)\n\r",pMob->alignment,pMob->group);
  send_to_char( buf, ch );

    xprintf( buf, "{wSize: {c({y%s{c) ",
	flag_string( size_flags, pMob->size ) );
    send_to_char( buf, ch );

    xprintf( buf, "{wMaterial: {c({y%s{c) ",
        pMob->material );
    send_to_char( buf, ch );

    xprintf( buf, "{wWealth: {c({y%d{c)\n\r",
	pMob->wealth );
    send_to_char( buf, ch );

    xprintf( buf, "{wStart: {c{c({y%s{c) ",
	flag_string( position_flags, pMob->start_pos ) );
    send_to_char( buf, ch );

    xprintf( buf, "{wDefault:  {c({y%s{c)\n\r",
	flag_string( position_flags, pMob->default_pos ) );
    send_to_char( buf, ch );


    xprintf( buf, "{wHit dice: {c({y%2dd%-3d+%4d{c) ",
	     pMob->hit[DICE_NUMBER],
	     pMob->hit[DICE_TYPE],
	     pMob->hit[DICE_BONUS] );
    send_to_char( buf, ch );

    xprintf( buf, "{wDamage dice: {c({y%2dd%-3d+%4d{c)\n\r",
	     pMob->damage[DICE_NUMBER],
	     pMob->damage[DICE_TYPE],
	     pMob->damage[DICE_BONUS] );
    send_to_char( buf, ch );

    xprintf( buf, "{wDam Type: {c({y%11.11s{c)\n\r",
    	     attack_table[pMob->dam_type].name );
    send_to_char( buf, ch );

    xprintf( buf, "{wArmor: {c({wpierce: {y%d  {wbash: {y%d  {wslash: {y%d  {wmagic: {y%d{c)\n\r",
...
/*  removes
    xprintf( buf, "Mana dice:   [%2dd%-3d+%4d]\n\r",
...
/* ROM values end */
    ...
    if ( pMob->spec_fun )
    {
	xprintf( buf, "{wSpec fun:    {c({y%s{c)\n\r",  spec_name( pMob->spec_fun ) );
	send_to_char( buf, ch );
    }

    if ( pMob->practicer)
	{
	xprintf( buf, "{wPracticer:	{c({y%s{c){x\n\r",
	prac_table[pMob->practicer].sh_name);
	send_to_char( buf, ch );
    }


    xprintf( buf, "Description:\n\r%s", pMob->description );
    send_to_char( buf, ch );
...
MEDIT( medit_level )
{
	...
	return FALSE;
        ...
        if ((atoi(argument) <= 1) || (atoi(argument) > 200))
         {
send_to_char("Mob level ranges from level 1 to 200.\n\r", ch);
return FALSE;
}
...
MEDIT( medit_long )
{
    ...
    xprintf( strsave,"%s%s",argument, "\n\r" );
    free_string( pMob->long_descr );
    pMob->long_descr = str_dup( strsave );
...
MEDIT( medit_name )
{
    ...
    xprintf(strsave, "%s%s", PLAYER_DIR, capitalize(argument));
...
MEDIT( medit_shop )
{
	...
	{
	    xprintf( buf, "MEdit:  May sell %d items max.{x\n\r", MAX_TRADE );
	    send_to_char( buf, ch );
	    return FALSE;
	}
...
int ac_n, ac_x;
EDIT_MOB( ch, pMob );
        if ((pMob->level <= 1) || (pMob->level > 150))
{
send_to_char("Mob level ranges from level 1 to 150.\n\r", ch);
return FALSE;
}
...
size = 10;
bonus = 85;
xprintf(temp, "%dd%d + %d", dice, size, bonus);
...
bonus = ((pMob->level * pMob->level));
size = (pMob->level);
dice = (pMob->level*3);
xprintf(temp, "%dD%d + %d", dice, size, bonus);
medit_hitdice(ch, temp);
xprintf(temp, "%d", pMob->level);
...
MEDIT( medit_race )
{
	...
	for ( race = 0; race_table[race].name != NULL; race++ )
	{
	    if ( ( race % 3 ) == 0 )
		send_to_char( "{x\n\r", ch );
	    xprintf( buf, " %-15s", race_table[race].name );
	    send_to_char( buf, ch );
	}
...
MEDIT( medit_class )
{
	...
	int npc_class;
		...
		if ( argument[0] != '\0'
		 && ( npc_class = npc_class_lookup( argument ) ) != 0 )
		 {
			EDIT_MOB( ch, pMob );
			pMob->class = npc_class;  
			send_to_char( "Class set.{x\n\r", ch );
			//medit_auto(ch,pMob)
			return TRUE;
		 }
	...
	if ( argument[0] == '?' )
	 {
		...
		for ( npc_class = 1; npc_class_table[npc_class].name != NULL; npc_class++ )
		 {
			if ( ( npc_class % 4 ) == 0 )
				send_to_char( "{x\n\r", ch );
				xprintf( buf, " %-15s", npc_class_table[npc_class].name );
				send_to_char( buf, ch );
		 }
		...
		send_to_char( "{x\n\r", ch );
		send_to_char( "Syntax:  setclass [class]\n\r", ch );
    return FALSE;
}
///****
...
void show_liqlist(CHAR_DATA *ch)
{
	...
	xprintf(buf, "%-20s %-14s %5d %4d %6d %4d %5d\n\r",
...
void show_damlist(CHAR_DATA *ch)
{
	...
	if (ch->desc->editor == ED_OBJECT && attack_table[att].weaponallow == 0)
		continue;
...
REDIT( redit_delete )
{
  ...
  /* Now PCs and Mobs */
      ...
      for( i = 0; i <= MAX_DIR; i++ )
        ...
        edit_done(ch);
        xprintf( buf, "Removed room vnum {C%d{x, %d resets, %d extra "
             "descriptions and %d exits.\n\r", index, rcount, edcount,ecount );
 send_to_char( buf, ch );
        xprintf( buf, "{C%d{x objects and {C%d{x mobiles were extracted "
                "from the room.\n\r", ocount, mcount );
        send_to_char( buf, ch );
