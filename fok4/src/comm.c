 ...
 */
    ...
    init_signals();    
    log_string( "Wiping Inactive Pfiles");
    wipe_pfiles();
    log_string( "Removing Inactive Archive Pfiles");
    remove_inactive( ARCHIVE_DIR, ARCHIVE_EXPIRE); 
  if (port == 4000)
   {
    log_string( "Setting Mud Pid");
    getpidmud();
   }
    loadhelp = TRUE;
    boot_db( );
...
/*
   ...
   write_to_descriptor( desc,
                     ...
                     " authorization.\n\r"
                     "Please send email to ban@kingdom.dk \n\r"
...
bool process_output( DESCRIPTOR_DATA *d, bool fPrompt )
{
	...
	ch = d->character;
		...
		if (victim->max_hit > 0)
            ...
            if (percent >= 100)
                xprintf(wound,"{Gis in perfect health.{w");
            else if (percent >= 90)
                xprintf(wound,"{ghas a few scratches.{g");
            else if (percent >= 75)
                xprintf(wound,"{Mhas some small but disgusting cuts.{w");
            else if (percent >= 50)
                xprintf(wound,"{mis covered with bleeding wounds.{w");
            else if (percent >= 30)
                xprintf(wound,"{Yis gushing blood.{w");
            else if (percent >= 15)
                xprintf(wound,"{Ris writhing in agony.{w");
            else if (percent >= 0)
                xprintf(wound,"{ris convulsing on the ground.{w");
            else
                xprintf(wound,"{yis nearly dead.{w");
...
void bust_a_prompt( CHAR_DATA *ch )
{
    ...
    if (!str || str[0] == '\0')
    {
        xprintf( buf, "{Y<{R%dhp {B%dm {C%dmv {G%s {Y>{w",
   ...
   while( *str != '\0' )
   {
	...
	i = buf2; break;
	    ...
	    if ( ch->level >= 40 )
	      ...
	      xprintf( buf2, "%d%%", PERCENT(ch->mana, ch->max_mana ));
            i = buf2; break;
         case 'M' :
	    ...
	    if ( ch->level >= 40 )
	      ...
	      xprintf( buf2, "%d%%", PERCENT( ch->move, ch->max_move) );
            i = buf2; break;
         case 'V' :
            xprintf( buf2, "%d", ch->max_move );
            i = buf2; break;
	case 'X' :
	xprintf(buf2, "%d",  IS_NPC(ch) ? 0 :
...
/*         case 'x' :
	 ...
	 case 'X' :
	    ...
	    i = buf2; break; */
         case 'g' :
            xprintf( buf2, "%ld", ch->gold);
            i = buf2; break;
	 case 's' :
	    xprintf( buf2, "%ld", ch->silver);
	    i = buf2; break;
         case 'a' :
            if( ch->level > 9 )
               xprintf( buf2, "%d", ch->alignment );
            else
               xprintf( buf2, "%s", IS_GOOD(ch) ? "good" : IS_EVIL(ch) ?
                "evil" : "neutral" );
            i = buf2; break;
         case 'r' :
            if( ch->in_room != NULL )
		...
		? ch->in_room->name : "darkness");
         ...
         case '%' :
            ...
            i = buf2; break;
         case 'o' :
            xprintf( buf2, "%s", olc_ed_name(ch) );
            i = buf2; break;
	 case 'O' :
	    xprintf( buf2, "%s", olc_ed_vnum(ch) );
	    i = buf2; break;
      }
      ++str;
      while( (*point = *i) != '\0' )
...
xprintf(centered,"%*c%s",spaces,' ',argument);
...
/* middle */
xprintf(buf,"{C%c",UPPER(argument[0]));
center_to_char(buf,ch,72-(2*size+2) );

for (place=1;place<size;place++) 
	{
        xprintf(buf," %c",UPPER(argument[place]));
	send_to_char(buf,ch);
	}
send_to_char("{x\n\r",ch);
...
int colour( char type, CHAR_DATA *ch, char *string )
{
	...
	case 'c':
	    ...
	    break;
	case 'g':
	    xprintf( code, C_GREEN );
	    break;
	case 'm':
	    xprintf( code, C_MAGENTA );
	    break;
	case 'r':
	    xprintf( code, C_RED );
	    break;
	case 'w':
	    xprintf( code, C_WHITE );
	    break;
	case 'y':
	    xprintf( code, C_YELLOW );
	    break;
	case 'B':
	    xprintf( code, C_B_BLUE );
	    break;
	case 'C':
	    xprintf( code, C_B_CYAN );
	    break;
	case 'G':
	    xprintf( code, C_B_GREEN );
	    break;
	case 'M':
	    xprintf( code, C_B_MAGENTA );
	    break;
	case 'R':
	    xprintf( code, C_B_RED );
	    break;
	case 'W':
	    xprintf( code, C_B_WHITE );
	    break;
	case 'Y':
	    xprintf( code, C_B_YELLOW );
	    break;
	case 'D':
	    xprintf( code, C_D_GREY );
	    break;
	case '*':
	    xprintf( code, "%c", 007 );
	    break;
...
{ 
	/*Now save from the crash by doing a copyover!!*/
	copyover_crash();
}
