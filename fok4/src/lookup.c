...
int npc_class_lookup (const char *name)
{
   int npcclass;

   for ( npcclass = 0; npc_class_table[npcclass].name != NULL; npcclass++)
   {
       if (LOWER(name[0]) == LOWER(npc_class_table[npcclass].name[0])
       &&  !str_prefix( name,npc_class_table[npcclass].name))
           return npcclass;
   }
