 ...
 */ 
   ...
   if((ch->race == race_lookup("illithid")) &&
        ...
        racenow = 1;
   if(racenow == 1) {
	sprintf(buf,"Your size prevents you from wearing %s.\n",
		obj->short_descr);
        send_to_char(buf,ch);
	return;
   }
...
*/
        ...
        if (!IS_SET(ch->act, PLR_THIEF))
	{
          ...
          log_string( buf );
          send_to_char( "You've been caught stealing and is now 
WANTED.\n\r", ch);
	}
    ...
    if (auction->item != NULL)
        if (--auction->pulse <= 0) /* decrease pulse */
        {
			...
			auction->item->short_descr,
                ...
                sprintf (buf, "%s: going %s (no bets received yet).",
			auction->item->short_descr,
	...
	if ( ch == auction->seller )
	 {
	  ...
	  return;
         ...
         if (argument[0] == '\0')
            ...
            {
     send_to_char ("You must at least bid 10% more than the current bet 
{Ygold{w over the current bet.\n\r",ch);
                return;
            }
...
sprintf (buf, "Your interest %d Gold and %d Silver\n\r",gold,silver);
send_to_char(buf,ch);
return;
}
