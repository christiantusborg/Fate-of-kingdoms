	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_CLAN:
	    return( lval_char != NULL && lval_char->kingdom == clan_lookup(buf ) );
	case CHK_RACE:
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
    ...
    while ( *str != '\0' )
    {
		...
		? ( IS_NPC( ch ) ? ch->short_descr : ch->pcdata->pretitle )
		: someone;                         		break;
            case 't': 
		i = someone;
...
void mp_act_trigger(  char *argument, CHAR_DATA *mob, CHAR_DATA *ch,  const void *arg1, const void *arg2, int type )
{
   MPROG_LIST *prg;
	...
	// ABORT THE TRIGGER CRASHES THE MUD!! 
	return;
...
(
	...
	if (mob->pIndexData->moprogs)
	   return;

	if (!mob->pIndexData->mprogs)
	   return;
*/
...
//return;
  if (!IS_NPC(ch)) // NPCs do trigger it 
	return;
...
int clan_lookup (const char *name)
{
	int clan;
	     ...
	     if (!strcmp(name, kingdom_table[1].name)) 	{clan=1;}
	else if (!strcmp(name, kingdom_table[2].name)) 	{clan=2;}
	else if (!strcmp(name, kingdom_table[3].name)) 	{clan=3;}
	else if (!strcmp(name, kingdom_table[4].name)) 	{clan=4;}
	else if (!strcmp(name, kingdom_table[5].name)) 	{clan=5;}
	else if (!strcmp(name, kingdom_table[6].name)) 	{clan=6;}
	else if (!strcmp(name, kingdom_table[7].name))	{clan=7;}
        else return 0; /* none found return*/
        return clan;
}
