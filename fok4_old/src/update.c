...
*/
   if (((current_time - ch->time_last_level)<LAST_LEVEL_TIME) && (ch->level >= 30))
    {
     sprintf( log_buf, "%s gained a in really short time and he is %d)",ch->name,ch->level);
     log_string( log_buf );
...
//		    class_table[ch->class].hp_max );
    add_mana 	= 2 + ((get_curr_stat(ch,STAT_INT)*get_curr_stat(ch,STAT_INT))/20.0 + (get_curr_stat(ch,STAT_WIS)/7) + number_range(-2,6));
    
add_hp = con_app[get_curr_stat(ch,STAT_CON)].hitp + number_range(1,class_table[ch->class].hp_max +5);
add_hp += number_range(1,pc_race_table[ch->race].hp_bonus  +5 );
...
int move_gain( CHAR_DATA *ch )
{
	...
	switch ( ch->position )
	{
	case POS_SLEEPING: gain += UMAX(get_curr_stat(ch,STAT_DEX),get_curr_stat(ch,STAT_CON));		break;
	case POS_RESTING:  gain += UMAX(get_curr_stat(ch,STAT_DEX),get_curr_stat(ch,STAT_CON)) / 2;	break;
	}
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	(ch->in_room == get_room_index(ROOM_VNUM_AFTERLIFE)))
	return;
...
//	    damage( ch, ch, 10, TYPE_HIT, DAM_HUNGER, TRUE );
	...
	case COND_THIRST:
	    ...
	    ch->hit -= ch->max_hit * number_range(2, 4) / 100;
//	    damage( ch, ch, 10, TYPE_HIT, DAM_THIRST, TRUE );
	    break;
...
void mobile_update( void )
{
	...
	ch_next = ch->next;
        ...
        if (IS_NPC(ch) && !IS_AFFECTED(ch,AFF_CALM)
            && (ch->spec_fun != spec_lookup("spec_cleric") )
//            && (ch->spec_fun != spec_lookup("spec_assassin") )
//            && (ch->spec_fun != spec_lookup("spec_special_guard") )
//            && (ch->spec_fun != spec_lookup("spec_fasttracking") )
            && ch->fighting == NULL
...
//            do_track(ch,ch->last_fought->name);
   ...
   if ( IS_AFFECTED2(ch, AFF2_SUBMISSION ))
      { 
       ...
       victim = ch->fighting;
        ...
        if ( IS_AFFECTED2(victim, AFF2_ARMLOCK) && victim != NULL)
        {
          if (ch->position == POS_FIGHTING && ch->fighting == victim)
          {
          	...
          	chance -= get_curr_stat(victim,STAT_STR) * 3;
       		...
       		if (number_percent() < chance)
       		{
       		   ...
       		   switch(number_range(1,8))
       		    {
     			case 1:
     			act( "{yYou crush a bone in $N's arm.", ch, NULL, victim, TO_CHAR );
     			act( "{y$N wince in agony and falls to his knees as $n breaks his arm.", ch, NULL, victim, TO_NOTVICT );
     			act( "{yYou scream in agony as you feel your arm break.", ch, NULL, victim, TO_VICT ); 
     			dam *= 5/2;    	 
     			victim->position = POS_RESTING;
         		break;
     			case 2:
     			act( "{yYou manage to twist $N's arm counterclockwise, causing him to lose his balance.", ch, NULL, victim, TO_CHAR );
     			act( "{y$N drops to the ground as $n twists his arm to an unhealthy position.", ch, NULL, victim, TO_NOTVICT );
     			act( "{yYou lose your balance due to a tricky move from $n.", ch, NULL, victim, TO_VICT );
     			victim->position = POS_SLEEPING;
     			break;
     			case 3:
     			act( "{yYou fail to get a good grip in $N's arm, and he almost escapes.", ch, NULL, victim, TO_CHAR );
     			act( "{y$N almost manages to pry himself lose.", ch, NULL, victim, TO_NOTVICT );
     			act( "{yYou almost manage to escape as $n's hand slips.", ch, NULL, victim, TO_VICT );
     			dam = 0;
     			break;
     			case 4:
     			act( "{yYou twirl $N's arm to an awkward position.", ch, NULL, victim, TO_CHAR );
     			act( "{y$n twists $N's arm in a painful way.", ch, NULL, victim, TO_NOTVICT );
     			act( "{yYou feel an intense pain as $n twists your arm.", ch, NULL, victim, TO_VICT );
     			dam  *= 3/2;
     			break;
     			case 5:    
     			act( "{yYour attempt to break $N's arm fails miserably and he almost slips.", ch, NULL, victim, TO_CHAR );
     			act( "{y$n attempts to break $N's arm, but fails miserably.", ch, NULL, victim, TO_NOTVICT );
     			act( "{yYou blocks $n's assault at your arm.", ch, NULL, victim, TO_VICT );
     			dam *= 3;
     			break;
     			case 6:    
     			break;
     		...
     		{
     			REMOVE_BIT(ch->affected_by2, AFF2_SUBMISSION);
     			REMOVE_BIT(victim->affected_by2, AFF2_ARMLOCK);
     			act( "{yYou lose your grip on $N's arm.", ch, NULL, victim, TO_CHAR );
     			act( "{y$N forces $n to let go of his arm.", ch, NULL, victim, TO_NOTVICT );
     			act( "{yWith a quick pull $n lose the grip of your arm.", ch, NULL, victim, TO_VICT );
       		}
       	   }
       	   else
       	   REMOVE_BIT(victim->affected_by2, AFF2_ARMLOCK);
  ...
  if ( ch->race == 13 && ch->in_room != NULL )
	...
	if (is_safe_spell(ch,vch,TRUE))
	  ...
	  if (vch->fighting == ch)
          damage(ch,vch,dam,gsn_blade_barrier,DAM_OTHER,TRUE);
         }
 ...
 if ( IS_AFFECTED2(ch, AFF2_CONTAGION ) && ch->in_room != NULL )
        {
         switch(number_range(1,5))
         {
     case 1:
        af.where     = TO_AFFECTS;
    	af.type      = gsn_contagion_s;
    	af.level     = ch->level;
    	af.bitvector = 0;
    	af.duration = 30;
    	...
    	af.modifier  = -1;
    	affect_join( ch, &af );
    	send_to_char( "Everything seems a bit blurry.\n\r", ch );
         break;
     case 2:
        af.where     = TO_AFFECTS;
    	af.type      = gsn_contagion_i;
    	af.level     = ch->level;
    	af.duration = 30;
    	...
    	af.bitvector = 0;
    	affect_join( ch, &af );
     	send_to_char( "Red dots appear on your skin.\n\r", ch );
     break;
     case 3:
        af.where     = TO_AFFECTS;
    	af.type      = gsn_contagion_w;
    	af.level     = ch->level;
    	af.duration = 30;
    	af.location  = APPLY_WIS;
     	...
     	af.bitvector = 0;
    	affect_join( ch, &af );
     	send_to_char( "Your headache is increasing.\n\r", ch );
     break;
     case 4:
        af.where     = TO_AFFECTS;
    	af.type      = gsn_contagion_d;
    	af.level     = ch->level;
    	af.duration = 30;
    	...
    	af.bitvector = 0;
    	affect_join( ch, &af );
    	send_to_char( "Your stomach hurts.\n\r", ch );
     break;
     case 5:
        af.where     = TO_AFFECTS;
    	af.type      = gsn_contagion_c;
    	af.level     = ch->level;
    	af.duration = 30;
     ...
     break;
        ...
        for ( vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
            {
                if (!saves_spell(vch->level,vch,DAM_DISEASE) 
		&&  !IS_IMMORTAL(vch)
		&&  !IS_SET(vch->imm_flags,IMM_DISEASE)
            	&&  !IS_AFFECTED(vch,AFF2_CONTAGION) && number_bits(2) == 0
            	&& !IS_SET(vch->form,FORM_UNDEAD)
        	&& !IS_SET(vch->form,FORM_CONSTRUCT))
            	{
            	    send_to_char("{gYour stomach doesn't act normally.{x\n\r",vch);
            	    act("{g$n is infected by a contagious disease!!.{x",vch,NULL,NULL,TO_ROOM);
...
if (!IS_NPC(ch) )
 if (ch->pcdata->condition[COND_TOXIC] > 1) 
  ch->pcdata->condition[COND_TOXIC] -= 2;
...
//    static  int     pulse_violence;
    static  int     Pulse_Violence_Tiny;
    static  int     Pulse_Violence_Small;
    static  int     Pulse_Violence_Medium;
    static  int     Pulse_Violence_Large;
    static  int     Pulse_Violence_Huge;
    static  int     Pulse_Violence_Giant;
    static  int     Pulse_Violence_act;
...
/*	song_update(); */
    ...
    if ( --Pulse_Violence_Tiny <= 0 )
    {
	Pulse_Violence_Tiny= PULSE_VIOLENCE_TINY;
	violence_update	(SIZE_TINY);
    }
    
    if ( --Pulse_Violence_Small <= 0 )
    {
	Pulse_Violence_Small= PULSE_VIOLENCE_SMALL;
	violence_update	(SIZE_SMALL);
    }
    
    if ( --Pulse_Violence_Medium <= 0 )
    {
	Pulse_Violence_Medium= PULSE_VIOLENCE_MEDIUM;
	violence_update	(SIZE_MEDIUM);
    }

    if ( --Pulse_Violence_act <= 0 )
    {
	Pulse_Violence_act= PULSE_VIOLENCE_ACT;
	violence_update_act();
    }
    
    if ( --Pulse_Violence_Large <= 0 )
    {
	Pulse_Violence_Large= PULSE_VIOLENCE_LARGE;
	violence_update	(SIZE_LARGE);
    }
    
    if ( --Pulse_Violence_Huge <= 0 )
    {
	Pulse_Violence_Huge= PULSE_VIOLENCE_HUGE;
	violence_update	(SIZE_HUGE);
    }
    
    if ( --Pulse_Violence_Giant <= 0 )
    {
	Pulse_Violence_Giant= PULSE_VIOLENCE_GIANT;
	violence_update	(SIZE_GIANT);
    }
 ...
 switch(copyover_counter)
 {
  ...
  case 60:
    if (copyover_counter == 1)
        sprintf(buf,"***** COPYOVER IN %i TICK *****\n\r",copyover_counter);
    else
...
void room_affect_update( void )
{
    ...
    //   &&  !is_safe_rspell(af->level,vch)
        ...
        while (IS_ROOM_AFFECTED(room, AFF_ROOM_REGENERATION) && room->people != NULL)
        {
            AFFECT_DATA *af, paf;
            CHAR_DATA *vch;
            OBJ_DATA *obj;
            ...
            for ( af = room->affected; af != NULL; af = af->next )
            {
                if (af->type == gsn_totem)
                    break;
            }

            if (af == NULL)
            {
                REMOVE_BIT(room->affected_by,AFF_ROOM_REGENERATION);
                break;
        }
	  ...
	  for ( vch = room->people; vch != NULL; vch = vch->next_in_room)
            {
             for ( obj = vch->in_room->contents; obj != NULL; obj = obj->next_content )
	     {
	      
	      if ( obj->pIndexData->vnum == OBJ_VNUM_TOTEM )
	      {
		            
            	if (vch->race == race_lookup("ogre") && !IS_NPC(vch))
            	{
               	vch->hit = UMIN( vch->hit + af->level / 2, vch->max_hit );
        	}
            	
                if ((vch->race != race_lookup("ogre"))
                &&  !IS_IMMORTAL(vch)
                &&  !(IS_NPC(vch) && IS_SET(vch->act,ACT_UNDEAD) )
                &&  !IS_AFFECTED(vch,AFF_SLEEP))
                {
                  if (!saves_spell(af->level+15,vch,DAM_NEGATIVE))
                  {
    		send_to_char( "The monstrous and heinious totem fills you with fear.\n\r",vch);
    		act("$n's eyes grow wide with terror.",vch,NULL,NULL,TO_ROOM);
    		interpret(vch,"flee");
    		  }
                }
       	      }
