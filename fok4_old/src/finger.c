...
if (ch->level >= 87)
	{
		...
		sprintf(buf, "%s{x last logged on from %s{x\n\r",victim->name,victim->pcdata->host);
		send_to_char(buf, ch);
	}
send_to_char("---------------------------------------------------------------\n\r", ch);
}
