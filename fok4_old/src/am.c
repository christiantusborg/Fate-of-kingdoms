...
void do_train( CHAR_DATA *ch, char *argument )
{
 ...
 send_to_char("Train stats is disaled on fate of kingdom", ch );
 return;
...
void do_push_drag( CHAR_DATA *ch, char *argument, char *verb )
{
 ...
 char * const rev_name [] =
  ...
  else
  {
    ...
    {
	sprintf(buf,"You attempt to %s $N, but an unseen force hold $m in place.",
		 capitalize(verb));
        act( buf, ch, NULL, victim, TO_CHAR );
	sprintf(buf,"$n attempts to %s $n, but fails as an unseen force hold $n in place.", capitalize(verb));
     act( buf,ch, NULL, victim, TO_ROOM );
        return;
    }
    ...
    if(!str_cmp(arg, "rp") )
     {
      ...
      sprintf( buf,"You have been deducted %d rptokens.\n\r",ch->level/5);
      send_to_char(buf, ch);
      do_look(ch,"auto");
      }
      else
   ...
   if(!str_cmp(arg, "glory") )
     {
      ...
      act( "$n returns from the dead.", ch, NULL, NULL, TO_ROOM );
      send_to_char( "You are brought back to life.\n\r", ch );
      sprintf( buf,"You have been deducted %d glorypointst.\n\r",ch->level*4);
      send_to_char(buf, ch);
      do_look(ch,"auto");
      }
      else
   ...
   if(!str_cmp(arg, "gold") )
     {
      ...
      act( "$n returns from the dead.", ch, NULL, NULL, TO_ROOM );
      send_to_char( "You are brought back to life.\n\r", ch );
      sprintf( buf,"%d gold has been subtracted from your account.\n\r",ch->level*250);
      send_to_char(buf, ch);
      do_look(ch,"auto");
      }
      else
    ...
    {
	send_to_char( "Your corpse still exists, someone may save you..\n\r", ch );
	return;
    }
