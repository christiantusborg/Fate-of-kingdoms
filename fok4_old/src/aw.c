   ...
   if ( (str_cmp( arg, "room" ) ) && (str_cmp( arg, "zone" ) ) && (str_cmp( arg, "all" ) ) )
    {
     if  ( (victim = get_char_world(ch, arg) ) == NULL )
      ...
      {
       if (get_trust(victim) >= get_trust(ch))
        sprintf( buf,"{W%s personal> {c%s{x\n\r",ch->name,argument);
       else
         sprintf( buf,"{c%s{x\n\r",argument);
         if (boolfont)
 ...
 ****************************************************************************/
{
    ...
    sprintf(buf,"'%s' nonvnum",string);
    string = str_dup(buf);
    sprintf(buf,"'%s' nonvnum",argument);
...
*/
    ...
    if ( value == 0 )
    {
     sprintf( buf,"%s have %d glorypoints.\n\r", victim->name,victim->quest_curr);
     send_to_char(buf, ch);
     return;
    }
    ...
    sprintf( buf,"You have bonused %s a whopping %d glorypoints.\n\r",
                victim->name, value);
                send_to_char(buf, ch);
    sprintf( buf,"%s  now have %d glory points.\n\r",
                victim->name, victim->quest_curr);
                send_to_char(buf, ch);

    if ( value > 0 )
    { 
      sprintf( buf,"You have been bonused %d glorypoints.\n\r",value );
      send_to_char( buf, victim );
    }
    else
    {
      sprintf( buf,"You have been penalized %d glorypoints.\n\r",value );
      send_to_char( buf, victim );
    }
...
void do_punish( CHAR_DATA *ch, char *argument )
 {
	...
	boolsussess = TRUE;
      ...
      if (boolsussess)
       {
        sprintf(buf,"Your %s priviliges is %s!.\n\r",arg,cmd);
        send_to_char( buf, victim );
        sprintf(buf,"You %s %ss %s priviliges.\n\r",cmd,victim->name,arg);
        send_to_char( buf, ch );
        sprintf(buf,"%s %s %ss %s priviliges.\n\r",cmd,ch->name,victim->name,arg);
        wiznet(buf,ch,NULL,WIZ_PENALTIES,WIZ_SECURE,0);
        return;
       }
      else
...
void do_balefire( CHAR_DATA *ch, char *argument)
{
   ...
   if (IS_NPC(victim))
    ...
    sprintf( strsave, "%s%s", PLAYER_DIR, capitalize( victim->name ) );
    stop_fighting(victim,TRUE);
    sprintf( buf,"all %s points down at %s{x!!",ch->name,victim->name);
	do_echo(ch,buf);
    sprintf( buf,"all {wSuddenly a {yg{Ylowin{yg {wd{Wivin{we {rb{Rea{rm{x of {wl{Wigh{wt{x descends from the {ch{Ceaven{cs{x and strikes %s.{x",victim->name);
	do_echo( ch,buf);
    sprintf( buf,"all {w%s suddenly {rb{Rurn{rs{x into {da{Dshe{ds{x and is no more.{x",victim->name);
	do_echo(ch,buf);
  ...
  if (!str_cmp( arg, "immtitle" ))
   {
    ...
    log_string("Title 1");
    sprintf( buf, "{CYou have been granted a %s by the gods:\n\r'%s'{x\n\r",arg,victim->immtitle );
   } 
...
/*  
  ...
  MOB_INDEX_DATA *pMob;
   ...
   sprintf( buf, "%d %d", ch->in_room->area->min_vnum,ch->in_room->area->max_vnum);
   send_to_char(buf,ch);
...
/*
   for (i = ch->in_room->area->min_vnum; i <= ch->in_room->area->max_vnum; i++)
	 ...
	 if ( (pObj = get_obj_index(i)) != NULL) 
	  {
	if IS_SET(pObj->class_flags,CLASS_MALIGNER)
	 {
...
/*
 ...
 send_to_char("{xOK. Objects to all Race and class falgs\n\r",ch);
*/
/*  
    i = 0;
 while (skill_table[i].name != "toxic")
    i++;
...
*/
       ...
       if (0 == atoi(arg2))
	{
         ...
         sprintf(buf,"You removed %s's WANTED flag.\n\r", victim->name);
         send_to_char(buf, ch );
         sprintf(buf,"%s removed %s's WANTED flag\n\r",ch->name, victim->name);
         victim->wantedtime =0;
         wiznet(buf,ch,NULL,WIZ_PENALTIES,WIZ_SECURE,0);
	} 
       else
	{
	 ...
	 sprintf(buf,"You changed %s wantedtime to %d\n\r", victim->name,victim->wantedtime);
	 send_to_char(buf, ch );
	 sprintf(buf,"Your wantedtime changed to %d\n\r",victim->wantedtime);
	 send_to_char( buf, victim );
	 sprintf(buf,"%s changed wantedtime to %d on %s\n\r",ch->name,victim->wantedtime, victim->name);
	 wiznet(buf,ch,NULL,WIZ_PENALTIES,WIZ_SECURE,0);
	}
    }
    else
	...
	victim->wantedtime=atoi(arg2);
        SET_BIT(victim->act,PLR_WANTED);
        sprintf(buf,"The gods have deemed you a criminal, for %d tick(s).\n\r",victim->wantedtime);
	send_to_char( buf, victim );
	sprintf(buf,"WANTED Set %s, for %d tick(s).\n\r", victim->name,victim->wantedtime);
	send_to_char(buf, ch );
	sprintf(buf,"%s makes %s wanted for %d tick(s).\n\r",ch->name,victim->name, victim->wantedtime);
	wiznet(buf,ch,NULL,WIZ_PENALTIES,WIZ_SECURE,0);
    }
...
//  int      value;
 ...
 if (argument[0] == '\0')
  ...
  reset_char(d.character);
   ...
   sprintf(buf,"Name:          %s",d.character->name);
