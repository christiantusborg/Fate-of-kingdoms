...
int get_thac(CHAR_DATA *ch, CHAR_DATA *victim,int dt,int skill)
{
	...
	thac += class_table[ch->class].thac0; //(1- 30)
...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
  ...
  if (IS_AFFECTED(ch,AFF_STUN) )
        {
 act_new("You are paralyzed and unable to attack.",
                ch,NULL,victim,TO_CHAR,POS_FIGHTING);
 act_new("$n is paralyzed and can't attack.",
...
/*  adds dual */
    ...
    if (get_eq_char (ch, WEAR_SECONDARY) != NULL && !IS_AFFECTED2(ch, AFF2_SUBMISSION) && !IS_AFFECTED2(ch, AFF2_ARMLOCK))
    {
...
void update_pos( CHAR_DATA *victim )
{
	...
	return;
         ...
         if ( victim->hit <= -1 ) victim->position = POS_MORTAL;
...
int xp_compute(CHAR_DATA *gch, CHAR_DATA *victim, int total_levels,int members)
{
  ...
  else if ( IS_GOOD(gch) && IS_EVIL(victim) )
    xp = base_exp * 2;
