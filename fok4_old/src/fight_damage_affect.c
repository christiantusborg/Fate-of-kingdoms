...
int affect_armor_damage(CHAR_DATA *victim,int dam_type,int dam,int body_part)
{
     switch(dam_type)
    {
    case(DAM_PIERCE):	dam -= (dam *(-GET_AC(victim,AC_PIERCE))/1500);	break;
    case(DAM_BASH):	dam -= (dam *(-GET_AC(victim,AC_BASH))/1500);	break;
    case(DAM_SLASH):	dam -= (dam * (-GET_AC(victim,AC_SLASH))/1500);	break;
    default:	dam -= (dam *(-GET_AC(victim,AC_EXOTIC))/1500);	break;
    }
return dam;
}
