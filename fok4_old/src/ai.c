...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
 ...
 //   if ( IS_AFFECTED(victim, AFF_IMPROVED_INVISIBLE)   ) strcat( buf, "{m(Invis){B " );
    ...
    if ( IS_AFFECTED2(victim, AFF2_CHARISMATIC)   ) strcat( buf, "{M(Charismatic){B ");
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if (victim->pcdata->pkills > 1)
     {
  	 sprintf(buf,"{W(*-*){C%s has %d old bones attached to your belt.{W(*-*){x\n\r",victim->sex == 0 ? "It" : victim->sex == 1 ? "He" : "She",
			...
			if ( fAuto )
			  {
				...
				if (!IS_SET(pexit->exit_info, EX_HIDDEN) && !IS_IMMORTAL(ch))
					if (!IS_SET(pexit->exit_info, EX_CLOSED) && !IS_SET(pexit->exit_info, EX_HIDDEN) )
						sprintf(buf," %s%s",doormark,dir_name[door]);
					else
			...
			else
			  {
				...
				if (!IS_SET(pexit->exit_info, EX_HIDDEN) && !IS_IMMORTAL(ch))
					if (!IS_SET(pexit->exit_info, EX_CLOSED) && !IS_SET(pexit->exit_info, EX_HIDDEN) )
						sprintf( buf + strlen(buf), "%-5s - %s",capitalize( dir_name[door] ),room_is_dark( pexit->u1.to_room ,NULL) ?  "Too dark to tell" : pexit->u1.to_room->name);
					else
						sprintf( buf + strlen(buf), "%s%-5s - %s",doormark,capitalize( dir_name[door] ),room_is_dark( pexit->u1.to_room ,NULL) ?  "Too dark to tell" : pexit->u1.to_room->name);
			  }
...
send_to_char("{B+-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=+{x\n\r",ch);
    ...
    if ((ch->wantedtime > 0) && !IS_NPC(ch) && IS_SET(ch->act,PLR_WANTED))
     {
      sprintf(buf,"You will be considered a criminal for another %d ticks.\n\r",ch->wantedtime);
      send_to_char( buf, ch );
     }
...
if (!is_number( argument ))
 {
    ...
    sprintf( buf, " Help file not found\n\r"
                  "{C-------------------------------------------------------{X\n\r");
...
//	char const *Txtclass;
	 ...
	 */
      ...
      sprintf( buf, "{c[%-22s{c]{m %s%s%s%s%s%s%s%s%s%s{x\n\r",
        wch->immtitle,
...
if (IS_TRUSTED(ch,(MAX_LEVEL - 14 )) || ch==wch ||
                   wch->level >= LEVEL_HERO)
	  sprintf( buf, "{W[{W%3d %s {G%s{W]{x %s%s%s%s%s%s%s%s%s{m%s%s{x\n\r",
	    wch->level,
	  ...
	  sprintf( buf, "{W[    %s{W] {x%s%s%s%s%s%s%s%s%s{m%s%s{x\n\r",
	    wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
	    kingdombuf,
	    	    IS_SET(wch->tag_flags,TAG_RED) ? "{R[RED] {x" :
...
if (IS_TRUSTED(ch,(MAX_LEVEL - 14 )) || ch==wch || wch->level >= LEVEL_HERO)
	  sprintf( buf, "{W[%3d %s {G%s{W]{x %s%s%s%s%s%s%s%s%s{m%s%s{x\n\r",
	    wch->level,
	    wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
	    class,
...
void do_equipment( CHAR_DATA *ch, char *argument )
{
	...
	if (ch->race == race_lookup("ogre") && !IS_NPC(ch))
     {
  	 sprintf(buf,"{W(*-*){CYou have %d old bones attached to your belt.{W(*-*){x\n\r",ch->pcdata->pkills);
   	 send_to_char(buf,ch);
     }
