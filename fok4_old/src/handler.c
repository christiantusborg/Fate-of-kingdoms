...
char *affect_bit_name( int vector )
{
 ...
 //   if ( vector & AFF_IMPROVED_INVISIBLE     ) strcat( buf, "improved_invisible"     );
...
char *detect_bit_name( int vector )
{
    ...
    if ( vector & DETECT_UNDEAD ) strcat( buf, " detect_undead"  );
...
char *class_bit_name( int class_flags )
{
    ...
    if ( class_flags & CLASS_NECROMANCER       ) strcat( buf, " necromancer");
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *off_bit_name(int off_flags)
{
    ...
    if (off_flags & OFF_XATTACK		) strcat(buf, " xattack");
    if (off_flags & OFF_XATTACK2	) strcat(buf, " xattack2");
    if (off_flags & OFF_XATTACK3	) strcat(buf, " xattack3");
    if (off_flags & OFF_XATTACK4	) strcat(buf, " xattack4");
    if (off_flags & OFF_XATTACK5	) strcat(buf, " xattack5");
    if (off_flags & OFF_XATTACK6	) strcat(buf, " xattack6");
...
//    if ( vector & AFF_ROOM_FEAR      ) strcat( buf, " fear"            );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
