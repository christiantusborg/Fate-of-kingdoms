...
{   
    ...
    extract_obj(obj);
    sprintf(buf1, "%s labeled '%s'", potion->short_descr, skill_table[potion->value[1]].name);
    free_string(potion->short_descr);
    potion->short_descr = str_dup(buf1);
    sprintf(buf2, "%s %s", potion->name, skill_table[potion->value[1]].name);
