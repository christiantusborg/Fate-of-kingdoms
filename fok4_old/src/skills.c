...
void do_shieldbash( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_AFFECTED2(ch, AFF2_ARMLOCK))
	{
...
void do_breath( CHAR_DATA *ch, char *argument )
{
    ...
    dam += number_range(10, ch->level * 5);
...
void do_submission( CHAR_DATA *ch)
{
    ...
    if ( IS_AFFECTED2(victim, AFF2_ARMLOCK) && IS_AFFECTED2(ch, AFF2_SUBMISSION))
       {
    ...
    if ( IS_AFFECTED2(victim, AFF2_ARMLOCK))
	...
	if (victim->lagspell   < 9)
	  ...
	  SET_BIT(ch->affected_by2, AFF2_SUBMISSION);
	  SET_BIT(victim->affected_by2, AFF2_ARMLOCK);
    }
    else
    {
    ...
    if (!(IS_NPC(victim)) && !(IS_NPC(ch)) && victim->position > POS_STUNNED)
      {
	if (!can_see(victim, ch))
	do_yell(victim," Help! Someone is trying to break my arm!");
	else
	  {
	    sprintf(buf, "Help! is %s attempting to break my arm!{x",
		(is_affected(ch,gsn_doppelganger) && !IS_IMMORTAL(victim)) ?
		ch->doppel->name : ch->name);
	do_yell(victim,buf);
	  }
