	...
	if ( IS_AFFECTED2(victim, AFF2_INVUL))
	  {
		...
		sprintf(buf,"{cYour aura of invulnerability absorbs %s's attack!{x\n\r",PERS(victim,ch));
...
bool check_cheat_damage(CHAR_DATA *ch,int dam)
{
  ...
  char buf[MAX_STRING_LENGTH];
    ...
    if ( dam > 3000 && !IS_IMMORTAL(ch) && !IS_NPC(ch))
    {
	bug( "Damage: %d: more than 3000 points!{x", dam );
	dam = 1500;
...
bool check_block( CHAR_DATA *ch, CHAR_DATA *victim )
{
  ...
  if (IS_AFFECTED2(ch, AFF2_SUBMISSION) || IS_AFFECTED2(ch, AFF2_ARMLOCK))
      return FALSE;
