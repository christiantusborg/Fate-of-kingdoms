        ...
        if (!IS_NPC(ch) && victim != ch &&
            ...
            (IS_SET(victim->affected_by,AFF_CHARM) || !IS_NPC(victim)))
          {
            if (!can_see(victim, ch))
              ...
              {
                 sprintf(buf,"Die, %s, you danceing fool!",
...
//    AFFECT_DATA *paf;
	...
	extract_obj(obj);
	damage( ch, ch, dam, TYPE_HIT, DAM_FIRE, FALSE);
	return;
    }
...
//    EXTRA_DESCR_DATA *ed_next;
       ...
       ||!strcmp(color,"acid"))
    {
        ...
        else
        {
            ...
            {
                sprintf(buf1, "an %s rose", color);
            } 
            else sprintf(buf1, "a %s rose", color);
            sprintf(buf2, "A %s rose is lying on the ground.", color);
            sprintf(buf3, "It's the perfect example of %s.\n",buf1);
        }
        ...
        if(tag[0]!='\0')
            sprintf(buf1, "%s with a tag that reads: '%s'",buf1,tag);
