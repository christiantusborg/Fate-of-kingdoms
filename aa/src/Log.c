...
void Inet(CHAR_DATA *ch, char *LogLine,int Kingdom,int Inet)
{
	...
	if (Inet & INET_ARENA)
	  {
		xprintf(buffer,"{R[{YARENA{R]: %s",LogLine);
...
void Log_system (CHAR_DATA *ch, char *LogLine,int Log,int Kingdom,int InetFlag)
{
	...
	if (ch != NULL)
		xprintf(LogNameAndTime,"%s :[%s]: %s\n", strtime,ch->name,LogLine);
	else
		xprintf(LogNameAndTime,"%s :[System]: %s\n",strtime,LogLine);
	...
	if (Log & LOG_ROLEPLAY_SAY)
	  {
		xprintf( LogFile, "../../../logs/say/%s.log",buffer);		
	  }
	if (Log & LOG_ROLEPLAY_GOSSIP)
	  {
//		xprintf( buffer, "%s%F",ch->name,time);
		xprintf( LogFile, "../../../logs/gossip/%s.log",buffer);
	  }

	if (Log & LOG_BUG)
	  {
	    xprintf( LogFile, "../../../logs/bug/%s.log",buffer);
	  }

	if (Log & LOG_DEATH)
	  {
	    xprintf( LogFile, "../../../logs/death/%s.log",buffer);
	  }
...
/* Toggle an option! */
    ...
    if (IS_SET(ch->info,info_table[flag].flag))
    {
        xprintf(buf,"You will no longer see %s on the info channel.\n\r",
    ...
    {
        xprintf(buf,"You will now see %s on the info channel.\n\r",
                        info_table[flag].name);
        send_to_char(buf,ch);
