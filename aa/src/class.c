...
void save_web (int num)
{
	FILE *fp;
	char buf[MAX_STRING_LENGTH];
	int lev, i;
	
	xprintf (buf, "%s%s.html", WEB_CLASS_FILES, class_table[num].name);
	if (!(fp = fopen (buf, "w")))
	{
		return;
	}
  ...
  fprintf (fp, "%s<br>", class_table[num].name);
	for (lev = 0; lev < LEVEL_IMMORTAL; lev++)
		fprintf (fp, "<h2>%d</h2>", lev);
		for (i = 0; i < MAX_SKILL; i++)
		{
			if (!skill_table[i].name || !skill_table[i].name[0])
				continue;
			...
			if (skill_table[i].skill_level[num] == lev)
				fprintf (fp, "%s<br>",skill_table[i].name);
		}
