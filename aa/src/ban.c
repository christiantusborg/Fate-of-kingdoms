...
bool check_notallow(char *site,char *bansite)
{
    char host[MAX_STRING_LENGTH];
	...
	if(strstr(bansite,host) != NULL)
            return TRUE;

	if(!str_suffix(bansite,host))
	    return TRUE;

	if(!str_prefix(bansite,host))
	    return TRUE;
