	...
	if (Dir->d_name[0] >= 'A' && Dir->d_name[0] <= 'Z') {
	  ...
	  // d->character->login was set equal to lastlogoff ("LogO" in save.c)
 	  if ( d->character->level < LEVEL_HERO &&
	      ...
	      {
               sprintf( buf, "Autodelete: %s",FName);
	       log_string(buf);
               sprintf( strsave, "%s%s", PLAYER_DIR, capitalize( FName ) );
               sprintf(pfile,"/home/fate/.ftpbak/ss/Backup/autodelete/%s.bak",capitalize( FName ) );
...
/*  board code added, in right place? do pfiles have to be ordered? */
	...
	{
	    if ( skill_table[sn].abiliti_name != NULL && ch->pcdata->learned[sn] > 0 )
	    {
		fprintf( fp, "Sk %d '%s'\n",
		    ch->pcdata->learned[sn], skill_table[sn].abiliti_name );
	    }
	}
/* : quest code added */
    if (ch->questpoints != 0)
	...
	fprintf( fp, "Affc '%s' %3d %3d %3d %3d %3d %10d\n",
	    skill_table[paf->type].abiliti_name,
	    paf->where,
	    paf->level,
	    paf->duration,
	    paf->modifier,
...
//       fprintf( fp, "LL   %d\n",       obj->limlevel );
	...
	if ( obj->value[1] > 0 )
	{
		...
		skill_table[obj->value[1]].abiliti_name );
	}

	if ( obj->value[2] > 0 )
	{
	    fprintf( fp, "Spell 2 '%s'\n",
		skill_table[obj->value[2]].abiliti_name );
	}
