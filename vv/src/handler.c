...
int lookup_deity(const char *name) 
{
 int deity = 0;

 for (deity = 0;religion_table[deity].name != NULL;deity++)
  {
   if (!str_prefix(name,religion_table[deity].name))
    break;
  }

if (religion_table[deity].name != NULL)  
  return deity;
...
int lookup_Bloodline(const char *name)
{
 int bloodcount = 0;

 for (bloodcount = 0;bloodline_table[bloodcount].name != NULL;bloodcount++)
  {
   if (!str_prefix(name,bloodline_table[bloodcount].name))
    break;
  }
return bloodcount;
}
...
int apply_ac( OBJ_DATA *obj, int iWear, int type )
{
    ...
    case WEAR_HANDS_1:	return     obj->value[type];
    case WEAR_HANDS_2:	return     obj->value[type];
    case WEAR_ARMS_1:	return     obj->value[type];
    case WEAR_ARMS_2:	return     obj->value[type];
    ...
    case WEAR_SADDLE:	return     obj->value[type];
    case WEAR_HOOF:	return     obj->value[type];
    case WEAR_TAIL:	return     obj->value[type];
    case WEAR_HORN:	return     obj->value[type];
    case WEAR_WAIST:	return     obj->value[type];
    case WEAR_WRIST_1:	return     obj->value[type];
    case WEAR_WRIST_2:	return     obj->value[type];
    case WEAR_WRIST_3:	return     obj->value[type];
    case WEAR_WRIST_4:	return     obj->value[type];
...
char *wear_bit_name(int wear_flags)
{
    ...
    if (wear_flags & ITEM_WEAR_SADDLE	) strcat(buf, " back");
    if (wear_flags & ITEM_WEAR_HOOF	) strcat(buf, " hooves");
    if (wear_flags & ITEM_WEAR_TAILPIECE	) strcat(buf, " tail");
    if (wear_flags & ITEM_WEAR_HORN	) strcat(buf, " horns");
