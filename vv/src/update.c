...
void advance_level( CHAR_DATA *ch )
{
   ...
   add_hp   = dice(1,pc_race_table[ch->race].hpdice) + dice(1,class_table[ch->class].hpdice);
   add_mana = dice(1,pc_race_table[ch->race].manadice) + dice(1,class_table[ch->class].manadice);
   add_move = dice(1,pc_race_table[ch->race].movedice) + dice(1,class_table[ch->class].movedice);
    ...
    sprintf( buf,"{WYour gain is: {G%d{W/{g%d {Whp, {M%d{W/{m%d {Wm, {Y%d{W/{Y%d {Wmv {B%d{W/{b%d {Wprac.{x\n\r",
...
/*  removes the above just because it is...DUMB! */
 ...
 if ( !IS_SET ( ch->in_room->room_flags, ROOM_ARENA ) ) {
	...
	for ( paf = ch->affected; paf != NULL; paf = paf_next )
	{
		...
		||   paf_next->duration > 0 )
		{
		    if ( paf->type > 0 && skill_table[paf->type].abiliti_msg_off )
		    {
			send_to_char( "{g", ch );
			send_to_char( skill_table[paf->type].abiliti_msg_off, ch );
			send_to_char( "{x\n\r", ch );
		    }
		}
	...
	/* go through affects and decrement */
        for ( paf = obj->affected; paf != NULL; paf = paf_next )
        {
            ...
            else
            {
                ...
                ||   paf_next->duration > 0 )
                {
                    if ( paf->type > 0 && skill_table[paf->type].abiliti_msg_obj )
                    {
			if (obj->carried_by != NULL)
			{
			    rch = obj->carried_by;
			    act(skill_table[paf->type].abiliti_msg_obj,
