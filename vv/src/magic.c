...
int skill_lookup( const char *name )
{
	...
	if ( skill_table[sn].abiliti_name == NULL )
	    break;
	if ( LOWER(name[0]) == LOWER(skill_table[sn].abiliti_name[0])
	&&   !str_prefix( name, skill_table[sn].abiliti_name ) )
	    return sn;
    }
...
/*        if(skill_table[sn].name == "reserved")
	    break; */
//        send_to_char("C",ch);
        sprintf(buf, "Find Spell %s,%s!\n\r",skill_table[sn].abiliti_name,name);
...
int slot_lookup( int slot )
{
    ...
    for ( sn = 0; sn < MAX_SKILL; sn++ )
    {
	if ( slot == skill_table[sn].abiliti_slot )
	    return sn;
    }
...
void say_spell( CHAR_DATA *ch, int sn )
{
    ...
    buf[0]	= '\0';
    for ( pName = skill_table[sn].abiliti_name; *pName != '\0'; pName += length )
    {
...
/*  :: add switch statement and all classes */
switch (ch->class)
{ 
case CLASS_MAG:
sprintf( buf2, "{g$n waves $s hands fancifully and utters, '%s'.{x", buf);
sprintf( buf,  "{g$n waves $s hands fancifully and utters, '%s'.{x",skill_table[sn].abiliti_name);
send_to_char("{cWith a fanciful wave of hands your magic is cast.{x",ch);
	break;
case CLASS_MAL:
sprintf( buf2, "{g$n's eyes flash red as $e utters, '%s'.{x", buf);
sprintf( buf,  "{g$n's eyes flash red as $e utters, '%s'.{x",skill_table[sn].abiliti_name);
send_to_char("{cWith a flash of red in your eyes, your magic is cast.{x",ch);
	break;
case CLASS_CLE: 
sprintf( buf2, "{g$n breathes deeply as $e closes $s eyes in communion.{x"); 
sprintf( buf,  "{g$n breathes deeply as $e closes $s eyes in communion.{x"); 
send_to_char("{cYou commune with the divine for your powers...{x",ch);
	break;
case CLASS_PDN:
sprintf( buf2, "{g$n closes $s eyes for a moment and nods solemnly.{x");
sprintf( buf,  "{g$n closes $s eyes for a moment and nods solemnly.{x");
send_to_char("{cYou commune with the divine for your powers...{x",ch);
	break;
case CLASS_BKT:
sprintf( buf2, "{g$n utters fiercely, '%s'.{x", buf);
sprintf( buf,  "{g$n utters fiercely, '%s'.{x", skill_table[sn].abiliti_name);
send_to_char("{cYou fiercely utter some arcane words.{x" ,ch);
	break;
case CLASS_PRO:
sprintf( buf2, "{g$n fixes $s eyes in absolute concentration for a split second.{x" );
sprintf( buf,  "{g$n fixes $s eyes in absolute concentration for a split second.{x" );
send_to_char("{cYou bring your mind to absolute focus.{x",ch);
	break;
case CLASS_BRD:
sprintf( buf2, "{g$n clears $s throat.{x" );
sprintf( buf,  "{g$n clears $s throat.{x" );
send_to_char("{cYou clear your throat in preparation...{x" ,ch);
	break;
case CLASS_ALC:
sprintf( buf2, "{g$n traces symbols in the air with $s fingers.{x");
sprintf( buf,  "{g$n traces symbols in the air with $s fingers.{x");
send_to_char("{cYou trace symbols of the elements in the air with your finger.{x" ,ch);
	break;
case CLASS_SPE:
sprintf( buf2, "{g$n crudely utters the words, '%s'.{x", buf);
sprintf( buf,  "{g$n crudely utters the words, '%s'.{x", skill_table[sn].abiliti_name);
send_to_char("{cYou try your best to cast the magics{x",ch);
	break;
case CLASS_INQ:
sprintf( buf2, "{g$n utters with an eerie fervor, '%s'.{x", buf);
sprintf( buf,  "{g$n utters with an eerie fervor, '%s'.{x", skill_table[sn].abiliti_name);
send_to_char("{cYou commune with the fire burning within.{x",ch);
	break;
case CLASS_ASN:
sprintf( buf2, "{g$n waves $s hand deftly as $e utters, '%s'.{x", buf);
sprintf( buf,  "{g$n waves $s hand deftly as $e utters, '%s'.{x", skill_table[sn].abiliti_name);
send_to_char("{cYou wave your hand deftly as you bring forth dark magics.{x",ch);
	break;
case CLASS_RAN:
sprintf( buf2, "{g$n carefully utters the words, '%s'.{x", buf );
sprintf( buf,  "{g$n carefully utters the words, '%s'.{x", skill_table[sn].abiliti_name );
send_to_char("{cWith thoughts of nature, you cast your magic.{x" ,ch);
	break;
case CLASS_THI:
case CLASS_MER:
case CLASS_WAR:
sprintf( buf2, "{g$n barely pronounces the words, '%s'.{x", buf );
sprintf( buf,  "{g$n barely pronounces the words, '%s'.{x", skill_table[sn].abiliti_name );
send_to_char("{cYou barely manage to pronounce the words.{x" ,ch);
	break;
default:
sprintf( buf2, "{g$n utters the words, '%s'.{x", buf );
sprintf( buf,  "{g$n utters the words, '%s'.{x", skill_table[sn].abiliti_name );
send_to_char("{cYou utter the arcane words.{x",ch);
	break;
}
/*  adds final linefeed, oops forget them up there */
send_to_char("\n\r",ch);
	...
	if ( rch != ch )  {
	    act( 
(get_skill(rch,gsn_spell_craft)*0.9)/2 <number_percent() ? 
		buf2 : buf, ch, NULL, rch, TO_VICT );
	...
	for ( af = victim->affected; af != NULL; af = af->next )
        {
            if ( af->type == sn )
            {
        	    ...
        	    if ( skill_table[sn].abiliti_msg_off )
        	    {
            		send_to_char( skill_table[sn].abiliti_msg_off, victim );
            		send_to_char( "\n\r", victim );
        	    }
		    return TRUE;
		}
...
//    sprintf(buf, "Find Spell %d,%s!\n\r",sn,arg1);
//    send_to_char(buf,ch);
//    sprintf(buf, "%d,level %d, level for class %d \n\r",ch->pcdata->learned[sn],ch->level,skill_table[sn].skill_level[ch->class]);
//    send_to_char(buf,ch); 
     
    if ( ( sn = find_spell( ch,arg1 ) ) < 0
    || ( !IS_NPC(ch) && (ch->level < skill_table[sn].abiliti_level[ch->class]
    ||   		 ch->pcdata->learned[sn] == 0)))
    {
...
*/
    ...
    if ( ch->position < skill_table[sn].abiliti_minimum_position )
    {
    ...
    if (ch->level + 2 == skill_table[sn].abiliti_level[ch->class])
	mana = 50;
    else
	mana = UMAX(
	    skill_table[sn].abiliti_manacost,
	    100 / ( 2 + ch->level - skill_table[sn].abiliti_level[ch->class] ) );
...
//	    if ( ( victim = get_char_room( ch, target_name ) ) == NULL )
	...
	if ( !IS_NPC(ch) )
	{
	    ...
	    {
		send_to_char("This room is protected with a magical aura.\n\r",ch);
		return;
	    }
	    if ( is_safe(ch, victim) )
		return;
...
//	else if ((victim = get_char_room(ch,target_name)) != NULL)
    ...
    if ( str_cmp( skill_table[sn].abiliti_name, "ventriloquate" ) )
	say_spell( ch, sn );

    WAIT_STATE( ch, skill_table[sn].abiliti_beats[1] );
...
/*	else And i dont like it give to much troble
    ...
    if ((skill_table[sn].abiliti_target == TAR_CHAR_OFFENSIVE
    ||   (skill_table[sn].abiliti_target == TAR_OBJ_CHAR_OFF && target == TARGET_CHAR))
    &&   victim != ch
    &&   victim->master != ch)
    {
	CHAR_DATA *vch;
...
*/
    ...
    if ( ( sn = find_spell( ch,arg1 ) ) < 0
    || ( !IS_NPC(ch) && (ch->level <
skill_table[sn].abiliti_level[ch->class]
    ||   		 ch->pcdata->learned[sn] == 0)))
    {
        ...
        if (!IS_NPC(ch) && victim != ch &&
            ...
            (IS_SET(victim->affected_by,AFF_CHARM) || !IS_NPC(victim)))
          {
            if (!can_see(victim, ch))
              ...
              {
                 sprintf(buf,"Die, %s, you singing fool!",
...
vo,target);
    ...
    if ((skill_table[sn].abiliti_target == TAR_CHAR_OFFENSIVE
    ||   (skill_table[sn].abiliti_target == TAR_OBJ_CHAR_OFF && target ==
TARGET_CHAR))
    &&   victim != ch
    &&   victim->master != ch)
    {
...
TO_ROOM);
	...
	mana = UMAX(
	    skill_table[sn].abiliti_manacost,100 /( 2 + ch->level - skill_table[sn].abiliti_level[ch->class]
) );
...
deflected */
					   /* and scourge... */
          {
            if (ch==victim)
              {
                act("Your spellbane protects you!",ch,NULL,NULL,TO_CHAR);
                act("$n's spellbane protects your from
t!",ch,NULL,NULL,TO_ROOM);
//                damage(ch,ch,10 * ch->level,gsn_spellbane,DAM_NEGATIVE,TRUE);
              }
            else {
              act("$N deflects your divine
power!",ch,NULL,victim,TO_CHAR);
              act("You deflect $n's divine
power!",ch,NULL,victim,TO_VICT);
              act("$N deflects $n's divine
power!",ch,NULL,victim,TO_NOTVICT);
              damage(victim,ch,10 *
victim->level,gsn_spellbane,DAM_NEGATIVE, TRUE);
              multi_hit(victim,ch,TYPE_UNDEFINED);
            }
...
void spell_sleep( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
   ...
   if (ch->class == 13)
    ...
    ||   saves_spell( level-8, victim,DAM_CHARM) )
	return;
...
void spell_ventriloquate( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
    ...
    sprintf( buf1, "{g%s says '{G%s{g'.{x\n\r",              speaker, target_name );
    sprintf( buf2, "{gSomeone makes %s say '{G%s{g'.{x\n\r", speaker, target_name );
    buf1[0] = UPPER(buf1[0]);
