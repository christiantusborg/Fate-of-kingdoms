...
void load_mobiles( FILE *fp )
{
    ...
    for ( ; ; )
    {
        ...
        if ( letter != '#' )
        {
            bug( "Load_mobiles: # not found. (db2.c)", 0 );
            exit( 1 );
        }
	...
	pMobIndex->race		 	= race_lookup(fread_string( fp ));
        ...
        pMobIndex->detection            = fread_flag( fp )
					| race_table[pMobIndex->race].detect;
/*
...
/*  reads in old manadice for the hell of it and replaces below */
/* 	recalc manadice   */
	pMobIndex->mana[DICE_NUMBER]	= 1;
	pMobIndex->mana[DICE_TYPE]	= pMobIndex->level;
	pMobIndex->mana[DICE_BONUS]	= pMobIndex->level*20;
...
/*  :: old ac values read in, will be replaced below */
	pMobIndex->ac[AC_PIERCE]	= fread_number( fp ) * 10;
	pMobIndex->ac[AC_BASH]		= fread_number( fp ) * 10;
	pMobIndex->ac[AC_SLASH]		= fread_number( fp ) * 10;
	pMobIndex->ac[AC_EXOTIC]	= fread_number( fp ) * 10;
/*  :: autocalc mob ac, fread past old ones */
	...
	pMobIndex->parts		= fread_flag( fp )
        ...
        pMobIndex->resistance[0] =fread_number( fp );
        pMobIndex->resistance[1] =fread_number( fp );
        pMobIndex->resistance[2] =fread_number( fp );
        pMobIndex->resistance[3] =fread_number( fp );
        pMobIndex->resistance[4] =fread_number( fp );
        pMobIndex->resistance[5] =fread_number( fp );
        pMobIndex->resistance[6] =fread_number( fp );
        pMobIndex->resistance[7] =fread_number( fp );
        pMobIndex->resistance[8] =fread_number( fp );
        pMobIndex->resistance[9] =fread_number( fp );
        pMobIndex->resistance[10] =fread_number( fp );
        pMobIndex->resistance[11] =fread_number( fp );
        pMobIndex->resistance[12] =fread_number( fp );
        pMobIndex->resistance[13] =fread_number( fp );
        pMobIndex->resistance[14] =fread_number( fp );
        pMobIndex->resistance[15] =fread_number( fp );
        pMobIndex->resistance[16] =fread_number( fp );
        pMobIndex->resistance[17] =fread_number( fp );
        pMobIndex->resistance[18] =fread_number( fp );
        pMobIndex->resistance[19] =fread_number( fp );
        pMobIndex->resistance[20] =fread_number( fp );
        pMobIndex->resistance[21] =fread_number( fp );
        pMobIndex->resistance[22] =fread_number( fp );
  
        pMobIndex->immuniti[0] =fread_number( fp );
        pMobIndex->immuniti[1] =fread_number( fp );
        pMobIndex->immuniti[2] =fread_number( fp );
        pMobIndex->immuniti[3] =fread_number( fp );
        pMobIndex->immuniti[4] =fread_number( fp );
        pMobIndex->immuniti[5] =fread_number( fp );
        pMobIndex->immuniti[6] =fread_number( fp );
        pMobIndex->immuniti[7] =fread_number( fp );
        pMobIndex->immuniti[8] =fread_number( fp );
        pMobIndex->immuniti[9] =fread_number( fp );
        pMobIndex->immuniti[10] =fread_number( fp );
        pMobIndex->immuniti[11] =fread_number( fp );
        pMobIndex->immuniti[12] =fread_number( fp );
        pMobIndex->immuniti[13] =fread_number( fp );
        pMobIndex->immuniti[14] =fread_number( fp );
        pMobIndex->immuniti[15] =fread_number( fp );
        pMobIndex->immuniti[16] =fread_number( fp );
        pMobIndex->immuniti[17] =fread_number( fp );
      pMobIndex->immuniti[18] =fread_number( fp );
        pMobIndex->immuniti[19] =fread_number( fp );
        pMobIndex->immuniti[20] =fread_number( fp );
        pMobIndex->immuniti[21] =fread_number( fp );
        pMobIndex->immuniti[22] =fread_number( fp );


        CHECK_POS( pMobIndex->size, size_lookup(fread_word(fp)), "size" );
        pMobIndex->nrattack             = fread_number( fp );
/*       pMobIndex->size                 = size_lookup(fread_word(fp)); */
        pMobIndex->material		= str_dup(fread_word( fp ));
	pMobIndex->moprogs		= NULL;
	...
	for ( ; ; )
		...
		else if (!str_prefix(word,"detect"))
...
void load_objects( FILE *fp )
{
    ...
    for ( ; ; )
    {
        ...
        if ( letter != '#' )
        {
            bug( "Load_objects: # not found. (db2.c)", 0 );
            exit( 1 );
        }
	...
	case ITEM_TRAP:
	    ...
	    pObjIndex->value[1]		= fread_number(fp);
	    pObjIndex->value[2]		= fread_number(fp);
	    pObjIndex->value[3]		= fread_number(fp); /* max charge */
	    pObjIndex->value[4]		= fread_number(fp);	
	    pObjIndex->value[2]		= pObjIndex->value[2]; /*charge left*/
	    break;
	case ITEM_CONTAINER:
	...
	case ITEM_FOUNTAIN:
	    ...
	    {
		bug("Load_objects: Unknown liquid type, vnum %d", vnum);
		pObjIndex->value[2] = 0;
	    }
            pObjIndex->value[3]         = fread_number(fp);
            pObjIndex->value[4]         = fread_number(fp);
...
/*
	...
	switch (letter)
 	{
	    case ('P') :		pObjIndex->repop = 100; break;
	    case ('G') :		pObjIndex->repop =  90; break;
	    case ('A') :		pObjIndex->repop =  75; break;
	    case ('W') :		pObjIndex->repop =  50; break;
	    case ('D') :		pObjIndex->repop =  25; break;
	    case ('B') :		pObjIndex->repop =  10; break;
	    case ('R') :		pObjIndex->repop =   0; break;
	    default:			pObjIndex->repop = 100; break;
	}
*/
	pObjIndex->condition     = fread_number( fp );
        pObjIndex->repop         = fread_number( fp );
        pObjIndex->crepop        =fread_number( fp );
        pObjIndex->size          =fread_number( fp );
        pObjIndex->resistance[0] =fread_number( fp );
        pObjIndex->resistance[1] =fread_number( fp );
        pObjIndex->resistance[2] =fread_number( fp );
        pObjIndex->resistance[3] =fread_number( fp );
        pObjIndex->resistance[4] =fread_number( fp );
        pObjIndex->resistance[5] =fread_number( fp );
        pObjIndex->resistance[6] =fread_number( fp );
        pObjIndex->resistance[7] =fread_number( fp );
        pObjIndex->resistance[8] =fread_number( fp );
        pObjIndex->resistance[9] =fread_number( fp );
        pObjIndex->resistance[10] =fread_number( fp );
        pObjIndex->resistance[11] =fread_number( fp );
        pObjIndex->resistance[12] =fread_number( fp );
        pObjIndex->resistance[13] =fread_number( fp );
        pObjIndex->resistance[14] =fread_number( fp );
        pObjIndex->resistance[15] =fread_number( fp );
        pObjIndex->resistance[16] =fread_number( fp );
        pObjIndex->resistance[17] =fread_number( fp );
        pObjIndex->resistance[18] =fread_number( fp );
        pObjIndex->resistance[19] =fread_number( fp );
        pObjIndex->resistance[20] =fread_number( fp );
        pObjIndex->resistance[21] =fread_number( fp );
        pObjIndex->resistance[22] =fread_number( fp );

        pObjIndex->immuniti[0] =fread_number( fp );
        pObjIndex->immuniti[1] =fread_number( fp );
        pObjIndex->immuniti[2] =fread_number( fp );
        pObjIndex->immuniti[3] =fread_number( fp );
        pObjIndex->immuniti[4] =fread_number( fp );
        pObjIndex->immuniti[5] =fread_number( fp );
        pObjIndex->immuniti[6] =fread_number( fp );
        pObjIndex->immuniti[7] =fread_number( fp );
        pObjIndex->immuniti[8] =fread_number( fp );
        pObjIndex->immuniti[9] =fread_number( fp );
        pObjIndex->immuniti[10] =fread_number( fp );
        pObjIndex->immuniti[11] =fread_number( fp );
        pObjIndex->immuniti[12] =fread_number( fp );
        pObjIndex->immuniti[13] =fread_number( fp );
        pObjIndex->immuniti[14] =fread_number( fp );
        pObjIndex->immuniti[15] =fread_number( fp );
        pObjIndex->immuniti[16] =fread_number( fp );
        pObjIndex->immuniti[17] =fread_number( fp );
        pObjIndex->immuniti[18] =fread_number( fp );
        pObjIndex->immuniti[19] =fread_number( fp );
        pObjIndex->immuniti[20] =fread_number( fp );
        pObjIndex->immuniti[21] =fread_number( fp );
        pObjIndex->immuniti[22] =fread_number( fp );
...
void load_improgs( FILE *fp )
{
  ...
  char progname[MAX_INPUT_LENGTH];
	...
	case 'M':
	    ...
	    if (pMobIndex->moprogs == NULL)
	      pMobIndex->moprogs = alloc_perm(sizeof(MPROG_DATA));
