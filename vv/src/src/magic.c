...
sprintf( buf,  "{g$n waves $s hands fancifully and utters, '%s'.{x",skill_table[sn].name);
send_to_char("{cWith a fanciful wave of hands your magic is cast.{x",ch);
	break;
case CLASS_MAL:
sprintf( buf2, "{g$n's eyes flash red as $e utters, '%s'.{x", buf);
sprintf( buf,  "{g$n's eyes flash red as $e utters, '%s'.{x",skill_table[sn].name);
...
sprintf( buf,  "{g$n utters fiercely, '%s'.{x", skill_table[sn].name);
...
sprintf( buf,  "{g$n carefully utters the words, '%s'.{x", skill_table[sn].name );
...
sprintf( buf,  "{g$n barely pronounces the words, '%s'.{x", skill_table[sn].name );
send_to_char("{cYou barely manage to pronounce the words.{x" ,ch);
	break;
default:
sprintf( buf2, "{g$n utters the words, '%s'.{x", buf );
sprintf( buf,  "{g$n utters the words, '%s'.{x", skill_table[sn].name );
...
send_to_char("\n\r",ch);
	...
	if ( rch != ch )  {
	    act( 
(get_skill(rch,gsn_spell_craft)*0.9)/(class_table[rch->class].fMana?1:2)<number_percent() ? 
