...
// int i;
 ...
 if (victim->kingdom != 0) {
   ...
   subtract_member(victim->name,victim->kingdom);
	...
	else if(!str_prefix(arg2,"gondor"))  { number=KINGDOM_GONDOR; }
	else if(!str_prefix(arg2,"aldon"))    { number=KINGDOM_ALDON; }
	else if(!str_prefix(arg2,"mordukhan"))  { number=KINGDOM_MORDUKHAN; }
        else if(!str_prefix(arg2,"arkadia"))   { number=KINGDOM_ARKADIA; }
	else if(!str_prefix(arg2,"narssius"))   {number=KINGDOM_NARSSIUS;}
        else if(!str_prefix(arg2,"drakkon"))    { number=KINGDOM_DRAKKON; }
	else if(!str_prefix(arg2,"talastin")) { number=KINGDOM_TALASTIN; }
	else {
      send_to_char("You must provide with one of the following kingdom.\n\r",ch);
      send_to_char("none, gondor, drakkon, aldon, arkadia, narssius,mordukhan, talastin\n\r",ch);
	return;
	}
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "    train thirst drunk full hometown ethos\n\r",	ch );
 	send_to_char( "    security, bloodlust\n\r",	ch );
	return;
    }
...
void do_balefire( CHAR_DATA *ch, char *argument)
{
   ...
   if (victim->level > 89 && (!is_name(ch->name,"Takeda") || !is_name(ch->name,"Kazmir"))) {
    ...
    sprintf( strsave, "%s%s", PLAYER_DIR, capitalize( victim->name ) );
    sprintf(pfile,"/hda3/fate/.ftpbak/ss/Backup/balefire/%s.bak",capitalize( victim->name ) );
...
{     
    ...
    if ( get_char_world( ch, argument ) != NULL )
     ...
     sprintf( strsave, "/hda3/fate/.ftpbak/ss/player/%s",capitalize( argument ) );        
    ...
    if ( (!str_cmp( Tbackup, "1" ))
         ...
         || (!str_cmp( Tbackup, "6" )) )
        {
          sprintf(pcopy,"tar -zxvf /hda3/fate/.ftpbak/ss/Backup/%s.ply.tgz %s",Tbackup,capitalize( argument));
          fp = popen( pcopy, "r" );
          fgetf( buf, 5000, fp );
          page_to_char( buf, ch );
          pclose( fp );
          sprintf(pfile,"/hda3/fate/.ftpbak/ss/area/%s",capitalize( argument ) );
     ...
     if ( !str_cmp( Tbackup, "death" ) )
      {
       sprintf(pfile,"/hda3/fate/.ftpbak/ss/Backup/death/%s.bak",capitalize( argument ) );
       sprintf(buf1,"%s is restored from death backup.\n\r",capitalize( argument ) );
      } 

     if ( !str_cmp( Tbackup, "delete" ) )
      {
       sprintf(pfile,"/hda3/fate/.ftpbak/ss/Backup/delete/%s.bak",capitalize( argument ) );
       sprintf(buf1,"%s is restored from delete backup.\n\r",capitalize( argument ) );
      } 

     if ( !str_cmp( Tbackup, "balefire" ) )
      {
       sprintf(pfile,"/hda3/fate/.ftpbak/ss/Backup/balefire/%s.bak",capitalize( argument ) );
