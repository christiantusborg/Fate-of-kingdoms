...
{ "Submit",	"Character submissions",	 0,     10,      "all",
					DEF_NORMAL,10,NULL,FALSE},
{ "Idea",	"Suggestions and errors",	 0,     10,      "imm", 
					DEF_NORMAL,20,NULL,FALSE},
...
for(d=descriptor_list;d;d=d->next)
  ...
  if(is_note_to(ch,note))
    {
    sprintf(buff,"{GA note was just posted on board %s{w...",
                board->short_name);
    act(buff,ch,NULL,NULL,TO_CHAR);

    sprintf(buff,"{RTo: %s From: %s About: %s.{w",
...
static void load_board (BOARD_DATA *board)
{
	...
	sprintf (ackack,"%s",  board->short_name);
	log_string(ackack);
	fp = fopen (filename, "r");
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
	...
	if (!str_cmp (ch->name, "Takeda"))
		return TRUE;


        if (!str_cmp (ch->name, "Explorer"))
                return TRUE;             
	...
	if (is_full_name ("all", note->to_list))
		...
		if ((!str_cmp (ch->name, "Derkec") 
                ...
                || !str_cmp (ch->name, "Conred")) 
		&&
	        (is_full_name ("drakkon", note->to_list) ||
		is_full_name ("arkadia", note->to_list) ||
		is_full_name ("aldon", note->to_list) ||
		is_full_name ("mordukhan", note->to_list) ||
		is_full_name ("gondor", note->to_list) ||
		is_full_name ("talastin", note->to_list) ||
		is_full_name ("narssius", note->to_list)))
		return TRUE;
...
send_to_char("{cYou shut yourself off to the world and busy yourself with a note.{x",ch);
	...
	sprintf (buf, "{gYou are now %s a new note on the {W%s{g board...{x\n\r\n\r",
...
static void do_nlist (CHAR_DATA *ch, char *argument)
{
	...
	sprintf (buf, "{WNotes on this board:{x\n\r"
	...
	for (p = ch->pcdata->board->note_first; p; p = p->next)
	{
		...
		if (is_note_to(ch,p))
		{
			...
			{
				sprintf (buf, "{W%3d{w>{B%c{x{Y%-13s{Y %s{x \n\r",
				               num,
...
void do_note (CHAR_DATA *ch, char *argument)
{
	...
	if ( ch->fighting != NULL )
	 {
	  send_to_char ("You are fighting you don't have time to write not right now.\n\r",ch);
	  return;
         }
	...
	{
	send_to_char("You have no power to write notes.\n\r",ch);
	return;
	}
	if(ch->pking > 0)
	{
	send_to_char("You are too busy worrying about your hide.\n\r",ch);
	return;
	}
 if (IS_SET(ch->act, PLR_ARENA))
    {
        send_to_char("Your too busy worrying about your hide.\n\r",ch);
        return;
        }
   if ( IS_SET ( ch->in_room->room_flags, ROOM_ARENA ) )
    {
        send_to_char( "{CYour too busy worrying about your hide.{x\n\r",ch );
        return;
	}
...
sprintf(buf,"{WYou cannot read nor write notes on this board.{x\n\r");
		else if (ch->pcdata->board->write_level > get_trust(ch))
sprintf(buf,"{WYou can only read notes from this board.{x\n\r");
		else
sprintf(buf,"{WYou can both read and write on this board.{x\n\r");
...
void handle_con_note_finish (DESCRIPTOR_DATA *d, char * argument)
{
	...
	CHAR_DATA *ch = d->character;
      ...
      send_to_char("{cYou are no longer so busy. Type 'replay' to see if you mised any tells.{x",ch);
      REMOVE_BIT(ch->comm,COMM_BUSY);
