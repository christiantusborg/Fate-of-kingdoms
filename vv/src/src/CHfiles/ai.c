...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    if (obj->pIndexData->vnum > 5)      /* money, gold, etc */
        sprintf(buf_con," {W[{Y%d%%{W]{w",obj->condition);
    else buf_con[0] = '\0';
...
/*  adds hidden items */
 ...
 if ((ch->level > 81) && (  obj->pIndexData->limit != -1))  strcat(
buf,"{R(Limited){g");
...
if ( IS_AFFECTED(victim, AFF_INVISIBLE)   ) strcat( buf, "{m(Invis){B " );
    ...
    if ( IS_SET(victim->tag_flags,TAG_RED) ) strcat( buf,"{R[RED] {x"  );
    if ( IS_SET(victim->tag_flags,TAG_BLUE)) strcat( buf,"{B[BLUE] {x" );
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	strcat ( buf, " is writhing in agony.{x\n\r");
    else if (percent >= 0 )
	strcat (buf, " is convulsing on the ground.{x\n\r");
    else
   ...
   if (ch->kingdom > 0) /* only if in kingdom */
    {
     if (ch->kingdom_rank==9)
      {
       sprintf(buf,"{m%s, %s, is yours to lead, how fortunate.{x\n\r",
        capitalize(kingdom_table[ch->kingdom].short_name),kingdom_table[ch->kingdom].long_name);
       send_to_char(buf,ch);
      }
     else
      {
       sprintf(buf,"{m%s has promoted you to rank %d, %s.{x\n\r",
...
/*  ends clan info */
	...
	IS_NPC(ch) ? "Sanaria" : hometown_table[ch->hometown].name
     ...
     if(ch->legendlevel > 0)
     sprintf(buf,"{mYou are legendlevel %d.{x\n\r",
		ch->legendlevel);
...
if (!IS_NPC(ch))
{
   ...
   {
   sprintf(buf,"{mYou have managed to stay clear of Fate's murderous customs.{x\n\r");
   send_to_char(buf,ch);
   }

   if (ch->pcdata->pkills > 1)
   {
   sprintf(buf,"{mIn your travels, you have managed to slay %d of your enemies.{x\n\r",ch->pcdata->pkills);
   send_to_char(buf,ch);
   }

   if (ch->pcdata->pkills == 1)
   {
   sprintf(buf,"{mYou have managed to kill a single enemy in your travels.{x\n\r");
   send_to_char(buf,ch);
   }
}
/*  end */
...
if (!IS_NPC(ch))
{
   ...
   {
   sprintf(buf,"{mYou bear the scars of %d deaths, a somewhat trifling matter.{x\n\r",ch->pcdata->total_death);
   send_to_char(buf,ch);
   }

   if (ch->pcdata->total_death > 9)
   {
   sprintf(buf,"{mYou bear the scars of %d deaths, you might be better off dead.{x\n\r",ch->pcdata->total_death);
   send_to_char(buf,ch);
   }

   if (ch->pcdata->total_death == 1)
   {
   sprintf(buf,"{mYou have suffered but a single death, consider yourself lucky.{x\n\r");
   send_to_char(buf,ch);
   }

   if (ch->pcdata->total_death == 0)
   {
   sprintf(buf,"{mYou have never been killed, and should be proud of yourself.{x\n\r");
   send_to_char(buf,ch);
   }
}

if (ch->religion > 0) {
sprintf(buf,"You believes the religion of %s.\n\r",
...
if(ch->pcdata->spouse != NULL) {
	sprintf(buf, "{mYou are married to %s.{w\n\r",ch->pcdata->spouse);
	send_to_char(buf,ch);
        }
...
void do_scroll(CHAR_DATA *ch, char *argument)
{
	...
	{
	    sprintf(buf,"{gYou currently display %d lines per page.{x\n\r",
		    ch->lines + 2);
	    send_to_char(buf,ch);
	}
	return;
    ...
    sprintf(buf,"{gScroll set to %d lines.{x\n\r",lines);
    send_to_char(buf,ch);
    ch->lines = lines - 2;
}
...
void do_socials(CHAR_DATA *ch, char *argument)
{
    ...
    {
	sprintf(buf,"{g%-12s{x",social_table[iSocial].name);
	send_to_char(buf,ch);
	if (++col % 6 == 0)
	    send_to_char("\n\r",ch);
    }
...
send_to_char( "{CYou look around, things sure are pleasant, perhaps you should wake up from your fantasy.{x\n\r", ch);
    ...
    {
        send_to_char( "{bIt is pitch black... {x\n\r", ch );
...
{ /* for NON wilderness */
	...
	{
	    sprintf(buf," {M[Room %d]{w",ch->in_room->vnum);
	    send_to_char(buf,ch);
	}
	    send_to_char( "\n\r",ch);
...
{ /* if IS wilderness */
	...
	case ITEM_DRINK_CON:
	    if ( obj->value[1] <= 0 )
	    {
		send_to_char( "{CIt is empty.{x\n\r", ch );
		break;
	    }

	    sprintf( buf, "{cIt's %sfilled with  a %s liquid.{x\n\r",
...
send_to_char( "\033[H\033[K\n\033[K\n\033[K", ch );
        ...
        if( IS_IMMORTAL(ch) )
        {
            ...
            sprintf( buf, "{w[{c%d{w]", ch->in_room->vnum );
            send_to_char( buf, ch );
        }
...
void do_worth( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch))
    {
	sprintf(buf,"{cYou have %ld gold and %ld silver.{x\n\r",
	    ch->gold,ch->silver);
	send_to_char(buf,ch);
	return;
    }
...
for ( d = descriptor_list; d != NULL; d = d->next )
{
  ...
  if (d->character->pcdata->bounty > 0 || d->character->pcdata->owed > 0 )
	{
sprintf(buf,"{r%-15s{R%10d{B%10d{x\n\r",d->character->name,d->character->pcdata->bounty,d->character->pcdata->owed);
	send_to_char(buf,ch);
	} /* if2 */
} /* for */	    
...
send_to_char("{gYou collect your hard earned gold bounty.{x\n\r",ch);
  ...
  if ((!IS_NPC(ch)) && (number_percent() < 5)){
    char buf[MAX_STRING_LENGTH];
    sprintf(buf, " the Mercurial %s", capitalize(class_table[ch->class].name ));
...
else /* for placing bounties */
{
    if (amount < 100 )
    {
    send_to_char("{gA voice whispers '{GYou have to place at least 100 gold{g'.{x\n\r",ch);
    return;
     }
  ...
  if ((!IS_NPC(ch)) && (number_percent() < 5)){
    char buf[MAX_STRING_LENGTH];
    sprintf(buf, " the Contractor %s", capitalize(class_table[ch->class].name ));
    free_string( ch->pcdata->title );ch->pcdata->title = str_dup( buf );}

  if ((!IS_NPC(victim)) && (number_percent() < 5)){
    char buf[MAX_STRING_LENGTH];
    sprintf(buf, " the Unusually Popular %s", capitalize(class_table[victim->class].name ));
...
*/
    ...
    {
	sprintf( buf, "{mYou are trusted at level %d.{x\n\r",
	    get_trust( ch ) );
	send_to_char( buf, ch );
    }
sprintf(buf,
...
send_to_char("{mStatus: ",ch);
    ...
    if (ch->guarding != NULL)
      {
	sprintf(buf1, " {cGuarding %s{m,", ch->guarding->name);
	send_to_char( buf1, ch);
      }
    if (ch->guarded_by != NULL)
      {
	sprintf(buf2, " {cGuarded by %s{m,", ch->guarded_by->name);
...
/*  begins replacement */
sprintf( buf, "            {mYou also carry {b%ld{m gold \
and {b%ld{m silver.{x",
       ch->gold,
       ch->silver);
       send_to_char( buf, ch );
...
int i;
	...
	if      (GET_AC(ch,i) >=  101 ) 
	    sprintf(buf,"hopelessly vulnerable to %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= 80) 
	    sprintf(buf,"defenseless against %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= 60)
	    sprintf(buf,"barely protected from %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= 40)
	    sprintf(buf,"slightly armored against %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= 20)
	    sprintf(buf,"somewhat armored against %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= 0)
	    sprintf(buf,"armored against %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= -20)
	    sprintf(buf,"well-armored against %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= -40)
	    sprintf(buf,"very well-armored against %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= -60)
	    sprintf(buf,"heavily armored against %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= -80)
	    sprintf(buf,"superbly armored against %s{M.{x\n\r",temp);
	else if (GET_AC(ch,i) >= -100)
	    sprintf(buf,"almost invulnerable to %s{M.{x\n\r",temp);
	else
	    sprintf(buf,"divinely armored against %s{M.{x\n\r",temp);
	...
	if (ch->invis_level)
      {
        sprintf( buf, "      {MInvisible: {mlevel {b%d{x\n\r",ch->invis_level);
        send_to_char(buf,ch);
      }

	if (ch->incog_level)
	{
	  sprintf(buf,"      {MIncognito: {mlevel {b%d{x\n\r", ch->incog_level);
	  send_to_char(buf,ch);
	}
send_to_char("{B+-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=+{x\n\r",ch);
    if (ch->nochanned > 0)
	...
	{
	    sprintf( buf, "{mSpell: '{Y%s{m'", skill_table[paf->type].name);
	    send_to_char( buf, ch );
...
ch->class==8 || ch->class==9 || ch->class==14 ||
		ch->race == 6 || ch->race==7 || ch->class == CLASS_DRD */
	    	...
	    	sprintf( buf, "{mSpell: %-15s",skill_table[paf->type].name );
...
ch->class==8 || ch->class==9 || ch->class==14 ||
		...
		if ( paf->duration == -1 )
		    ...
		    sprintf( buf, "for {b%d{m hours", paf->duration );
		send_to_char( buf, ch );
	    }
	    send_to_char( "{x\n\r", ch );
	    paf_last = paf;
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "{mFate started up at %sThe system time is %s{x",
	str_boot_time,
	(char *) ctime( &current_time ) );
    send_to_char( buf, ch );
  sprintf(buf, "{GFate will copyover in %d minutes.{w\n",
        copyover_counter);
        send_to_char(buf,ch);
    return;
}
...
void do_weather( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "{cThe sky is {C%s{c and {C%s{c.{x\n\r",
...
void do_whois (CHAR_DATA *ch, char *argument)
{
 ...
 // ||
	//		       wch->kingdom == KINGDOM_SIRIRIUS)
	      sprintf(buf2, "{W[%s]{x ",kingdom_table[wch->kingdom].short_name);
	    else buf2[0] = '\0';

            sprintf(buf, "{G%s is a %s%s from %s{w\n\r",
		wch->name,
...
*/
/*
	      sprintf(buf, "{m[{B%2d %s %s{m]{x %s%s%s%s%s{m%s%s{x\n\r",
		      wch->level,
		      wch->race < MAX_PC_RACE ? 
	      ...
	      sprintf(buf, "{m[   {B%s{m    ] %s%s%s%s%s{m%s%s{x\n\r",
		      wch->race < MAX_PC_RACE ? 
		        pc_race_table[wch->race].who_name: "     ",
		      ( (ch==wch && ch->exp < PK_MIN) || 
		        is_safe_nomessage(ch,wch) ) ? 
...
/*	|| ( fTattoo &&(vnum == get_eq_char(wch,WEAR_TATTOO)->pIndexData->vnum))*/
	    ...
	    || (fHordeRestrict && wch->kingdom != KINGDOM_NARSSIUS)
	    ...
	    || (fTristanRestrict && wch->kingdom != KINGDOM_TALASTIN)
	)
	    continue;
...
if ( wch->level >= LEVEL_IMMORTAL )
  ...
  sprintf(kingdombuf,"{W[%s]{x ",kingdom_table[wch->kingdom].short_name);
	else kingdombuf[0] = '\0';
...
if (IS_TRUSTED(ch,(MAX_LEVEL - 14 )) || ch==wch ||
                   wch->level >= LEVEL_HERO)
	  sprintf( buf, "{W[{W%3d %s {G%s{W %s]{x %s%s%s%s%s%s%s%s%s{m%s%s{x\n\r",
	    wch->level,
	    wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
	    class,
	    !IS_SET(wch->info,INFO_ICOOC) ? "{W[{GOOC{W]{X " : "{W[{GIC{W]{X  ", 
	    kingdombuf,
	    IS_SET(wch->tag_flags,TAG_RED) ? "{R[RED] {x" :
            (IS_SET(wch->tag_flags,TAG_BLUE) ? "{B[BLUE] {x" : ""),
	    IS_SET(wch->act, PLR_WANTED) ? "{r(ORB QUEST){x " : "", 
	    IS_SET(wch->act, PLR_KILLER) ? "{r(KILLER){x " : "",
	    IS_SET(wch->act, PLR_THIEF)  ? "{r(THIEF){x "  : "",
	  ...
	  sprintf( buf, "{W[    %s{W     %s] {x%s%s%s%s%s%s%s%s{m%s%s{x\n\r",
	    wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
	    kingdombuf,
	    !IS_SET(wch->info,INFO_ICOOC) ? "{W[{GOOC{W]{X " : "{W[{GIC{W]{X  ", 
...
if (IS_TRUSTED(ch,(MAX_LEVEL - 14 )) || ch==wch || wch->level >= LEVEL_HERO)
	  sprintf( buf, "{W[%3d %s {G%s{W %s]{x %s%s%s%s%s%s%s%s%s{m%s%s{x\n\r",
	    wch->level,
	    wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
	    class,
	  ...
	  sprintf( buf, "{W[    %s    {W %s] {x%s%s%s%s%s%s%s%s%s{m%s%s{x\n\r",
	    wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
          !IS_SET(wch->info,INFO_ICOOC) ? "{W[{GOOC{W]{X " : "{W[{GIC{W]{X  ", 
	  ( (ch==wch && ch->exp < PK_MIN) || 
    ...
    page_to_char( buf_string(output), ch );
    free_buf(output);
    sprintf( buf2, "{mImmortals Found: %d{x\n\r{cPlayers found: %d{x\n\r\n\r", immMatch, nMatch );
    send_to_char(buf2,ch);
    return;
}
...
void do_count ( CHAR_DATA *ch, char *argument )
{
    ...
    if (max_on == count)
sprintf(buf,"{B%d{M adventurers... the most I've seen today.{x\n\r",
	    count);
    else
sprintf(buf,"{B%d{M adventurers... I saw {m%d{M earlier today.{x\n\r",
	    count,max_on);
...
int RANGE=5,
	...
	send_to_char("You can't seem to find anyone in this vast wilderness.",ch);
	return;
...
if ( pretitle[0] != '.' && pretitle[0] != ',' && pretitle[0] != '!' &&
		pretitle[0] != '?' )
    {
        buf[0] = '\0';
        strcpy( buf, pretitle );
...
void do_title( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
	{send_to_char( "{cChange whose title to what?{x\n\r", ch);return;}
    ...
    if ( argument[0] == '\0' )
	{send_to_char( "{cChange the title to what?{x\n\r", ch);return;}
  ...
  send_to_char( "Ok.\n\r", ch );
  sprintf( buf, "{MYou have been granted a title by the gods:\n\r '{W%s %s{M'{x\n\r", victim->name, victim->pcdata->title );
...
void do_pretitle( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
	{send_to_char( "{cChange whose pretitle to what?{x\n\r", ch);return;}
    ...
    if ( IS_NPC(victim) ) {
        send_to_char("{cSyntax: pretitle <pc_name> <pretitle>{x\n\r",ch);
	return;
    }	

    if ( argument[0] == '\0' )
	{send_to_char( "{cChange the pretitle to what?{x\n\r", ch);return;}
    ...
    sprintf( buf, "{MYou have been granted a pretitle by the gods:\n\r '{W%s%s{M'{x\n\r", victim->pcdata->pretitle, victim->name );
    send_to_char( buf, victim ); 
}
...
void do_scan(CHAR_DATA *ch, char *argument)
{
  ...
  WAIT_STATE(ch,2);
  sprintf(buf, "{gYou scan %s.{x\n\r",dir2);
  send_to_char(buf,ch);
  sprintf(buf, "{g$n scans %s.{x",dir2);
  act(buf,ch,NULL,NULL,TO_ROOM);
...
void do_finger(CHAR_DATA *ch, char *argument)
{
   ...
   if (IS_NPC(ch))
    ...
    sprintf(log_buf, "Look for name some of who crashing fate %s.",
    ch->name);
    log_string(log_buf);  
    ...
    sprintf(buf,"Name: %16s   Real Name: %16s\n\r", victim->name,
       victim->pcdata->real_name != NULL ? victim->pcdata->real_name:
	"Not specified.");
  if(IS_IMMORTAL(ch) || IS_IMMORTAL(victim)) {
    send_to_char(buf, ch);
    sprintf(buf,"Level: %16d{w  Race: %16s{w\n\r",
    victim->level,
    ...
    sprintf(buf,"Kingdom: %16s{w Rank: %16s{w\n\r",
    victim->kingdom > MAX_KINGDOM ? "Error" :
    victim->kingdom < 1 ? "None" :
     kingdom_table[victim->kingdom].name,
    0 < victim->kingdom < MAX_KINGDOM ? buf1 : "None");
    }
    send_to_char(buf,ch);
    sprintf(buf, "E-mail address: %35s\n\r", 
    victim->pcdata->email != NULL ? victim->pcdata->email:
	"Not specified.");
    send_to_char(buf, ch);
    if(victim->desc != NULL)
   ...
   if(seconds > 0)
    ...
    if((finished)||
 (real_found&&laston_found&&email_found&&raceb&&sexb&&kingdomb&&rankb&&levelb))
      break;   
     }
  ...
  if(IS_IMMORTAL(ch)) {
		if(levelb == TRUE) 
		send_to_char("level: true",ch);
		else
		send_to_char("level: false",ch);
		if(raceb == TRUE) 
		send_to_char("race: true",ch);
		else
		send_to_char("race: false",ch);
		if(sexb == TRUE) 
		send_to_char("sex: true",ch);
		else
		send_to_char("sex: false",ch);
		if(kingdomb == TRUE) 
		send_to_char("kingdom: true",ch);
		else
		send_to_char("kingdom: false",ch);
		if(rankb == TRUE) 
		send_to_char("rank: true\n",ch);
		else
		send_to_char("rank: false\n",ch);

    if(raceb == TRUE && levelb == TRUE) {
    sprintf(buf,"Level: %-16d  Race: %-16s\n\r", 
       levelb ? level : 0, 
	race < MAX_PC_RACE ? pc_race_table[race].who_name
                  : "Error");
    send_to_char(buf, ch);
	}

  if (sexb == TRUE && kingdomb == TRUE && rankb == TRUE) {
    if(sex == SEX_FEMALE)
    sprintf(buf1, "%s",
	kingdom_table[kingdom].rank_titlesf[rank]);
        else
    sprintf(buf1, "%s",
        kingdom_table[kingdom].rank_titles[rank]);

  sprintf(buf,"Kingdom: %16s Rank: %16s\n\r",
    0 < kingdom < MAX_KINGDOM ? kingdom_table[kingdom].name
                                    : "None",
    0 < kingdom < MAX_KINGDOM ? buf1 : "None");
	send_to_char(buf,ch);
    }
  }
...
void do_mk( CHAR_DATA *ch, char *argument )
{
    char arg1[MAX_INPUT_LENGTH];
    char buf[160];
    CHAR_DATA *victim;
    ...
    if ( IS_NPC(victim) ) {
        send_to_char("{cSyntax: mk <pc_name>{x\n\r",ch);
	return;
    }	
    ...
    smash_tilde( argument );
    set_pretitle( victim, "{rM{Dordukha{rn {rC{Dommone{rr{b" );
    send_to_char( "Ok.\n\r", ch );
 
    sprintf( buf, "{MYou have been granted a pretitle by the Mordukhan:\n\r '{W%s%s{M'{x\n\r", victim->pcdata->pretitle, victim->name );
    send_to_char( buf, victim ); 
}