...
/*  end */
  ...
  bug("malloc'ing AUCTION_DATA didn't give %d bytes",sizeof(AUCTION_DATA));
	...
	{
	    if ( skill_table[sn].abiliti_pgsn != NULL )
		*skill_table[sn].abiliti_pgsn = sn;
	}
    }
...
fix_objprogs( );
log_string("[*]-------OBJ Progs-----[*]");	
fix_roomprogs( );
log_string("[*]-------ROOM Progs-----[*]");	
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
	...
	mob->nrattack		= pMobIndex->nrattack;
...
/*  debug string */
    ...
    if ( !isdigit(c) )
    {
	sprintf (buf, "-char> %c   number> %d", c, number);
	bug( buf, 0 );
...
sprintf(buf2, "Tally_pfile_objects: [%s]'s file +: %d\n", buf, UMAX(0, i-5));
