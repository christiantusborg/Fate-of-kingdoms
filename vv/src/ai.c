...
sprintf(log_buf, "%s Added a bounty %d gold and %s bounty is now %d.",
...
send_to_char("{B+-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=+{x\n\r",ch);
	...
	{
	    sprintf( buf, "{mSpell: '{Y%s{m'", skill_table[paf->type].abiliti_name);
	    send_to_char( buf, ch );
...
ch->class==8 || ch->class==9 || ch->class==14 ||
		ch->race == 6 || ch->race==7 || ch->class == CLASS_DRD */
	    	...
	    	sprintf( buf, "{mSpell: %-15s",skill_table[paf->type].abiliti_name );
    ...
    if (ch->in_room->affected != NULL )
    {
        ...
        for ( paf = ch->in_room->affected; paf != NULL; paf = paf->next )
        {
            if (paf_last != NULL && paf->type == paf_last->type)
                ...
                sprintf( buf, "Spell: %-15s", skill_table[paf->type].abiliti_name);
    send_to_char( buf, ch );
...
{
 send_to_char("This command is not used in fok4",ch);
 return;
}
