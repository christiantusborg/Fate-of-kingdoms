...
/*  end */
    ...
    {
	if ( get_eq_char( ch, WEAR_HANDS_1 ) != NULL
	&&   get_eq_char( ch, WEAR_HANDS_2 ) != NULL
	&&   !remove_obj( ch, WEAR_HANDS_1, fReplace )
	&&   !remove_obj( ch, WEAR_HANDS_2, fReplace ) )
	    return;

	if ( get_eq_char( ch, WEAR_HANDS_1 ) == NULL )
	{
	    act( "{g$n wears $p on $s hands{x",
		ch, obj, NULL, TO_ROOM );
	    act( "{gYou wear $p on your hands.{x",
		ch, obj, NULL, TO_CHAR );
	    equip_char( ch, obj, WEAR_HANDS_1 );
	    return;
	}

	if ( get_eq_char( ch, WEAR_HANDS_2 ) == NULL )
	{
	    act( "{g$n wears $p on $s lower set of hands.{x",
		ch, obj, NULL, TO_ROOM );
	    act( "{gYou wear $p on your lower set of hands.{x",
		ch, obj, NULL, TO_CHAR );
	    equip_char( ch, obj, WEAR_HANDS_2 );
	    return;
	}

	bug( "Wear_obj: no free arms.", 0 );
	send_to_char( "{cYour arms are already protected.{x\n\r", ch );
	return;
    }

    if ( CAN_WEAR( obj, ITEM_WEAR_ARMS ) )
    {
	if ( get_eq_char( ch, WEAR_ARMS_1 ) != NULL
	&&   get_eq_char( ch, WEAR_ARMS_2 ) != NULL
	&&   !remove_obj( ch, WEAR_ARMS_1, fReplace )
	&&   !remove_obj( ch, WEAR_ARMS_2, fReplace ) )
	    return;

	if ( get_eq_char( ch, WEAR_ARMS_1 ) == NULL )
	{
	    act( "{g$n wears $p on $s arms{x",
		ch, obj, NULL, TO_ROOM );
	    act( "{gYou wear $p on your arms.{x",
		ch, obj, NULL, TO_CHAR );
	    equip_char( ch, obj, WEAR_ARMS_1 );
	    return;
	}

	if ( get_eq_char( ch, WEAR_ARMS_2 ) == NULL )
	{
	    act( "{g$n wears $p on $s lower set of arms.{x",
		ch, obj, NULL, TO_ROOM );
	    act( "{gYou wear $p on your lower set of arms.{x",
		ch, obj, NULL, TO_CHAR );
	...
	send_to_char( "{cYour hands are already protected.{x\n\r", ch );
	return;
    }
    ...
    if ( CAN_WEAR( obj, ITEM_WEAR_SADDLE ) )
    {
	if ( !remove_obj( ch, WEAR_ABOUT, fReplace ) )
	    return;
	act( "{g$n wears $p on $s back.{x",   ch, obj, NULL, TO_ROOM );
	act( "{gYou wear $p on your back.{x", ch, obj, NULL, TO_CHAR );
	equip_char( ch, obj, WEAR_SADDLE );
	return;
    }
    
    if ( CAN_WEAR( obj, ITEM_WEAR_HOOF ) )
    {
	if ( !remove_obj( ch, WEAR_ABOUT, fReplace ) )
	    return;
	act( "{g$n wears $p on $s hooves.{x",   ch, obj, NULL, TO_ROOM );
	act( "{gYou wear $p on your hooves.{x", ch, obj, NULL, TO_CHAR );
	equip_char( ch, obj, WEAR_HOOF );
	return;
    }
    
    if ( CAN_WEAR( obj, ITEM_WEAR_TAILPIECE ) )
    {
	if ( !remove_obj( ch, WEAR_ABOUT, fReplace ) )
	    return;
	act( "{g$n attaches $p to $s tail.{x",   ch, obj, NULL, TO_ROOM );
	act( "{gYou attach $p to your tail.{x", ch, obj, NULL, TO_CHAR );
	equip_char( ch, obj, WEAR_TAIL );
	return;
    }
    
    if ( CAN_WEAR( obj, ITEM_WEAR_HORN ) )
    {
	if ( !remove_obj( ch, WEAR_ABOUT, fReplace ) )
	    return;
	act( "{g$n puts $p on $s horns.{x",   ch, obj, NULL, TO_ROOM );
	act( "{gYou put $p on your horns.{x", ch, obj, NULL, TO_CHAR );
	equip_char( ch, obj, WEAR_HORN );
	return;
    }
	...
	&&   get_eq_char( ch, WEAR_WRIST_3 ) != NULL
	&&   get_eq_char( ch, WEAR_WRIST_4 ) != NULL
	&&   !remove_obj( ch, WEAR_WRIST_1, fReplace )
	&&   !remove_obj( ch, WEAR_WRIST_2, fReplace )
	&&   !remove_obj( ch, WEAR_WRIST_3, fReplace )
	&&   !remove_obj( ch, WEAR_WRIST_4, fReplace ) )
	    return;
	...
	if ( get_eq_char( ch, WEAR_WRIST_3 ) == NULL )
	{
	    act( "{g$n wears $p around $s lower left wrist.{x",
		ch, obj, NULL, TO_ROOM );
	    act( "{gYou wear $p around your lower left wrist.{x",
		ch, obj, NULL, TO_CHAR );
	    equip_char( ch, obj, WEAR_WRIST_3 );
	    return;
	}

	if ( get_eq_char( ch, WEAR_WRIST_4 ) == NULL )
	{
	    act( "{g$n wears $p around $s lower right wrist.{x",
		ch, obj, NULL, TO_ROOM );
	    act( "{gYou wear $p around your lower right wrist.{x",
		ch, obj, NULL, TO_CHAR );
	    equip_char( ch, obj, WEAR_WRIST_4 );
	    return;
	}

	bug( "Wear_obj: no free wrist.", 0 );
	send_to_char( "{cYou can't wear more items around your wrists.{x\n\r", ch );
	return;
    }
