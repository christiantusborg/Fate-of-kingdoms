...
void show_skill_cmds( CHAR_DATA *ch, int tar )
{
    ...
    {
	if ( !skill_table[sn].abiliti_name )
	    break;

	if ( !str_cmp( skill_table[sn].abiliti_name, "reserved" )
	  || skill_table[sn].spell_fun == spell_null )
	    continue;

	if ( tar == -1 || skill_table[sn].abiliti_target == tar )
	{
	    sprintf( buf, "%-19.18s", skill_table[sn].abiliti_name );
	    strcat( buf1, buf );
	    if ( ++col % 4 == 0 )
		strcat( buf1, "\n\r" );
	}
...
const struct wear_type wear_table[] =
{
    ...
    {	WEAR_HANDS_1,	ITEM_WEAR_HANDS		},
    {	WEAR_HANDS_2,	ITEM_WEAR_HANDS		},    
    {	WEAR_ARMS_1,	ITEM_WEAR_ARMS		},
    {	WEAR_ARMS_2,	ITEM_WEAR_ARMS		},
    {	WEAR_SHIELD,	ITEM_WEAR_SHIELD	},
    {	WEAR_ABOUT,	ITEM_WEAR_ABOUT		},
    {	WEAR_SADDLE,	ITEM_WEAR_SADDLE	},
    {	WEAR_HOOF,	ITEM_WEAR_HOOF		},
    {	WEAR_TAIL,	ITEM_WEAR_TAILPIECE	},
    {	WEAR_HORN,	ITEM_WEAR_HORN		},
    {	WEAR_WAIST,	ITEM_WEAR_WAIST		},
    {	WEAR_WRIST_1,	ITEM_WEAR_WRIST		},
    {	WEAR_WRIST_2,	ITEM_WEAR_WRIST		},
    {	WEAR_WRIST_3,	ITEM_WEAR_WRIST		},
    {	WEAR_WRIST_4,	ITEM_WEAR_WRIST		},
...
void show_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *obj )
{
	...
	case ITEM_STAFF:
		...
		obj->value[1],
		obj->value[2],
		obj->value[3] != -1 ? skill_table[obj->value[3]].abiliti_name
		                    : "none" );
	    send_to_char( buf, ch );
	    break;
	...
	case ITEM_PILL:
		...
		"[v4] Spell:  %s\n\r",
		obj->value[0],
		obj->value[1] != -1 ? skill_table[obj->value[1]].abiliti_name
		                    : "none",
		obj->value[2] != -1 ? skill_table[obj->value[2]].abiliti_name
                                    : "none",
		obj->value[3] != -1 ? skill_table[obj->value[3]].abiliti_name
		                    : "none",
		obj->value[4] != -1 ? skill_table[obj->value[4]].abiliti_name
		                    : "none" );
	    send_to_char( buf, ch );
	    break;
...
//    send_to_char( buf, ch );
    ...
    send_to_char("mRes : ", ch );
    for (i = 0; i <= MAX_RESISTANCE -1; i++)
     {
      sprintf( buf, "%d ",pObj->resistance[i]);
      send_to_char( buf, ch );
     }
    send_to_char( "\n\r", ch );
     send_to_char("mImm : ", ch );
      for (i = 0; i <= MAX_RESISTANCE -1 ; i++)
        {
       	 sprintf( buf, "%d ",pObj->immuniti[i]);
	 send_to_char( buf, ch );
        }
  send_to_char( "\n\r", ch );
...
/*
    if ( bvector[0] == '\0' || ( bv = flag_value( bitvector_type, bvector ) ) == NO_FLAG )
    {
...
//    MPROG_LIST *list;
    ...
    send_to_char( buf, ch );
    sprintf( buf, "{mClass: {c[%6.6s]     ",                  
	pc_race_table[pMob->class].name );
    send_to_char( buf, ch );    
    sprintf( buf,
    ...
    for (i = 0; i <= MAX_RESISTANCE -1; i++)
     {
      sprintf( buf, "%d ",pMob->resistance[i]);
      send_to_char( buf, ch );
     }
    send_to_char( "\n\r", ch );
     send_to_char("mImm : ", ch );
...
OEDIT( oedit_size )
{
    ...
    send_to_char( "Syntax: osize [size]\n\r"
		  "Type '? size' for a list of sizes.{x\n\r", ch );
    return FALSE;
}
...
OEDIT( oedit_nres)
{
    ...
    if ( arg2[0] == '\0' )
    {
     send_to_char( "Syntax: nres <resistance> <1-75>\n\r? res for more help\n\r", ch );
     return FALSE;	     
    }
   ...
   if( ( val >= 75) || (val <= -101) )
    {
     send_to_char( "Syntax: nres <resistance> <-100 to 75>\n\r? res for more help\n\r", ch );
     return FALSE;	     
    }

    EDIT_OBJ( ch, pObj);

    if ( ( value = flag_value( vuln_new_flags, arg1 ) ) != NO_FLAG )
     {
      pObj->resistance[value] = val;
      send_to_char( "Res set.{x\n\r", ch );
      return TRUE;	     
     }  
 return FALSE;
}
...
OEDIT( oedit_nimm)
{
    ...
    if ( arg2[0] == '\0' )
    {
     send_to_char( "Syntax: nres <imm> <1-75>\n\r? imm for more help\n\r", ch );
     return FALSE;	     
    }
   ...
   if( ( val >= 75) || (val <= 0) )
    ...
    if ( ( value = flag_value( vuln_new_flags, arg1 ) ) != NO_FLAG )
     {
      pObj->immuniti[value] = val;
      send_to_char( "Imm set.{x\n\r", ch );
      return TRUE;	     
     }  
 return FALSE;
}
...
MEDIT( medit_class )
{
	...
	EDIT_MOB( ch, pMobIndex );
      ...
      if ( !str_cmp( pc_race_table[i].name, argument ))
	{
	    pMobIndex->class = i;
...
MEDIT( medit_nres)
{
    ...
    if ( arg2[0] == '\0' )
    {
     send_to_char( "error1", ch );
     return FALSE;	     
    }

    if (!is_number( arg2 ))
     {
      send_to_char( "error2", ch );
      return FALSE;	     
     }    
   ...
   if( ( val >= 75) || (val <= 0) )
    ...
    if ( ( value = flag_value( vuln_new_flags, arg1 ) ) != NO_FLAG )
     {
      pMobIndex->resistance[value] = val;
      send_to_char( "Res set.{x\n\r", ch );
      return TRUE;	     
     }  
 return FALSE;
...
MEDIT( medit_res_imm )
{
   ...
   if ( (str_cmp (arg1, "imm"))  && (str_cmp (arg1, "res") ))
     {
      send_to_char( "Syntax: vresimm res [res_flag] <1-75>\n\r",ch);
      send_to_char( "Syntax: vresimm imm [imm_flag] <1-75>\n\r",ch);
      send_to_char( "Syntax: vresimm imm|res show\n\r",ch);
      send_to_char( "Type '? res|imm' for a list of sizes.{x\n\r", ch );
      return FALSE;     
     }
     ...
     if (!str_cmp (arg1, "imm"))
      {
       for (i = 0; i <= MAX_RESISTANCE -1 ; i++)
        {
       	 sprintf( buf, "%-25s : %d\n\r",res_flags[i].name,pMobIndex->immuniti[i]);
	 send_to_char( buf, ch );
        }
        return TRUE;	     
       } 
      else
       {
        for (i = 0; i <= MAX_RESISTANCE -1; i++)
         {
       	  sprintf( buf, "%-25s : %d\n\r",res_flags[i].name,pMobIndex->resistance[i]);
	  send_to_char( buf, ch );
         }
         return TRUE;	      
        } 
    ...
    if (( arg3[0] != '\0' ))
    {
      ...
      if( ( val <= 75) && (val >= 0) )
       { //if0
        for (i = 0; i <= MAX_RESISTANCE -1 ; i++)
         { //for1
         if (!str_cmp (res_flags[i].name,arg2));
          { //if1
	   if (!str_cmp (arg1, "imm") )
	    { //if2
	     pMobIndex->immuniti[i] = val;
	     send_to_char( "Imm set.{x\n\r", ch );
	     return TRUE;	     
	    } //fi2
           else if (!str_cmp (arg1, "res") )
            { //if3
            pMobIndex->resistance[i] = val;
            send_to_char( "Res set.{x\n\r", ch );
	    return TRUE;	     
             } //fi3
          } //fi1
...
MEDIT( medit_nrattack )
{
    ...
    if ( is_number( argument ) )
     {
      ...
      if ( i > 0 && i < 10)
       {
        pMobIndex->nrattack = i;
        send_to_char( "Attack's Set\n\r",ch);
        return TRUE;
       } 
     }
    ...
    send_to_char( "Syntax: Attack [1-9]\n\r",ch);
    return FALSE;
}
