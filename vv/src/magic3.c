...
void spell_agitation ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    damage( ch, victim, dam, sn, WEAR_NONE ,TRUE);
...
void spell_death_field ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( !IS_NPC( ch ) ? IS_NPC( vch ) : IS_NPC( vch ) )
	{
	    ...
	    damage( ch, vch, dam, sn, WEAR_NONE,TRUE );
        }
    }
    return;
}
...
{
    damage( ch, (CHAR_DATA *) vo, dice( 2, 10 ) + level / 2, sn,WEAR_NONE,TRUE );
    return;
}
...
{
    damage( ch, (CHAR_DATA *) vo, dice( 1, 10 ) + level / 2, sn, WEAR_NONE,TRUE);
    return;
}
...
{
    damage( ch, (CHAR_DATA *) vo, dice( 4, 6 ) + level, sn, WEAR_NONE,TRUE);
    return;
}
...
{
    damage( ch, (CHAR_DATA *) vo, dice( 3, 5 ) + level, sn, WEAR_NONE,TRUE );
    return;
}
...
/* Animate Dead by Garion */
{
    ...
    OBJ_DATA    *obj;
    CHAR_DATA *gch;
     int numcharmie,extramana,chance;
 AFFECT_DATA af;
  ...
  if (is_affected(ch, sn))
    ...
    else 
    {
     ...
     if (obj->level < 10)
      ...
      zombie->level = (obj->level - 5 );

     zombie->max_hit = ( zombie->level *125 );
...
void spell_summon_dead( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  if (is_affected(ch,sn))
	...
	zombie->max_hit = (ch->max_hit*2);	/* give it hp	*/
	...
	return;
        ...
        if(!is_affected(victim,sn))
        {
         ...
         af.type	= gsn_aura_divine_deflection;
      	 ...
      	 af.location	= 0;
      	 affect_to_char(victim,&af);
         send_to_char( "The Immmortal will now deflect you attacks.\n\r",victim);
