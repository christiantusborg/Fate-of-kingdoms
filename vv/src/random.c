...
//         (skill_table[skill_no].kingdom==KINGDOM_NONE)&& /*not a kingdom spell*/
          mask & (1<<skill_table[skill_no].abiliti_target))  /* appropriate?*/
         {
         *type = skill_table[skill_no].abiliti_target;
         return skill_no;
         }
      }
}
...
void wield_random_magic( CHAR_DATA *mob )
{
   ...
   switch (item_type) {
   case 92: /* scroll */
      sprintf(buffer, "%s scroll", adj1[n_adj1]);
      ...
      break;
   case 93: /* wand */
      sprintf(buffer, "%s wand", adj1[n_adj1]);
...
if (chance<25)
      ...
      sprintf(buffer, "%s", armor_types[ac_type]);
else
      sprintf(buffer, "%s", adj1[n_adj1]);
...
number_fuzzy(armor_mul[ac_type])+((number_fuzzy(armor_div[ac_type]))/3);
      ...
      sprintf(buffer, "%s", weapon_types[wea_type]);
...
if (obj->item_type == ITEM_WEAPON)
   sprintf( buf2, "%s lies here, ready to do some damage.",capitalize(obj->short_descr));
else if (obj->item_type == ITEM_TREASURE )
   sprintf( buf2, "%s lies here in splendor.", capitalize(obj->short_descr) );
else
   sprintf( buf2, "%s lies here, ready to be worn.", capitalize(obj->short_descr) );
