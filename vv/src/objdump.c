...
7	Flags			GENVMDRghdlvbPVRLSBU#*
					anti-Good, anti-Evil, anti-Neutral, inVisible, Maic, noDrop,
...
void add_to_list (OBJ_RESET *list, int vnum, sh_int level)
{
	OBJ_RESET * new_item;
...
bool is_in_list (OBJ_RESET *list, int vnum, sh_int level)
{
...
if (is_in_list (list, vnum_what, level))
	return; 
/* only write obj of same level once */
...
case ITEM_STAFF:
	sprintf (buf1, "\"%s\",%d,%d,\"%s\",", 
			 ...
			 skill_table[obj->value[3]].abiliti_name
			 : "unknown spell"
	        ); 
	break; /* wand, staff */
...
case ITEM_POTION: /* pill|scroll|potion, spell-level, 0, spell-name(s) */
	...
	for (i = 1; i < 5; i++) /* extra slot for ROM */
	{
		...
		{
			strcat (buf1, skill_table[obj->value[i]].abiliti_name);
...
void do_objdump (CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("Syntax: objdump <ALL|AREA> <filename>\n\r",ch);
		return;
	}
...
send_to_char ("OBJDUMP: header printed.\n\r",ch);
    	
    	/* now print whatever the player wants */    	
    	if (!str_cmp (dumptype, "all"))
	{
send_to_char ("OBJDUMP: printing ALL.\n\r",ch);
			for (pArea = area_first; pArea != NULL; pArea = pArea->next)
				printarea (fp, pArea);
	}
		else
	{
send_to_char ("OBJDUMP: printing AREA.\n\r",ch);
			printarea (fp, ch->in_room->area);
	}
		fclose (fp);
send_to_char ("OBJDUMP: fp closed.\n\r",ch);
