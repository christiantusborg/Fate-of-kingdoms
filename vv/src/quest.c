...
"The Sceptre of Courage is lieing here, waiting to be returned to its
owner."},

{"quest crown", 	"the Crown of Wisdom", 
"The Crown of Wisdom is lieing here, waiting to be returned to its
owner."},

{"quest gauntlet", 	"the Gauntlets of Strength", 
"The Gauntlets of Strength are lieing here, waiting to be returned to its
owner."},
...
*/
  ...
  { "Bracer of Magic",	      "bracer magic",   900, TRUE, 136, 0},
  { "Crown of Horns",	      "crown horn",	750, TRUE, 137, 0},
  { "Ring of Newbie",	      "ring newbie",    500, TRUE, 139, 0},
  { "10,000 Gold Pieces",     "10000 gold",	500, FALSE, 10000,&ch->gold},
   ...
   send_to_char("JOB commands: Info, Time, Request, Complete, Quit, List, and Buy.\n\r",ch);
	...
	if (IS_SET(ch->act,PLR_QUESTOR))
	{
		...
		ch->questgiver->short_descr != NULL) {
	        sprintf(buf,"Your job is ALMOST complete!\n\rGet back to %s before your time runs out!\n\r",ch->questgiver->short_descr);
		send_to_char(buf, ch);
	    }
		...
		if (questinfoobj != NULL) {
		    sprintf(buf,"You are on a job to recover the fabled %s!\n\r",questinfoobj->name);		
		    send_to_char(buf, ch);
		}
		...
		if (questinfo != NULL) {
		    sprintf(buf,"You are on a job to slay the dreaded %s!\n\r",questinfo->short_descr);
		    send_to_char(buf, ch);
		}
	...
	if (!IS_SET(ch->act,PLR_QUESTOR))
	{
	    send_to_char("You have no pending job right now.\n\r",ch);
	    if (ch->nextquest > 1) {
	        sprintf(buf,"There are %d minutes remaining until you can go on another job.\n\r",ch->nextquest);
		send_to_char(buf, ch);
	    }
	    else if (ch->nextquest == 1) {
		sprintf(buf, "There is less than a minute remaining until you can go on another job.\n\r");
		send_to_char(buf, ch);
	    }
	}
        else if (ch->countdown > 0)
        {
	    sprintf(buf, "Time left for current job: %d\n\r",ch->countdown);
	    send_to_char(buf, ch);
	}
	return;
    }
	...
	if (IS_SET(ch->act,PLR_QUESTOR))
	{
	    sprintf(buf, "But you're already on a job!");
	    do_say(questman, buf);
	    return;
	}
	if (ch->nextquest > 0)
	{
	    sprintf(buf, "There is nothing you can do for me right now, %s",ch->name);
	    do_say(questman, buf);
	...
	sprintf(buf, "Thank you %s, you are truly wise in politics.",ch->name);
	if (!IS_SET(ch->act,PLR_QUESTOR))
	do_say(questman, buf);
        ch->questmob = 0;
	ch->questobj = 0;
        ...
        if (ch->questmob > 0 || ch->questobj > 0)
	{
	    ...
	    sprintf(buf, "You have %d minutes to complete this job.",ch->countdown);
	    do_say(questman, buf);
	...
	{
	    sprintf(buf, "I never sent you on a job! Perhaps you're thinking of someone else.");
	    do_say(questman,buf);
	    return;
	}
	...
	int level;
		...
		sprintf(buf, "Congratulations on completing your job!");
		do_say(questman,buf);
	 ...
	 return;
          ...
          && ch->countdown > 0)
	    {
		sprintf(buf, "You haven't completed the job yet, but there is still time!");
		do_say(questman, buf);
		return;
	    }
     }
	if (ch->nextquest > 0)
	{
	    sprintf(buf,"But you didn't complete your job in time!");
	}
	else sprintf(buf, "You have to request a job first, %s.",ch->name);
	do_say(questman, buf);
	return;
    }
	...
	if (IS_SET(ch->act,PLR_QUESTOR))
	{
	    ...
	    sprintf(buf, "Your job is over, but for your cowardly behavior, you may not quest again for 30 minutes.");
	    do_say(questman,buf);
		    return;
        }
	else
...
void generate_quest(CHAR_DATA *ch, CHAR_DATA *questman)
{
    ...
    {
	sprintf(buf, "I'm sorry, but I don't have any job for you at this time.");
	do_say(questman, buf);
...
void quest_update(void)
{
	...
	continue; 
        ...
        else if (IS_SET(ch->act,PLR_QUESTOR))
        {
            ...
            {
                ch->nextquest = 10;
                sprintf(buf,"You have run out of time for your job!\n\rYou may try again in %d minutes.\n\r",ch->nextquest);
		send_to_char(buf, ch);
                REMOVE_BIT(ch->act,PLR_QUESTOR);
                ch->questgiver = NULL;
                ch->countdown = 0;
...
void do_qminlevel( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "Minimum level for quest is now %d\r\n",quest->min_level);
    send_to_char(buf,ch);
...
void do_qmaxlevel( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "Maximum level for quest is now %d\r\n",quest->max_level);
    send_to_char(buf,ch);
...
void do_qname( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "Quest name set to: %s\r\n", quest->name);
    send_to_char(buf,ch);
...
void do_qlist( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "{BN{bame {W: %s\r\n", quest->name ? quest->name : "{W({wnone{W){n" );
    send_to_char(buf, ch);
    sprintf(buf, "{BS{bhort {BD{bescription {W:{n %s\r\n", quest->short_name ? quest->short_name : "{W({wnone{W){n" );
    send_to_char(buf, ch);
    sprintf(buf, "{BL{bong {BD{bescription {W:{w %s\r\n", quest->long_name ? quest->long_name : "{W({wnone{W){w" );
  send_to_char(buf, ch);
    sprintf(buf, "{BP{blayer {BP{brotection {W: {w%s      {BR{bunning {W:{w %s\r\n",
                 quest->protection ? "YES" : "NO",
                 quest->closed ? "YES" : "NO" );
    send_to_char(buf, ch);
    sprintf(buf, "{BM{binimum {BL{bevel {W: {w%d      {BM{baximum {BL{bevel {W: {w %d\r\n",
...
void do_qadd( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "The Questmaster welcomes %s to the Quest.",victim->name);
...
void do_qremove( CHAR_DATA *ch, char *argument )
{
   CHAR_DATA  *victim;
    ...
    REVOKE( victim, auth_questtalk );
    victim->pcdata->quest = NULL;
    sprintf(buf, "The Questmaster has removed {B%s{n from the Quest.",victim->name);
    send_to_quest(buf,ch);
...
void do_qapply( CHAR_DATA *ch, char *argument )
{
    ...
    if( !found )
    {
       sprintf(buf, "Nobody is running a quest called %s.\r\n", argument);
       send_to_char(buf,ch);
    }
    else
    {
       sprintf(buf, "You have applied to be in the {g%s{w quest.\r\n",ch->pcdata->quest_apply->name);
       send_to_char(buf,ch);
       sprintf(buf, "%s has applied for your quest.\r\n", ch->name);
...
void do_qlistp( CHAR_DATA *ch, char *argument )
{
    ...
    if( ch->pcdata->quest == NULL )
    {
       ...
       for ( d = descriptor_list; d != NULL; d = d->next )
       {
               ...
               victim->pcdata->quest )
           {
               ...
               sprintf(buf,"{BN{bame{W:{G %-20s{BQ{buest{BM{baster{W:{G %-12s {BL{bevels{W:{G %d-%d%s%s{w\r\n",
    ...
    else
    {
       ...
       sprintf(buf,"Other players in quest %s are{W:{w\r\n",ch->pcdata->quest->name);
       send_to_char(buf,ch);
...
void do_qquit( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "%s has quit from the Quest.",ch->name);
