...
if ( ( pObjIndex->limit == -1 ) || ( pObjIndex->count <= pObjIndex->limit ) )
 ...
 return;
    ...
    sprintf(log_buf, "MOB: mpoloadt %s: Loading %s",ch->name,obj->name);
    wiznet(log_buf,ch,NULL,WIZ_PROG,0,0);    
//**OLD***
...
void do_opdamage( OBJ_DATA *obj, char *argument )
{
    ...
    if( !str_cmp( target, "all" ) )
	fAll = TRUE;
    else if( ( victim = get_char_room( NULL, obj->carried_by->in_room, target ) ) == NULL )
	return;
...
void do_kdeposit( CHAR_DATA *ch, char *argument )
 {
    ...
    if (arg[0] == '\0')
     {
      ...
      return;
	     ...
	     if (!strcmp(arg, kingdom_table[1].name)) 	{kingdom=1;}
	else if (!strcmp(arg, kingdom_table[2].name)) 	{kingdom=2;}
	else if (!strcmp(arg, kingdom_table[3].name)) 	{kingdom=3;}
	else if (!strcmp(arg, kingdom_table[4].name)) 	{kingdom=4;}
	else if (!strcmp(arg, kingdom_table[5].name)) 	{kingdom=5;}
	else if (!strcmp(arg, kingdom_table[6].name)) 	{kingdom=6;}
	else if (!strcmp(arg, kingdom_table[7].name))	{kingdom=7;}
    ...
    if ( arg2[0] == '\0' )
     {
      send_to_char( "{cDeposit how much?{x\n\r", ch);
      return;
     }
    ...
    if (amount > ch->gold)
     {
      send_to_char("{cThat's more than you've got.{x\n\r",ch);
      return;
     }
 
    sprintf(log_buf, "MOB: kdeposit %s: To kingdom %s of %d ",ch->name,arg,amount);
    wiznet(log_buf,ch,NULL,WIZ_PROG,0,0);
   
    kingdom_table[kingdom].gold += amount;
    ch->gold -= amount;
