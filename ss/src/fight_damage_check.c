...
bool check_pixie_quickly(CHAR_DATA *victim)
{
	if ((victim->race == race_lookup("pixie")) && (number_percent() < 25 ))
	  {
...
bool check_cheat_damage(CHAR_DATA *ch,int dam)
{
  ...
  char buf[MAX_STRING_LENGTH];
    ...
    if ( dam > 4000 && !IS_IMMORTAL(ch) && !IS_NPC(ch))
    {
	bug( "Damage: %d: more than 4000 points!{x", dam );
	dam = 4000;
	...
	if (!IS_IMMORTAL(ch))
	{
		...
		if (get_eq_char( ch, WEAR_WIELD ) == NULL)
			xprintf(buf,"%s is doing %d damage with a spell.",ch->name,dam);
		else
	 ...
	 if((victim->race == race_lookup("pixie")) && victim->level < 10)
	  {
        ...
        if((victim->class == class_lookup("monk")) && victim->level < 20)
	  {
		if(victim->pcdata->learned[gsn_dodge] < 50)
			victim->pcdata->learned[gsn_dodge] = 50; 	
         	chance += 40;
