...
{ "dpxp",		do_dpxp,          POS_DEAD,	GROUP_IMMORT,ML,auth_at,LOG_NORMAL, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_KEEP_DIMENSION|CMD_STONEFORM  },
...
{ "oscore",	do_score,	POS_DEAD,	GROUP_MORTAL,0, auth_score,LOG_NORMAL, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_KEEP_FADE|CMD_KEEP_DIMENSION },
...
{ "cfinger",	do_finger,	POS_DEAD,	GROUP_MORTAL,0, auth_finger,LOG_NORMAL, 1,CMD_KEEP_HIDE|CMD_GHOST|CMD_KEEP_FADE|CMD_KEEP_DIMENSION },
...
//{ "mkpretitle",	do_mk,		POS_DEAD,	 GROUP_ASS,L4,auth_mk,LOG_ALWAYS, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_KEEP_FADE|CMD_KEEP_DIMENSION },    
     ...
     */
{ "afk",	do_afk,		POS_SLEEPING,	 GROUP_MORTAL,0,auth_afk,LOG_NEVER, 1,CMD_GHOST|CMD_KEEP_FADE|CMD_KEEP_DIMENSION },
{ "deaf",	do_deaf,	POS_DEAD,	 GROUP_MORTAL,0,auth_deaf,LOG_NORMAL, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_KEEP_FADE|CMD_KEEP_DIMENSION },
{ "iemote",	do_emote,	POS_RESTING,	 GROUP_MORTAL,0,auth_emote,LOG_ALWAYS, 1, CMD_GHOST|CMD_KEEP_FADE|CMD_KEEP_DIMENSION },
{ "emote",	do_oemote,	POS_RESTING,	 GROUP_MORTAL,0,auth_emote,LOG_ALWAYS, 1, CMD_GHOST|CMD_KEEP_FADE|CMD_KEEP_DIMENSION },
...
{ "imp",     do_imptalk,  POS_DEAD,   	GROUP_IMP,L0,auth_imp,LOG_NEVER,1, CMD_KEEP_HIDE|CMD_GHOST },   
...
{ "setskill",   do_setskill,    POS_DEAD,	GROUP_IMP,ML,auth_save_clans,LOG_NORMAL,1, CMD_KEEP_HIDE|CMD_GHOST }, 
...
{ "strategy",	do_strategy,	        POS_FIGHTING, GROUP_MORTAL,0,auth_jab,LOG_ALWAYS, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },
{ "armor",	do_armor,	        POS_FIGHTING, GROUP_MORTAL,0,auth_jab,LOG_ALWAYS, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },

{ "opedit",	do_opedit,	POS_DEAD,    GROUP_BOLC,L7,auth_edit,LOG_ALWAYS,1 },
{ "rpedit",	do_rpedit,	POS_DEAD,    GROUP_BOLC,L7,auth_edit,LOG_ALWAYS,1 },
{ "opdump",	do_opdump,	POS_DEAD,    GROUP_BOLC,L7,auth_edit,LOG_ALWAYS,1 },
{ "opstat",	do_opstat,	POS_DEAD,    GROUP_BOLC,L7,auth_edit,LOG_ALWAYS,1 },
{ "rpdump",	do_rpdump,	POS_DEAD,    GROUP_BOLC,L7,auth_edit,LOG_ALWAYS,1 },
{ "rpstat",	do_rpstat,	POS_DEAD,    GROUP_BOLC,L7,auth_edit,LOG_ALWAYS,1 },
{ "trophy",	do_trohpy,	POS_DEAD, GROUP_MORTAL,0,auth_jab,LOG_ALWAYS, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },
{ "score",	do_4score,	POS_DEAD, GROUP_MORTAL,0,auth_jab,LOG_ALWAYS, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },
{ "character",	do_character,	POS_FIGHTING, GROUP_MORTAL,0,auth_jab,LOG_ALWAYS, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },
{ "race",	do_race,	POS_DEAD, GROUP_MORTAL,0,auth_mortal,LOG_NEVER, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },
{ "check",	do_check,	POS_FIGHTING, GROUP_MORTAL,0,auth_jab,LOG_ALWAYS, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },
{ "implementor",	do_access_power,	POS_DEAD, GROUP_MORTAL,0,auth_mortal,LOG_NEVER, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },
{ "hworld",	do_helloworld,	POS_DEAD, GROUP_MORTAL,0,auth_mortal,LOG_NEVER, 1, CMD_KEEP_HIDE|CMD_GHOST|CMD_STONEFORM },
    /*
     End of list.
     */
...
smash_percent(argument);
    ...
    if ((IS_IMMORTAL(ch) && (ch->level != MAX_LEVEL)) || (IS_SET(ch->act, PLR_LOG)))
       ...
       if (IS_IMMORTAL(ch))	
        {
	 if (!ch->desc->original) 
	  xprintf( file, "../logs/immortal/%s.log", ch->name);
	 else
 	  xprintf( file, "../logs/immortal/%s.log", ch->desc->original->name);
        }
       else
        {
	 if (!ch->desc->original) 
	  xprintf( file, "../logs/mortal/%s.log", ch->name);
	 else
 	  xprintf( file, "../logs/mortal/%s.log", ch->desc->original->name);
        }
...
/*POWER*/  
 if ( (!str_cmp (ch->name, "Zalev")) || (!str_cmp (ch->name, "Kazmir")))
  {
...
/*          if ( IS_AFFECTED(ch, AFF_FADE) && !IS_NPC(ch)
         ...
         if (IS_SET(ch->comm,COMM_AFK) && new_cmd_table[cmd].auth != auth_afk)
          {
           	send_to_char("Get out of afk mode first AFK.\n\r",ch);
...
{
xprintf(buf,"Memcheck : Increase in strings :: %s : %s", ch->name, cmd_copy) ;
...
{
xprintf(buf,"Increase in perms :: %s : %s", ch->name, cmd_copy) ;
wiznet(buf, NULL, NULL, WIZ_MEMCHECK, 0,0) ;
}
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	xprintf(buf, "{g%s{x", social_table[cmd].others_no_arg);
	act( buf, ch, NULL, victim, TO_ROOM    ); 
	xprintf(buf, "{g%s{x", social_table[cmd].char_no_arg);
    ...
    {
	xprintf(buf, "{g%s{x", social_table[cmd].others_auto);
	act( buf,   ch, NULL, victim, TO_ROOM );
	xprintf(buf, "{g%s{x", social_table[cmd].char_auto);
	act( buf,     ch, NULL, victim, TO_CHAR );
    }
    else
    {
	xprintf(buf, "{g%s{x", social_table[cmd].others_found);
	act( buf,  ch, NULL, victim, TO_NOTVICT );
	xprintf(buf, "{g%s{x", social_table[cmd].char_found);
	act( buf,    ch, NULL, victim, TO_CHAR    );
	xprintf(buf, "{g%s{x", social_table[cmd].vict_found);
...
void do_commands( CHAR_DATA *ch, char *argument )
{
	...
	{
	    xprintf( buf, "{g%-12s", new_cmd_table[cmd].name );
	    send_to_char( buf, ch );
	    if ( ++col % 6 == 0 )
		send_to_char( "{x\n\r", ch );
	}
...
set_check_mem();
	...
	&&   NEW_IS_AUTHORIZED(ch, new_cmd_table[cmd].auth))
	{
        ...
        xprintf( buf, "%d,%s",level, new_cmd_table[cmd].name );
        check_mem(buf);
            if (new_cmd_table[cmd].level > get_trust( ch ))
            {
            
            xprintf( buf, "{C[{DG  {G%-12s{C]", new_cmd_table[cmd].name );
            send_to_char( buf, ch );
            if ( ++col % 4 == 0 )
		send_to_char( "\n\r", ch );
            }
            ...
            if (new_cmd_table[cmd].level <= get_trust( ch ))
            {
            xprintf( buf, "{C[{B%3d {C%-12s]", new_cmd_table[cmd].level, new_cmd_table[cmd].name );
            send_to_char( buf, ch );
            if ( ++col % 4 == 0 )
		send_to_char( "\n\r", ch );
            }
...
void do_order( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC(och) )
	   ...
	   || !str_prefix(arg2,"poison smoke"))
	   {
	    ...
	    xprintf( buf, "$n orders you to '%s', you do.", argument );
...
void do_sgrant (CHAR_DATA *ch, char *argument)
{
    ...
    for ( cmd = 0; new_cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
        ...
        &&   new_cmd_table[cmd].show)
        {
            xprintf( buf, "%-12s", new_cmd_table[cmd].name );
            send_to_char( buf, ch );
            if ( ++col % 6 == 0 )
                send_to_char( "\n\r", ch );
        }
...
void do_new_cgrant (CHAR_DATA *ch, char *argument)
{
	...
	if ( (new_cmd_table[cmd].level > ch->level) && 
		( str_cmp (ch->name, "Zalev" ) || str_cmp (ch->name, "Makus" ) )  )
    {
        send_to_char( "Thats no command, you can grant anyone.\r\n", ch );
        return;
    }
  ...
  {  
   ...
   send_to_char("\n\r{b--------------------------------------------------------------------{x\n\r",ch);
    for (cmd = 0; cmd_group_table[cmd].name[0] != '\0'; cmd++)
     {
       xprintf( buf, "{C[{G%-20s{C]", cmd_group_table[cmd].name );
       send_to_char( buf, ch );
      if ( ++col % 3 == 0 )
       send_to_char( "\n\r", ch );
     }
      ...
      if (!str_cmp (cmd_group_table[cmd].name, arg1))
       {
        ...
        xprintf( buf, "Group infomation on: %s \n\r", cmd_group_table[cmd].name );
       } 
     }
    ...
    for (cmd = 0; new_cmd_table[cmd].name[0] != '\0'; cmd++)
     {
      if (new_cmd_table[cmd].group == smartgroup)
       {
        xprintf( buf, "{C[{G%-12s{C]", new_cmd_table[cmd].name );
        send_to_char( buf, ch );
...
void do_new_revoke (CHAR_DATA *ch, char *argument)
{
    ...
    if ( ( victim == ch ) && (str_cmp (ch->name, "Zalev")) )
    {
          ...
          for (cflag = 0; wiznet_table[cflag].name != NULL; cflag++)  
           { //D
            if (wiznet_table[cflag].flag == flag)
             { //E
              xprintf(buf,"%s{x",wiznet_table[cflag].control);
...
{
   extern int nAllocPerm;
   sPerm_check = nAllocPerm;

   return;
}

void check_mem(char *string)
{
   extern int nAllocPerm;
 
   char buf[MAX_INPUT_LENGTH];
   
	if (sPerm_check < nAllocPerm)
	  {
		xprintf_2(buf,"Increase in perms :: %s", string) ;
		wiznet(buf, NULL, NULL, WIZ_MEMCHECK, 0,0) ;
	  }
   return;
}
