...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, bool secondary )
{
 ...
 int body_part;
	...
	if (ch->class == CLASS_MNK) 
		dam += ((get_skill(ch,gsn_hand_to_hand)* 1.3));
	...
	if ( dt == gsn_backstab && wield != NULL)
		...
		dam *= 3;
	else if (dt == gsn_cleave)/*Added damage for cleave because i was begged too...and it needed it Zalev 09/11/09*/
		dam *= 3;
		else if (dt == gsn_jab)
		dam /= 2;
...
int xp_compute(CHAR_DATA *gch, CHAR_DATA *victim, int total_levels,int members)
{
  ...
  bool vnum_found = FALSE;
	...
	/*if ( !IS_SET( gch->quest, QUEST_REDBLUERING ) && gch->level >= 45) 
	  {
	  	send_to_char( "You must complete the Knights Quest to gain more levels.\n\r", gch );
		return 0;
	  }
	...
	if(!IS_SET(gch->quest, QUEST_WEAPONSMITH ) && gch->level >= 50) 
	  {
	  	send_to_char( "You must complete the Weaponsmith Quest to gain more levels. (help weaponsmith_quest).\n\r", gch );
		return 0;
	  } 

	if ( !IS_SET( gch->quest, QUEST_GOLEM ) && gch->level >= 55) 
	  {
	  	send_to_char( "You must complete the Dressmakers Quest to gain more levels. (help dress_quest)\n\r", gch );
		return 0;
	  }*/
/*Quest removeed by Zalev 09/11/09 *WE WILL NEVER FORGET* because there is not enough information on the quest for newbies to 
use let alone most players.  Restore when information is complete or quest are changed.*/
/*Removed by Zalev 4/9/09: Low pbase not enough to rp for that many tokens to level to 81 with.*/
	if (gch->level >= 80)
	 return 0;
...
if (double_xp)
  ...
  // New alignment computations for xp 
	...
	if ( IS_EVIL(gch) && IS_GOOD(victim) )
		xp = base_exp * 4;
	if ( IS_EVIL(gch) && IS_NEUTRAL(victim) )
		xp = base_exp * 2;
