...
void violence_update_act	args ( (void) );
...
*/
   if (((current_time - ch->time_last_level)<LAST_LEVEL_TIME) && (ch->level >= 30))
    {
     xprintf_2( log_buf, "%s gained a in really short time and he is %d)",ch->name,ch->level);
     log_string( log_buf );
...
*/
	...
	ch->level += 1;
        ...
        if (ch->level == 80)
          {
            xprintf_2(log_buf, "%s made level 80.", ch->name);
            log_string(log_buf);
          }

        xprintf_2(log_buf, "%s attained level %d.",ch->name,ch->level);
        log_string(log_buf);
	xprintf(buf,"[$N has attained level %d]",ch->level);
...
int hit_gain( CHAR_DATA *ch )
{
   ...
   int number;
	...
	if ( IS_NPC(ch) )
	  ...
	  { //Player hitpoint regenration


		gain =  con_app[get_curr_stat(ch,STAT_CON)].regen;
...
//            do_track(ch,ch->last_fought->name);
  ...
  if (IS_AFFECTED2(ch, AFF2_SUBMISSION))
   if (ch->position != POS_FIGHTING || ch->fighting->submission == 0)
     affect_strip(ch,gsn_submission);
     
  if ((ch->submission > 0) && (ch->fighting == NULL))
   ch->submission = 0;
...
//        CHAR_DATA *victim;
  ...
  if ( ch->race == 13 && ch->in_room != NULL )
    ...
    if ( IS_AFFECTED(ch, AFF_CORRUPTION ) && ch->in_room != NULL )
        {
        	dam = (ch->hit/40);
          damage_spell(ch,ch,dam,gsn_corruption,DAM_NONE,TRUE);
          send_to_char("",ch);
        }

/*New DoT Spell (Exaethian) */       
    if ( IS_AFFECTED(ch, AFF_MISERY ) && ch->in_room != NULL )
        {
        	dam = (ch->hit/30);
          damage_spell(ch,ch,dam,gsn_misery,DAM_NONE,TRUE);
          send_to_char("",ch);
        }
    
    if ( IS_AFFECTED2(ch, AFF2_HELLFIRE ) && ch->in_room != NULL )
    	  {
    	  	dam = (ch->hit/50);
    	  	damage_spell(ch,ch,dam,gsn_hellfire,DAM_NONE,TRUE);
    	  	send_to_char("",ch);
    	  }
    	  
    if ( IS_AFFECTED2(ch, AFF2_LIFE_LEECH ) && ch->in_room != NULL )
        {
        	dam = (ch->hit/50);
          damage_spell(ch,ch,dam,gsn_life_leech,DAM_NONE,TRUE);
          send_to_char("",ch);
        }
			...
			for ( obj = ch->carrying; obj; obj = obj->next_content )
		  	{
		    		...
		    		if (chance <= 10)
		    		if (IS_OBJ_STAT(obj,ITEM_INVENTORY) && obj->wear_loc == WEAR_NONE )
				  {
					OBJ_DATA *obj_copy = create_object( obj->pIndexData, obj->level );
					count = 0;
			    		for ( objcount = ch->carrying; objcount; objcount = objcount->next_content )
			    			if (objcount->pIndexData == obj_copy->pIndexData)
							count++;
					if (count <= 25)
					  {
						//bug(count,0);
						REMOVE_BIT( obj_copy->extra_flags, ITEM_INVENTORY);
	      					SET_BIT( obj_copy->extra_flags,ITEM_SELL_EXTRACT );
	      					obj_to_keeper( obj_copy, ch );
	      				  }
	      				 else
	      				 	extract_obj( obj_copy );
				  }
...
/*           xprintf(buf2, "*** BUG*** %s - %s.", ch->name,obj->name); */
/*           log_string(buf2); */
/*  removes the above just because it is...DUMB! */
 ...
 if ( !IS_SET ( ch->in_room->room_flags, ROOM_ARENA )) {
	...
	for ( paf = ch->affected; paf != NULL; paf = paf_next )
	{
	    ...
	    if ( paf->duration > 0 )
	    {
	    	if ( IS_NPC(ch) && paf->bitvector == AFF_PLAGUE && paf->duration > 10)
	    		paf->duration = 10;
		paf->duration--;
...
/*
	...
	act(buf, obj->carried_by, obj, NULL, TO_CHAR );
        xprintf(buf,"%s is crumbles into dust.\n\r",obj->short_descr);
	act(buf, obj->carried_by, obj, NULL, TO_CHAR );
	extract_obj(obj);
        return;
        }
	...
	for ( ch = wch->in_room->people; ch != NULL; ch = ch_next )
	{
		...
		continue;
          ...
          if (ch->last_fought == wch
            ...
            if ((weapon != NULL) && ((weapon->value[0] == WEAPON_POLEARM) || (weapon->value[0] == WEAPON_LANCE)))
    		{    
    		    do_charge( ch, wch->name );
    		    continue;
    		}
...
void check_copyover( void )
{
 ...
 switch(copyover_counter)
 {
  case -1:
     break;
  case 0:
    xprintf(buf,"***** COPYOVER by System - please remain seated!\n\r");
    for (d = descriptor_list; d != NULL; d = d->next)
  ...
  case 60:
    if (copyover_counter == 1)
        xprintf(buf,"***** COPYOVER IN %i TICK *****\n\r",copyover_counter);
    else
        xprintf(buf,"***** COPYOVER IN %i TICKS *****\n\r",copyover_counter);
  for (d = descriptor_list; d != NULL; d = d->next)
          write_to_buffer(d,buf,0);
  default:
    copyover_counter--; 
