...
/*	else And i dont like it give to much troble
	...
	//check to see if the player is in a room, if not then jsut return
	if (ch->in_room==NULL)
		return;
...
void spell_cure_critical( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    heal = ((ch->level * 25)/10) + dice(15,10);
...
void spell_cure_light( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    heal = ((ch->level * 1)/10) + dice(15,10);
    victim->hit = UMIN( victim->hit + heal, victim->max_hit );
    update_pos( victim );
    send_to_char( "You feel better!\n\r", victim );
    if ( ch != victim )
	send_to_char( "You release healing power into $N.\n\r", ch );
    return;
}
...
void spell_cure_serious( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    heal = ((ch->level * 12)/10) + dice(15,10);
    victim->hit = UMIN( victim->hit + heal, victim->max_hit );
    update_pos( victim );
    send_to_char( "You feel better!\n\r", victim );
    if ( ch != victim )
...
*/
 //if (ch->class == 13)
  //level += 7;
   ...
   {
        ch->mana  +=  victim->mana / 1.15;
        ch->move  +=  victim->move / 1.15;
        victim->mana    /= 1.5;
        victim->move    /= 1.5;
        dam = ((ch->level * 8)/10) + dice(15,10);
     }
...
void spell_hellfire( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  ...
  dam = ((ch->level * 50)/15) + dice(15,10);
/*  Making Hellfire have a DoT portion on it. Not ready yet (Exaethian)*/
    if ( !saves_spell( level, victim,DAM_FIRE ) )
    {
    ...
    {
	    act("The flames do not take hold.",victim,NULL,NULL,TO_ROOM);
	    dam /= 2;
    }
...
void spell_harm( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    dam = ((ch->level * 35)/10) + dice(15,10);
    /*if ( saves_spell( level, victim, DAM_FIRE ) )
        dam /= 2;(Removed by Exaethian. Spell isn't fire. No need for this)*/
    damage_spell( ch, victim, dam, sn, DAM_HARM ,TRUE);
    return;
}
...
void spell_heal( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    int heal;
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    heal = ((ch->level * 37)/10) + dice(15,10);
    victim->hit = UMIN( victim->hit + heal, victim->max_hit );
    update_pos( victim );
    send_to_char( "A warm feeling fills your body.\n\r", victim );
    if ( ch != victim )
	  send_to_char( "You channel healing power into $N.\n\r", ch );
    return;
}
	      ...
	      obj_lose = obj_next)
	{
		...
		if (obj_lose->wear_loc != -1) /* remove the item */
		{
			...
			act("Your skin is seared by $p!",
			    victim,obj_lose,NULL,TO_CHAR);
			dam += (number_range(level/20,obj_lose->level));
			fail = FALSE;
		    }
		...
		else /* drop it if we can */
		{
			...
			act("Your skin is seared by $p!",
			    victim,obj_lose,NULL,TO_CHAR);
			dam += (number_range(level/20,obj_lose->level) / 2);
			fail = FALSE;
		    }
		}
		break;
...
*/
	...
	2 * level / 4, ch, (void *) victim,TARGET_CHAR);
}
