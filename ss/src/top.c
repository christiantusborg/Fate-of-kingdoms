...
//	int number;
  ...
  if (!(fp = fopen (TOP_FILE, "r")))
   {
          xprintf (buf, "Could not open file in order to load top.");
		return;
	}
   ...
   for (Top = 0; Top < 10 ; Top++)
    {
     ...
     xprintf( buf,"<tr><td><span style=\"font-size:11pt\">%d &nbsp;&nbsp;&nbsp;&nbsp; %s </span></td><td> <span style=\"font-size:11pt\"> %d",
    ...
    fprintf(fp, "</table>\n");
    xprintf(buf, "<span>This file last updated at %s GMT Time.</span><br>\n", ((char *) ctime( &current_time )));
    fprintf(fp, buf);
    fprintf(fp, "</div>\n");
    fprintf(fp, "</body>\n");
    fprintf(fp, "</html>\n");
