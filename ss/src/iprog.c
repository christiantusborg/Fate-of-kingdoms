...
DECLARE_IPROG_FUN_GIVE( give_prog_redbluering );
...
DECLARE_IPROG_FUN_FIGHT( fight_prog_redbluering );
...
DECLARE_IPROG_FUN_WEAR(wear_prog_RingDivine);
DECLARE_IPROG_FUN_WEAR(remove_prog_RingDivine);
...
void iprog_set(OBJ_INDEX_DATA *objindex,const char *progtype, const char *name)
{
 ...
 /* insert your functions here */
	 ...
	 objindex->iprogs->wear_prog = wear_prog_Dermostest;
       else if (!str_cmp(name, "wear_prog_RingDivine"))
	 objindex->iprogs->wear_prog = wear_prog_RingDivine;
...
//	 objindex->iprogs->wear_prog = wear_prog_Coldresist;
   ...
   if (!str_cmp(progtype, "remove_prog"))
	 ...
	 objindex->iprogs->remove_prog = remove_prog_Dermostest;
       else if (!str_cmp(name, "remove_prog_RingDivine"))
	 objindex->iprogs->remove_prog = remove_prog_RingDivine;
       else
	 {
	   bug("Load_improgs: 'I': Function not found for vnum %d",
	       objindex->vnum);
   ...
   if (!str_cmp(progtype, "give_prog"))
     {
       ...
       else if (!str_cmp(name, "give_prog_redbluering"))
         objindex->iprogs->give_prog = give_prog_redbluering;
       else
         {
           bug("Load_improgs: 'I': Function not found for vnum %d",
               objindex->vnum);
   ...
   if (!str_cmp(progtype, "fight_prog"))
	 ...
	 objindex->iprogs->fight_prog = fight_prog_Dermostest;
       else if (!str_cmp(name, "fight_prog_redbluering"))
	 objindex->iprogs->fight_prog = fight_prog_redbluering;
...
kingdom_table[KINGDOM_MORDUKHAN].obj_vnum)
    ...
    xprintf( buf, "You see %s forming again slowly.\n\r", 
	...
	if (ch->fighting != NULL)
	...
	else if (!str_cmp(speech, "valere") && (get_eq_char(ch,WEAR_HOLD) == obj))
	  {
  		if (number_range(1,100) <= 10)		
  			obj_cast_spell(skill_lookup("word of recall"),ch->level,ch,ch,NULL);
  		else
  			obj_cast_spell(skill_lookup("teleport"),ch->level,ch,ch,NULL);
	  }
...
char buf[160];
  ...
  if ( obj->value[1] == 0 )  {
    xprintf( buf, "Dice rolls to %d.", result1 );
    act( buf, ch, NULL, NULL, TO_CHAR );
    act( buf, ch, NULL, NULL, TO_ROOM );
    return;
  }
  xprintf( buf, "Two dice roll to %d and %d.", result1, result2 );
  act( buf, ch, NULL, NULL, TO_CHAR );
  act( buf, ch, NULL, NULL, TO_ROOM );
  return;
}
...
void wear_prog_RingDivine(OBJ_DATA *obj, CHAR_DATA *ch)
{
  AFFECT_DATA af;
...
void remove_prog_RingDivine(OBJ_DATA *obj, CHAR_DATA *ch)
{
  if (is_affected(ch, gsn_bless))
    {
      affect_strip(ch, gsn_bless);
      send_to_char("You feel less righteous.\n\r", ch);
    }
}     
...
bool sac_prog_vebrek(OBJ_DATA *obj, CHAR_DATA *ch)
{
  act("The gods are infuriated!",ch,NULL,NULL,TO_CHAR);
  act("The gods are infuriated!",ch,NULL,NULL,TO_ROOM);
  damage( ch, ch, 500, TYPE_HIT, DAM_HOLY, TRUE);
...
void give_prog_redbluering(OBJ_DATA *obj, CHAR_DATA *ch, CHAR_DATA *mob)
{

if (obj->pIndexData->vnum != 12218)
	return;
...
send_to_char("{cYour the ring from Gintid don't like to be give away{x.\n\r",ch);
 act("{y'{YGintid Ring {y' don't like to be give away.",ch,NULL,NULL,TO_ROOM);
 

	    send_to_char("{The Gintid vaporized with that extraordinary force.{x\n\r",ch);
	    if (obj)
	      extract_obj(obj);

/*//damage_spell(ch,ch,dam,gsn_torturer,DAM_HOLY,TRUE);*/

}

void fight_prog_redbluering(OBJ_DATA *obj, CHAR_DATA *ch)
{
...
if (IS_NPC(ch))
  ...
  if ( ch == NULL || ch->fighting == NULL )
	...
	if (count > 1)
	 {
			count--;
			send_to_char("{cYour the ring from Gintid don't like to be around an other of it kind{x.\n\r",ch);
			send_to_char("{cThis attempt to cheat has been logged sac all but one Gintid ring{x.\n\r",ch);
			send_to_char("{cbefore you get caught.  Punishment is slay!{x.\n\r",ch);
			/*damage_spell(ch,ch,ch->level * 10 * count,gsn_torturer,DAM_HOLY,TRUE);*/
			
   }
  chance = number_range(1,1000);
   if (chance <= 200)
   	{
     send_to_char("{cYour the ring from Gintid shines{x {bblue{x.\n\r",ch);
      act("{cThe ring from Gintid on $n finger shines {bblue{x.",ch,NULL,NULL,TO_ROOM); 	
   	obj_cast_spell(gsn_cure_serious,ch->level, ch, ch, obj );
	}

   if (chance == 1000)
   {
         send_to_char("{cYour the ring from Gintid shines{x {Rred{x.\n\r",ch);
      act("{cthe ring from Gintid shines  on $n finger  {Rred{x.",ch,NULL,NULL,TO_ROOM);
   	obj_cast_spell(gsn_sanctuary,ch->level, ch, ch, obj );
	}
	
	if (chance == 750)
   {
         send_to_char("{cYour the ring from Gintid shines{x {Yyellow{x.\n\r",ch);
      act("{cthe ring from Gintid shines  on $n finger  {Yyellow{x.",ch,NULL,NULL,TO_ROOM);
   	obj_cast_spell(gsn_aid,ch->level, ch, ch, obj );
	}
