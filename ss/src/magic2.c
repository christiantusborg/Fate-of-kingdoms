...
// Has a slight damage reduction due to its other effects (mana/move draining, healing)
   {
     dam = ((ch->level * 45)/15) + dice(15,10);
     victim->mana       /= 2;
     victim->move       /= 2;
     ch->hit            += dam / 2;
    }
...
void spell_restoring_light( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (IS_AFFECTED(victim,AFF_MISERY))
    	  {
    	  	affect_strip( victim, gsn_misery );
		      send_to_char( "Your misery subsides.\n\r", victim);
		      act( "The misery of $N subsides.", ch, NULL, victim, TO_NOTVICT );
    	  }
    if (IS_AFFECTED2(victim,AFF2_HELLFIRE))
    	  {
    	  	affect_strip( victim, gsn_hellfire );
		send_to_char( "The flames engulfing you are quelched.\n\r", victim);
		act( "The flames englufing $N are quelched.", ch, NULL, victim, TO_NOTVICT );	    	  	
    	  }	    	  	
