...
void check_magic_shield(CHAR_DATA *ch,CHAR_DATA *victim,int dam)
{
...
void check_death(CHAR_DATA *victim)
{
	if (victim->hit > 0)
		return;
...
void flee_wimpy(CHAR_DATA *ch,CHAR_DATA *victim)
{
	if ( IS_NPC(victim) && victim->wait < PULSE_VIOLENCE / 2)
    {
...
void death_title(CHAR_DATA *ch,CHAR_DATA *victim,int dam_type)
{
...
if ( ch==victim )
{
  if ((( dam_type == DAM_HUNGER || dam_type == DAM_THIRST)) && (!IS_NPC(ch))){
    ...
    xprintf(buf, " the Starving Beggar");
    ...
    if (number_percent() < 50) {
    char buf[MAX_STRING_LENGTH];
    xprintf(buf, " the Besmirched");
    free_string( ch->pcdata->title );ch->pcdata->title = str_dup( buf );}
    } 
  else if (!IS_NPC(ch)){
    char buf[MAX_STRING_LENGTH];
    xprintf(buf, " the Suicidal Maniac");
    free_string( ch->pcdata->title );ch->pcdata->title = str_dup( buf );}
...
bool pk_honor(CHAR_DATA *ch,CHAR_DATA *victim)
{
...
bool damage_spell( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int dam_type, bool show ) {

    char log_buf[MAX_STRING_LENGTH];
    bool immune = FALSE;
    bool VictimWanted;
    VictimWanted=FALSE;
		float spell_damage_modify = 0.40;
		float spell_damage_arcane_power = 0.60;
			...
			if ( victim->fighting == NULL )
			  {
				...
				if ( IS_NPC( victim ) && HAS_TRIGGER_MOB( victim, TRIG_KILL ) )
					p_percent_trigger( ch, NULL, NULL, NULL, victim, NULL, TRIG_KILL );
     ...
     * Damage modifiers.
     */
if (dt != skill_lookup("storm vengeance"))
{
  ...
  dam = affect_immune_damage(victim,dam_type,dam);
    ...
    if (get_curr_stat(ch,STAT_INT) != 15)
    	dam += (dam * int_app[get_curr_stat(ch,STAT_INT)].spelldamage) / 100;

   if (ch->level < skill_table[gsn_arcane_power].skill_level[ch->class])
   	dam = dam * spell_damage_modify;
   else
   	 dam = dam * spell_damage_arcane_power;
	...
	check_death(victim);
	update_pos( victim );
	check_magic_shield(ch,victim,dam);
	...
	switch( victim->position )
	 {
		...
		case POS_INCAP:
			...
			send_to_char("{cYou are incapacitated and will slowly die, if not aided.{x\n\r",victim );
		break;
	  case POS_STUNNED:
			act( "{c$n is stunned, but just might recover.{x",victim, NULL, NULL, TO_ROOM );
	...
	if ( victim->position == POS_DEAD )
	 {
		...
		if ( !IS_NPC(victim) )
		 {
			xprintf( log_buf, "%s killed by %s at %d",victim->name,(IS_NPC(ch) ? ch->short_descr : ch->name),victim->in_room->vnum );
			log_string( log_buf );
		 }

		pk_honor(ch,victim);
		death_title(ch,victim,dam_type);
		stop_fighting( victim, FALSE );
		raw_kill(ch,victim);
	 }
...
//					mp_percent_trigger( victim, ch, NULL, NULL, TRIG_KILL);
  ...
  dam = get_drow_damage_night(ch,dam);
...
/*  adds codes for mobs to use magical items */
  ...
  if ( dt != TYPE_HUNGER &&  dt >= TYPE_HIT && ch != victim && dam > 0)   
   ...
   if(is_affected(victim,skill_lookup("ironwill")))
	...
	if ( !IS_NPC(victim) )
	{
            xprintf( log_buf, "%s killed by %s at %d",
                victim->name,
