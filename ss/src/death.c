   ...
   if (!IS_NPC(ch))
    {
     ...
     xprintf(pfile,"/home/fate/.ftpbak/ss/Backup/death/%s.bak",capitalize( ch->name ) );
    ...
    if ( IS_NPC(ch) )
    {
        ...
        corpse->timer   = number_range( 20, 40 );
...
*/
    ...
    xprintf( buf, corpse->name, ch->name );
    ...
    if ( !IS_NPC( ch ) )
        {
         xprintf( log_buf, "DEATH LOG EQ: player: %s obj [vnum: %d]NAME: %s SHORTDESC: %s",ch->name,obj->pIndexData->vnum,obj->name,obj->short_descr);
         Log_system (ch,buf,LOG_DEATH,0,0);
         log_string( log_buf );
        }
...
if (IS_SET( obj->extra_flags, ITEM_SELL_EXTRACT))
 {
	wiznet("ITEM_SELL_EXTRACT 1 \n\r",ch,NULL,WIZ_CHEAT,0,0);
	extract_obj( obj );
	continue;
}
	...
	if (( (obj->pIndexData->limit != -1) 
          && (!IS_NPC( ch ) ) ) && (chance == 1) )
         {
           ...
           xprintf(buf,"limit 4 %d\n\r",chance); 
          	extract_obj( obj );
           	wiznet(buf,ch,NULL,WIZ_CHEAT,0,0);
            continue;
          }
      ...
      if ( IS_SET( obj->extra_flags, ITEM_INVENTORY )  )
      	 {
      	extract_obj( obj );
      	wiznet("ITEM_INVENTORY 3\r\n",ch,NULL,WIZ_CHEAT,0,0);
      }
      else if (IS_SET( obj->extra_flags, ITEM_SELL_EXTRACT ))
      	{
      		wiznet("ITEM_SELL_EXTRACT 4 \r\n",ch,NULL,WIZ_CHEAT,0,0);
      		extract_obj( obj );
      	}
        else if (floating)
        {
	...
	if (!IS_NPC(ch) && !IS_NPC(victim) )
	  {
		...
		ch->pcdata->score[PLAYER_KILL]++;
  		if ((!IS_NPC(ch)) && (number_percent() < 5))
  		  {
    			char buf[MAX_STRING_LENGTH];
    			xprintf(buf, " the Nefarious %s", capitalize(class_table[ch->class].name ));
