...
void kill_remove_guardians	args ((CHAR_DATA *ch) );
...
DECLARE_MPROG_FUN_GIVE( give_prog_jeweler );
...
/*  adds 2 lines for generic greet prog */
   ...
   if (!str_cmp(progtype, "give_prog"))
	 ...
	 mobindex->moprogs->give_prog = give_prog_weaponsmith;
       else if (!str_cmp(name, "give_prog_jeweler"))
	 mobindex->moprogs->give_prog = give_prog_jeweler;
  ...
  if ( IS_NPC( ch ) || IS_IMMORTAL(ch))  {
   ...
   if(IS_IMMORTAL(ch)) {
	...
	do_say(mob,"Your attempt have been logged.");
      xprintf(log_buf,"%s has tried to solve the weaponsmith quest.",ch->name);
...
void give_prog_wizard(CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj)
{
  ...
  if ( obj->pIndexData->vnum == OBJ_VNUM_SPEC_WEAPON_S  ||
	...
	xprintf(log_buf,"%s has solved the weaponsmith quest.",ch->name);
            wiznet(log_buf,NULL,NULL,WIZ_QUESTS,0,0);
      log_string( log_buf );
  	 act_color( "{Y( Well done!  You receive 2500 experience points.){w\n\r",
             ch, NULL, NULL, TO_CHAR, POS_SLEEPING, 0 );
	xprintf(buf,"all {YAndor shouts: Thanks, %s for helping me!!\n",ch->name);
	do_echo(ch,buf);
         gain_exp( ch, 2500 );
      }
      return;
...
OBJ_DATA *weapon;
  ...
  if ( (ch == mob->char_in_mind) && (mob->status==MOB_STAT_WAITING_ANSWER) )
  {
	...
	do_say(mob, "Here.  Take this weapon to the Tatorious the wizard in the High Tower.");
        ...
        xprintf( buf, weapon->pIndexData->extra_descr->description, ch->name );
...
int chosen = 0,correct = 1;
   ...
   if ( mob->status == RELIG_CHOSEN )
   {
    if (( ch->religion > 0) && (ch->religion < MAX_RELIGION) )
        {
       xprintf(buf,"You are already in the way of %s",
    ...
    ch->religion = chosen;
    xprintf(buf,"From now on and forever, you are in the way of %s",
                religion_table[ch->religion].leader);
    do_say(mob,buf);
    return;
   }
   do_say(mob,"Hmm yes, religion. Do you really interested in that?.");
 ...
 */
  ...
  if ( ch->pIndexData->vnum == MOB_VNUM_CITYGUARD )  {
    ...
    if (!str_cmp(speech, "Stupid, you are responsible for the terrible look in this dump")
	&& ch->status != 6)  
    {
      ch->status = 6;
      switch(dice(1,3))  {
	...
	break;
      case 3:
	interpret( mob, "Bored. Send your daughter or leave me alone." );
	break;
      }
  }
...
void give_prog_keeper(CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj)
{
  ...
  char log_buf[MAX_STRING_LENGTH];
   if(IS_IMMORTAL(ch)) {
	do_say(mob,"You are an immortal go back to your own business.");
	do_say(mob,"Your attempt have been logged.");
      xprintf(log_buf,"%s has tried to solve the dress quest.",ch->name);
            wiznet(log_buf,NULL,NULL,WIZ_QUESTS,0,0);
      log_string( log_buf );
	return;
	}
...
void greet_prog_queen(CHAR_DATA *mob, CHAR_DATA *ch)
{
  ...
  if ((obj = get_obj_carry(ch,"rug" , NULL)) == NULL)
    ...
    do_say(mob, "A rug's fringes blowing in the breezer, Like the Efreeti magic.");
  
    do_emote(mob,"unlocks a shiny box.");
    do_emote(mob,"open a shiny box.");
    do_emote(mob,"gets paper from box.");
      ...
      xprintf(buf, "papers %s", ch->name);
      do_give(mob, buf);
    do_emote(mob,"closes a shiny box.");
    do_emote(mob,"locks a shiny box.");
    }
}
...
void greet_prog_golem(CHAR_DATA *mob, CHAR_DATA *ch)
{
  ...
  if ((obj = get_obj_carry(ch, arg , NULL)) == NULL)
    {
      		interpret(mob, "grin");
      		do_say(mob, "I am almost complete, still missing a small part...");
      		do_say(mob, "You forgot to bring the queen's key in....");
      		do_murder(mob, ch->name);
    }
  else
  	do_say(mob, "Did get it?, give me the queen's key in....");
...

{
  ...
  if (obj->pIndexData->vnum == 9759)
	...
	xprintf(log_buf,"%s has solved the golem quest.",ch->name);
            wiznet(log_buf,NULL,NULL,WIZ_QUESTS,0,0);
      log_string( log_buf );
     ...
     saangreal = create_object(get_obj_index(188), 0);
//      obj->timer = 500;
      obj_from_char(obj);
      obj_to_char(saangreal, mob);
      xprintf(buf,"saangreal %s",ch->name);
      do_give(mob, buf);
      do_say(mob, "This has special powers, use it well.");
      xprintf(buf,"all {YAriana shouts: Thanks brave %s!!\n",ch->name);
	do_echo(ch,buf);
	}
      obj_from_char(obj);
      extract_obj(obj);
...
case 20: /* outland stranger handles arena */
	...
	xprintf(buf,"I can bring you to one for %d gold.",ch->level*2);
	do_say(mob, buf);
...
void speech_prog_sailor(CHAR_DATA *mob, CHAR_DATA *ch, char *speech)
{
  ...
  if (!str_cmp(speech, "yes"))
      ...
      else
	{
	  ...
	  if (sroll > proll)
	    {
	      xprintf(buf, "$N lost %d gold!", amount);
	      xprintf(arg, "You lost %d gold!!", amount);
	    ...
	    {
	      xprintf(buf, "$n won %d gold!", amount);
	      xprintf(arg, "You won %d gold!!", amount);
...
*/
  ...
  mob->kingdom_rank = 0;
  xprintf(buf, "I have failed trying to kill %s, I gasp my last breath.",
...
mob->kingdom = KINGDOM_XELENIA;
  ...
  mob->kingdom_rank = 0;
  xprintf(buf,"%s is being attacked!!!",pArea->name);
  do_gd(mob, buf);
  kill_remove_guardians(mob);
  return FALSE;
}
...
*/
  if ((ch->kingdom == KINGDOM_GONDOR) && (mob->kingdom == KINGDOM_GONDOR)){
    xprintf(buf,"bow %s",ch->name);
    interpret(mob,buf);
    return;
  }

  if ((ch->kingdom == KINGDOM_ALDON) && (mob->kingdom == KINGDOM_ALDON)){
    xprintf(buf,"Hail brother of magic, %s",ch->name);
    do_say(mob,buf);
    return;
  }
...
void give_prog_jeweler(CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj)
{
...
char log_buf[MAX_STRING_LENGTH];
 ...
 if (( obj->pIndexData->vnum == 12007 )
   || ( obj->pIndexData->vnum == 12017 )
   || ( obj->pIndexData->vnum == 12217 ))
     {
      if (((obj1 = get_obj_carry(mob,"QO1" , NULL)) != NULL)
       && ((obj2 = get_obj_carry(mob,"QO2" , NULL)) != NULL)
       && ((obj3 = get_obj_carry(mob,"QO3" , NULL)) != NULL))
       {
         extract_obj( obj1);
         extract_obj( obj2);
         extract_obj( obj3);
					interpret(mob, "smile" );
					do_say(mob, "It seems like I have all I need now.");
					interpret(mob, "pmote starts making something." );
					loadobj = create_object(get_obj_index(12218), 0);
					obj_to_char(loadobj, mob);
					xprintf(buf,"QOF %s",ch->name);
					do_give(mob, buf);      
					if ( !IS_SET( ch->quest, QUEST_REDBLUERING ) )  {
						SET_BIT( ch->quest, QUEST_REDBLUERING );
					xprintf(log_buf,"%s has solved the knight quest.",ch->name);
          wiznet(log_buf,NULL,NULL,WIZ_QUESTS,0,0);
      		log_string( log_buf );
					xprintf(buf,"all {YSerinnia shouts: %s solved the knight quest.\n",ch->name);
					do_echo(ch,buf);
  	 			act_color( "{Y( Well done!  You receive 3000 experience points.){w\n\r",ch, NULL, NULL, TO_CHAR, POS_SLEEPING, 0 );
          send_to_char("{cCheat note: You may only carry one Gintid ring with you.  If you are caught carrying more{x\n\r",ch);
		    	send_to_char("{cthan one the damage code for carrying more than one will be turned back on and you will be slain!{x\n\r",ch);
          gain_exp( ch, 3000 );
        }
       }
      else
      	do_say(mob, "This really is a nice item, I think I can use it for something useful, I just need more useful items to complete it.");
     }
  else
   {
   	do_say(mob, "This item is no good when I am trying to make magical rings.");
   } 
